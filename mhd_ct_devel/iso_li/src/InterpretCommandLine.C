			 ...
			 int &RegionLevel, int MyProcessorNumber)
{
  ...
  while (--argc > 0 && (*++argv)[0] == '-')
	...
	/* get beginning of region selection (float coordinate). */
      case 'a':
	argc--;
	//AnalysisParameterFileName = new char[ strlen( *++argv ) + 1];
	//strcpy( AnalysisParameterFileName, (*argv) );
	*AnalysisParameterFileName = (++argv)[0];
	ForceAnalysisOnly = TRUE;
	while (*(argv[0]+1))
	  ++argv[0];
	break;
      case 'b':
...
#ifdef USE_JBPERF
	//
	// -P mode <m>    m=1 next, 2 advance, 4 category, 8 trace, 16 newwrite
