...
/    using the OldBaryonField and BaryonField of the parent.  Make surem
...
//Note: The argument Level in this routine is no longer necessary.  Left in until the
//Fast Sibling Locator is installed in RebuildHierarchy in this version. dcc 01/19/06
int grid::InterpolateBoundaryFromParent(grid *ParentGrid, LevelHierarchyEntry * Level)
{
  //<dcc>
  //fprintf(stderr, "kludge: No Interpolate Boundary\n");
  //return SUCCESS;
  //</dcc>
  int npx=16;
  ...
  int dcci=1, dccj=1, dcck=5;

  if( WriteInThisF(70) == TRUE ){
    char basename[30];
    sprintf(basename,"data70%d%d.grid",MyProcessorNumber,dccCounter2);
  ...
  if (NumberOfBaryonFields > 0) {
    ...
    /*
      ...
      fprintf(stderr,"Note: unsure about which interpolation method for Acceleration Hack.\n");
    */
...
/*      if (HydroMethod == Zeus_Hydro && 
	...
	ParentOld = ParentGrid->BaryonField[field]; // not used
      ...
      if(0==1){	
      fprintf(stderr, "++++++++++++++++++++++++++ %d ++++++++++++++++++++++++++\n", field);
	ParentGrid->BaryonField[field][ (dcci+ParentStartIndex[0]) + 
					(dccj+ParentStartIndex[1])*ParentDim[0] + 
					(dcck+ParentStartIndex[2])*ParentDim[0]*ParentDim[1]]=-300;

	
	ParentOld[ (dcci+ParentStartIndex[0]) + 
		   (dccj+ParentStartIndex[1])*ParentDim[0] + 
		   (dcck+ParentStartIndex[2])*ParentDim[0]*ParentDim[1]]=-300;
	...
	fprintf(stderr, "coef's %f %f zero %d\n", coef1, coef2, Zero);
	fprintf(stderr, "ParentDim, %d %d %d \n ", ParentDim[0], ParentDim[1], ParentDim[2]);
	fprintf(stderr, "ParentTempDim, %d %d %d \n ", ParentTempDim[0], ParentTempDim[1], ParentTempDim[2]);
	fprintf(stderr, "ParentStartIndex, %d %d %d \n ", ParentStartIndex[0], ParentStartIndex[1], ParentStartIndex[2]);
      fprintf(stderr, "interpolate boundary from parent, before combine3d baryon field: [%d] [1,1,5]=%f \n",
	      field,
	      ParentGrid->BaryonField[field][ (dcci+ParentStartIndex[0]) + 
					      (dccj+ParentStartIndex[1])*ParentDim[0] + 
					      (dcck+ParentStartIndex[2])*ParentDim[0]*ParentDim[1]]);

      fprintf(stderr, "interpolate boundary from parent, before combine3d old baryon field: [%d] [1,1,5]=%f \n",
	      field,
	      ParentOld[ (dcci+ParentStartIndex[0]) + 
			 (dccj+ParentStartIndex[1])*ParentDim[0] + 
			 (dcck+ParentStartIndex[2])*ParentDim[0]*ParentDim[1]]);
      ...
      //ParentGrid->IsItShit();
      /*
      fprintf(stderr, "interpolate boundary from parent, after combine3d: [%d] [1,1,5]=%f \n", field,
	      ParentTemp[field][ (dcci) + (dccj)*ParentTempDim[0] + (dcck)* ParentTempDim[0]*ParentTempDim[1]]);
    ...
    if (DualEnergyFormalism && AccelerationHack != TRUE)
	...
	if(ParentGrid->ProcessorNumber != ProcessorNumber && 1==0)
      ...
      FORTRAN_NAME(mhd_interpolate)(MHDParentTemp[0], MHDParentTemp[1], MHDParentTemp[2],
				    ...
				    DxBz, DyBz, DxyBz,
                                    ...
                                    ParentGrid->CellWidth[1],
                                    ParentGrid->CellWidth[2],
				    &MHD_InterpolationMethod, &Step, &dccCounter13);
  ...
  if(WriteInThisF(71) == TRUE ){
    char basename[30];
    sprintf(basename,"data71%d%d.grid",MyProcessorNumber,dccCounter2++);
    FILE * dccptr = fopen(basename,"a");

    
    dccptr = fopen(basename,"a");
