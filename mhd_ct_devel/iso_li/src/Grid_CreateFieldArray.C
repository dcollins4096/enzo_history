...
// this number had better match the length of
// the field_map_list array
...
const field_map field_map_list[] = {
  ...
  {MagneticField1type, "MagneticField1",0},
  {MagneticField2type, "MagneticField2",0},
  {MagneticField3type, "MagneticField3",0},
  {ElectricField1type, "ElectricField1",0},
  {ElectricField2type, "ElectricField2",0},
  {ElectricField3type, "ElectricField3",0},
  {RadiationFreq0,"RadiationFreq0", 0},
  {RadiationFreq1,"RadiationFreq1", 0},
  {RadiationFreq2,"RadiationFreq2", 0},
  {RadiationFreq3,"RadiationFreq3", 0},
  {RadiationFreq4,"RadiationFreq4", 0},
  {RadiationFreq5,"RadiationFreq5", 0},
...
char *get_field_name(field_type_int field){
  ...
  for(i = 0; i < NUM_FIELDS; i++){
    if(field_map_list[i].field == field){
      name = new char[MAX_LINE_LENGTH];
      strcpy(name, field_map_list[i].field_name);
      break;
    }
  }
...
EnzoArray<float> *grid::CreateFieldArrayFloat(field_type_int field){
  ...
  if(in_range(field, 0, FieldUndefined - 1)){
    ...
    case gParticleMass:
  	...
  	array = new EnzoArray<float>(1, dims, sindex, eindex, cell_width);
...
EnzoArray<int> *grid::CreateFieldArrayInt(field_type_int field){
