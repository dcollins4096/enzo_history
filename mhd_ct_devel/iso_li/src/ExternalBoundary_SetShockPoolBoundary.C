...
#ifdef HAOXU
  float ShockPoolGasEnergy, ShockPoolShockGasEnergy;
   
   if(DualEnergyFormalism){
     ShockPoolShockGasEnergy = ShockPoolShockPressure/((Gamma-1.0)* ShockPoolShockDensity);
//    ShockPoolShockGasEnergy = ShockPoolShockTotalEnergy;
//   for(int field=0;field<BoundaryRank;field++)
//    ShockPoolShockGasEnergy -= 0.5*ShockPoolShockDensity*pow(ShockPoolShockVelocity[field],2);    
//    ShockPoolShockGasEnergy /=ShockPoolShockDensity;
    
      ShockPoolGasEnergy = ShockPoolPressure/((Gamma-1.0)*ShockPoolDensity);
//    ShockPoolGasEnergy = ShockPoolTotalEnergy;
//    for(int field=0;field<BoundaryRank;field++)
//    ShockPoolGasEnergy -=0.5*ShockPoolDensity*pow(ShockPoolVelocity[field],2);
//    ShockPoolGasEnergy /=ShockPoolDensity;
...
#ifdef HAOXU
  if(DualEnergyFormalism){
           BoundaryValue[GENum][dim][0][index] = ShockPoolShockGasEnergy;
    }
#endif
...
#ifdef HAOXU
  if(DualEnergyFormalism){
             BoundaryValue[GENum][dim][0][index] = ShockPoolGasEnergy;
   }
#endif
	  }
