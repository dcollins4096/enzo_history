...
/***********************************************************************
/
/  GRID CLASS (INITIALIZE THE GRID FOR A MHD COSMOLOGY SIMULATION)
/
...
int ReadFileHDF5(char *name, int Rank, int Dims[], int StartIndex[],
...
int grid::MHDCosmologySimulationInitializeGridHDF5(
                          ...
                          char *CosmologySimulationMagneticFieldNames[])
{
  ...
  char *subroutine="MHDCosmologySimulationInitializeGrid";
...
//  printf("CSIG Offsets %d %d %d\n", Offset[0], Offset[1], Offset[2]);
  ...
  {
    if (ReadFileHDF5(CosmologySimulationDensityName, GridRank, GridDimension,
...
//  fcol(BaryonField[0], size, 10, log_fptr);
  ...
  if (CosmologySimulationTotalEnergyName != NULL && ReadData)
    if (ReadFileHDF5(CosmologySimulationTotalEnergyName, GridRank, 
  ...
  if (CosmologySimulationGasEnergyName != NULL && DualEnergyFormalism && 
      ReadData)
    if (ReadFileHDF5(CosmologySimulationGasEnergyName, GridRank, GridDimension,
  ...
  {
    for (dim = 0; dim < GridRank; dim++)
      if (ReadFileHDF5(CosmologySimulationVelocityNames[dim], GridRank,
		   GridDimension, GridStartIndex, GridEndIndex, Offset,
...
//    fcol(BaryonField[vel+dim], size, 10, log_fptr);
  ...
  for(i=0;i < size; i++) {
          CenteredB[0][i] =  CosmologySimulationInitialMagneticField[0];
          CenteredB[1][i] =  CosmologySimulationInitialMagneticField[1];
          CenteredB[2][i] =  CosmologySimulationInitialMagneticField[2];
  }
  for(i=0;i<MagneticSize[0];i++) MagneticField[0][i] = CosmologySimulationInitialMagneticField[0];
  for(i=0;i<MagneticSize[1];i++) MagneticField[1][i] = CosmologySimulationInitialMagneticField[1];
  for(i=0;i<MagneticSize[2];i++) MagneticField[2][i] = CosmologySimulationInitialMagneticField[2];

   if (CosmologySimulationMagneticFieldNames[0] != NULL )
    for(dim=0;dim<3;dim++) 
    if (ReadFileHDF5(CosmologySimulationMagneticFieldNames[dim], GridRank,
                    GridDimension, GridStartIndex, GridEndIndex, Offset,
                    CenteredB[dim], &tempbuffer, dim, 1) == FAIL) {
      fprintf(stderr, "Error reading magnetic field.\n");
      return FAIL;
    }
...
/*          * POW(BaryonField[0][i]/CosmologySimulationOmegaBaryonNow,Gamma-1) 
  ...
  if(MHD_Used){
     //Add the magnetic energy to the total enengy
   for(i=0;i<size;i++){
    BaryonField[1][i] *= BaryonField[0][i];
    BaryonField[1][i] += 0.5*(CenteredB[0][i]*CenteredB[0][i]+
                                         CenteredB[1][i]*CenteredB[1][i]+
                                         CenteredB[2][i]*CenteredB[2][i]);
   }
  } //MHD_Used

}//CosmologySimulationDensityName != NULL && ReadData
...
//  This routine reads data from Inits and Ring, so its all 32-bit input!

      int ii = sizeof(float32);

//    fprintf(stderr, "CSIG size of float32 is %d\n", ii);
  ...
  if (CosmologySimulationParticlePositionName != NULL && ReadData) {
    ...
    ReadAttrHDF5(CosmologySimulationParticlePositionName,
...
//    {
//      fprintf(log_fptr, "Proc %d Offset %d Left %10.4f Right %10.4f\n", MyProcessorNumber, Offset[i], GridLeftEdge[i], GridRightEdge[i]);
//    }


#ifdef USE_MPI
      CHECK_MPI_ERROR(MPI_Comm_size(MPI_COMM_WORLD, &NOP));
#else
      NOP = 1;
#endif

      RNK=3;
//    printf("NOP %d, RNK %d", NOP, RNK);
      ...
      mpi_layout[1] = 0;
      mpi_layout[2] = 0;
      CHECK_MPI_ERROR(MPI_Dims_create(NOP, RNK, mpi_layout));
#else
      mpi_layout[0] = 1;
      mpi_layout[1] = 1;
      mpi_layout[2] = 1;
...
//    fprintf(log_fptr, "ABC %d %d %d;  IJK %d %d %d;  M = %d\n", A,B,C,I,J,K,M);

      sprintf(pid, "%4.4d", M);

// Pre-sorted particle input
...
//  fprintf(log_fptr, "Proc %d reads particle grid %s\n", MyProcessorNumber, PPos);
//  fprintf(log_fptr, "Proc %d reads particle grid %s\n", MyProcessorNumber, PVel);
  ...
  attr_id = H5Aopen_name(dset_id, "NumberOfParticles");
    if (io_log) fprintf(log_fptr, "H5Aopen (NumberOfParticles) id: %d\n", attr_id);
    assert( attr_id != h5_error );

  h5_status = H5Aread(attr_id, HDF5_I4, &NumSortedParticles);
  ...
  attr_id = H5Aopen_name(dset_id, "TotalParticleCount");
    if (io_log) fprintf(log_fptr, "H5Aopen (TotalParticleCount) id: %d\n", attr_id);
    assert( attr_id != h5_error );

  h5_status = H5Aread(attr_id, HDF5_I4, &TotParticleCount);
  ...
  printf("Calling AllocateNewParticles with %d\n", NumberOfParticles);

  this->AllocateNewParticles(NumberOfParticles);

  /* Allocate 32-bit buffer for ring input. */

  if (io_log) fprintf(log_fptr, "Allocate %d float32s for RingTemp\n", NumberOfParticles);

//  printf("Allocating %d (NumberOfParticles)\n", TotalParticleCount);

//  float32 *RingTemp = new float32[TotalParticleCount];

  printf("Allocating %d (NumberOfParticles)\n",NumberOfParticles);
  float32 *RingTemp = new float32[NumberOfParticles];
...
// Part are contiguous in the file, so stride = 1.
  ...
  h5_status = H5Dread(dset_id, type_id, mem_dsp_id, file_dsp_id,  H5P_DEFAULT, (VOIDP) RingTemp);
    ...
    assert( h5_status != h5_error );
    JBPERF_COUNT_READ(dset_id, type_id, mem_dsp_id);
...
*/
      ...
      for (dim = 0; dim < GridRank; dim++) {
	if (ReadFileHDF5(CosmologySimulationParticlePositionName, 1, Dim,
...
//      pcol32(tempbuffer, NumberOfParticles, 10, log_fptr);
	...
	delete [] tempbuffer;
      ...
      if (CosmologySimulationParticleVelocityName != NULL)
	for (dim = 0; dim < GridRank; dim++) {
	  if (ReadFileHDF5(CosmologySimulationParticleVelocityName, 1, Dim,
...
//        fcol(ParticleVelocity[dim], NumberOfParticles, 10, log_fptr);
      ...
      if (CosmologySimulationParticleMassName != NULL) {
	if (ReadFileHDF5(CosmologySimulationParticleMassName, 1, Dim, Start, End,
...
// ***#include "error.h"
// ***
// ***// HDF5 is not used, so CosmologySimulationInitializeGridHDF5 should
// ***//   not be called
// ***
// ***int grid::CosmologySimulationInitializeGridHDF5(
// ***			  float CosmologySimulationOmegaBaryonNow,
// ***			  float CosmologySimulationOmegaCDMNow,
// ***			  float CosmologySimulationInitialTemperature,
// ***			  char *CosmologySimulationDensityName,
