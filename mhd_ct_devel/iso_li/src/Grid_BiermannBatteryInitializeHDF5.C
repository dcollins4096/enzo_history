...
/***********************************************************************
/
/  GRID CLASS (INITIALIZE THE GRID FOR A Biermann Battery SIMULATION)
/
/  written by: Hao Xu
/  date:       March, 2006
/
...
int grid::BiermannBatteryInitializeGridHDF5(
                          ...
                          char *CosmologySimulationMagneticFieldNames[])
{
  ...
  char *subroutine="BiermannBatterySimulationInitializeGrid";
...
//    fcol(BaryonField[vel+dim], size, 10, log_fptr);
 ...
 for (dim = 0; dim < GridRank; dim++) {
    ...
    CenteredB[dim] = new float[size];
    Current[dim] = new float[size];
    MagneticField[dim] = new float[MagneticSize[dim]];
    ElectricField[dim] = new float[ElectricSize[dim]];  

     //Initialize Magnetic and Electric Fields
  
  for(i=0;i < size; i++) {
          CenteredB[dim][i] =  CosmologySimulationInitialMagneticField[dim];
          Current[dim][i] = 0.0;
  }
  for(i=0;i<MagneticSize[dim];i++) MagneticField[dim][i] = CosmologySimulationInitialMagneticField[dim];
...
/*          * POW(BaryonField[0][i]/CosmologySimulationOmegaBaryonNow,Gamma-1) 
    ...
    // Be careful that the total energy is different in enzo and MHDenzo   
    if (CosmologySimulationGasEnergyName == NULL && DualEnergyFormalism)
      for (i = 0; i < size; i++) {
