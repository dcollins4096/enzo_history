//  INITIALIZE A THE MHD SPHERICAL INFALL TEST
			      ...
			      HierarchyEntry &TopGrid, TopGridData &MetaData)
{
  ...
  float SphericalInfallMagneticfieldx      = 0.0;
  float SphericalInfallMagneticfieldy      = 0.0;
  float	SphericalInfallMagneticfieldz      = 0.0;
  ...
  while (fgets(line, MAX_LINE_LENGTH, fptr) != NULL) {
    ...
    ret += sscanf(line, "SphericalInfallUseBaryons = %d",
		  &SphericalInfallUseBaryons);
    ret += sscanf(line, "SphericalInfallSubgridIsStatic = %d",
		  &SphericalInfallSubgridIsStatic);
    ret += sscanf(line, "SphericalInfallFixedAcceleration = %d",
	...
	ret += sscanf(line, "SphericalInfallMagneticfieldx = %"FSYM,
		  &SphericalInfallMagneticfieldx);
    ret += sscanf(line, "SphericalInfallMagneticfieldy = %"FSYM,
		  &SphericalInfallMagneticfieldy);
	ret += sscanf(line, "SphericalInfallMagneticfieldz = %"FSYM,
		  &SphericalInfallMagneticfieldz);
  ...
  if (TopGrid.GridData->MHDSphericalInfallInitializeGrid(
					 TempPerturbation,
					 SphericalInfallUseBaryons,
					 SphericalInfallOmegaBaryonNow,
					 SphericalInfallOmegaCDMNow,
  ...
  if (NumberOfSubgridZones[0] > 0) {
    ...
    if (Subgrid->GridData->MHDSphericalInfallInitializeGrid(
                                   SphericalInfallInitialPerturbation,
	                           SphericalInfallUseBaryons,
				   SphericalInfallOmegaBaryonNow,
				   SphericalInfallOmegaCDMNow,
  ...
  if (MyProcessorNumber == ROOT_PROCESSOR) {
    fprintf(Outfptr, "SphericalInfallInitialPerturbation = %g\n", 
	    SphericalInfallInitialPerturbation);
    fprintf(Outfptr, "SphericalInfallSubgridLeft         = %"GOUTSYM"\n", 
	    SphericalInfallSubgridLeft);
    fprintf(Outfptr, "SphericalInfallSubgridRight        = %"GOUTSYM"\n", 
	    SphericalInfallSubgridRight);
    fprintf(Outfptr, "SphericalInfallOmegaBaryonNow      = %g\n",
	    SphericalInfallOmegaBaryonNow);
    fprintf(Outfptr, "SphericalInfallOmegaCDMNow         = %g\n",
	    SphericalInfallOmegaCDMNow);
    fprintf(Outfptr, "SphericalInfallUseBaryons          = %d\n",
	    SphericalInfallUseBaryons);
    fprintf(Outfptr, "SphericalInfallSubgridIsStatic     = %d\n",
	    SphericalInfallSubgridIsStatic);
    fprintf(Outfptr, "SphericalInfallFixedAcceleration   = %d\n",
	    SphericalInfallFixedAcceleration);
    fprintf(Outfptr, "SphericalInfallCenter              = ");
    WriteListOfFloats(Outfptr, MetaData.TopGridRank, SphericalInfallCenter);  
    fprintf(Outfptr, "SphericalInfallFixedMass           = %g\n\n",
	    SphericalInfallFixedMass);
  }
