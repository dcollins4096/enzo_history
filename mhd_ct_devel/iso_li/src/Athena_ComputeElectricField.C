...
int AthenaObj::ComputeElectricField(float dT, float ** Fluxes){  
...
#define Ec(index) (Bc[ dimY ][index]*Vel[ dimZ ][index] - Bc[ dimZ ][index]*Vel[ dimY ][index] )
   ...
   for(i=AthStart[0]; i<=AthEnd[0]+EndX; i++){
	 ...
	 As[flag]=1;
	 switch(MHD_DivB){
	 case MHD_DivB_Athena_Balsara:
	 case MHD_DivB_Balsara:
	   Cs[flag]= 0;
	   Bs[2*flag]  = 0;
	   Bs[2*flag+1]= 0;
	   break;
	 case MHD_DivB_Athena_LF:
	   Cs[flag]=1;
	   Bs[2*flag]  = 1;
	   Bs[2*flag+1]= 1;
	   break;
	 case MHD_DivB_Athena_Switch:
	   Cs[flag]= 0;
       ...
       if( MHD_DivB == MHD_DivB_Athena_Switch ){
	 // -d/dy
