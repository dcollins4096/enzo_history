...
int grid::DepositBaryons(grid *TargetGrid, FLOAT DepositTime)
{
  ...
  if (ProcessorNumber == MyProcessorNumber) {
    ...
    delete [] vel_field;
 //   for(i=0;i<64;i++) printf("%d %24.22f\n",i,dens_field[i]);   
  ...
  if (ProcessorNumber != TargetGrid->ProcessorNumber) {
    ...
    MPI_Status status;
    MPI_Datatype DataType = (sizeof(float) == 8) ? MPI_DOUBLE : MPI_FLOAT;
    ...
    if (MyProcessorNumber == TargetGrid->ProcessorNumber) {
      ...
      JBPERF_START_MPI_RECV("MPI_Recv",size,DataType);
      CHECK_MPI_ERROR(MPI_Recv(dens_field, size, DataType, ProcessorNumber, 
			       MPI_SENDREGION_TAG, MPI_COMM_WORLD, &status));
      JBPERF_STOP_MPI_RECV("MPI_Recv",size,DataType);
    }

//  if (MyProcessorNumber == ProcessorNumber)
//    CHECK_MPI_ERROR(MPI_Bsend(dens_field, size, DataType, TargetGrid->ProcessorNumber,
