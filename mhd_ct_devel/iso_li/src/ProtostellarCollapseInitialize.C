...
int ProtostellarCollapseInitialize(FILE *fptr, FILE *Outfptr, HierarchyEntry &TopGrid,
			 TopGridData &MetaData)
{
  ...
  if (MetaData.TopGridRank != 3) {
    printf("Cannot do ProtostellarCollapse in %d dimension(s)\n", MetaData.TopGridRank);
    return FAIL;
  }    
  ...
  */
  ...
  float ProtostellarCollapseB[3]             = {1e-8, 1e-8, 1e-8}; // ambient gas initally at rest
  ...
  while (fgets(line, MAX_LINE_LENGTH, fptr) != NULL) {
    ...
    ret += sscanf(line, "ProtostellarCollapseCoreDensity    = %"FSYM, 
		  &ProtostellarCollapseCoreDensity);

    ret += sscanf(line, "ProtostellarCollapseCoreEnergy    = %"FSYM, 
		  &ProtostellarCollapseCoreEnergy);
    ret += sscanf(line, "ProtostellarCollapseOuterEnergy    = %"FSYM, 
    ...
    ret += sscanf(line, "ProtostellarCollapseB               = %"FSYM" %"FSYM" %"FSYM,
		  ProtostellarCollapseB,ProtostellarCollapseB+1,ProtostellarCollapseB+2);
  ...
  if( MHD_Used ){
    ...
    float PsCMagEng = ProtostellarCollapseB[0]*ProtostellarCollapseB[0]+
      ProtostellarCollapseB[1]*ProtostellarCollapseB[1]+
      ProtostellarCollapseB[2]*ProtostellarCollapseB[1];
    ProtostellarCollapseOuterEnergy += 0.5*PsCMagEng;
    ProtostellarCollapseCoreEnergy  += 0.5*PsCMagEng;
  ...
  */
  ...
  for (lev = 0; lev < MaximumRefinementLevel; lev++) {
    ...
    if (debug)
      printf("PCI: Level[%d]: NumberOfSubgridZones = [%d,%d,%d]\n", lev+1, 
	...
	TopGrid.NextGridNextLevel  = Subgrid[0];
      ...
      if (debug)
	printf("PCI: Level[%d]: SubgridDims = [%d,%d,%d] [%f-%f, %f-%f, %f-%f]\n", lev+1, 
  ...
  */
  ...
  if (MyProcessorNumber == ROOT_PROCESSOR) {
    fprintf(Outfptr, "ProtostellarCollapseCoreDensity     = %f\n"  , 
	              ProtostellarCollapseCoreDensity);
    fprintf(Outfptr, "ProtostellarCollapseCoreEnergy      = %f\n"  , 
	              ProtostellarCollapseCoreEnergy);
    fprintf(Outfptr, "ProtostellarCollapseOuterDensity    = %f\n"  , 
	              ProtostellarCollapseOuterDensity);
    fprintf(Outfptr, "ProtostellarCollapseCoreRadius      = %f\n"  , 
	              ProtostellarCollapseCoreRadius);
    fprintf(Outfptr, "ProtostellarCollapseAngularVelocity = %f\n"  , 
	              ProtostellarCollapseAngularVelocity);
  }
