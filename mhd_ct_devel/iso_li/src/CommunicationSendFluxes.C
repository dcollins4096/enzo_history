			    ...
			    int Rank)
{
  ...
  /* Count space for Electric Fields */
  ...
  if( MHD_Used && MHD_FluxCorrection ){
	...
	//Note that for the fields that don't lie in a face, ESizes = 0
	for(i=0;i<ESizes[field][dim1];i++)
	  buffer[index++]=Fluxes->LeftElectric[field][dim1][i];

	for(i=0;i<ESizes[field][dim1];i++)
	  buffer[index++]=Fluxes->RightElectric[field][dim1][i];
...
//    DataType = MPI_DOUBLE;
  ...
  //CHECK_MPI_ERROR(MPI_Bsend(buffer, TotalSize, DataType, ToProc, MPI_FLUX_TAG, MPI_COMM_WORLD));
