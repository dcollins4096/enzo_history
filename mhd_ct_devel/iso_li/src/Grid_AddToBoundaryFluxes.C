...
int grid::AddToBoundaryFluxes(fluxes *BoundaryFluxesToBeAdded)
{
  ...
  for (dim = 0; dim < GridRank; dim++) {
	...
	//For unused fields, ElectricSize=0.
	for(i=0;i<BoundaryFluxes.ElectricSize[field][dim];i++){
	  BoundaryFluxes.LeftElectric[field][dim][i] +=
	    BoundaryFluxesToBeAdded->LeftElectric[field][dim][i];
	  BoundaryFluxes.RightElectric[field][dim][i] += 
	    BoundaryFluxesToBeAdded->RightElectric[field][dim][i];
	  /* Shit
	fprintf(stderr, "atbf: dim %d field %d ToLeft %f ToRight %f Left %f Right %f\n", 
		dim, field,
		BoundaryFluxesToBeAdded->LeftElectric[field][dim][i],
		BoundaryFluxesToBeAdded->RightElectric[field][dim][i],
		BoundaryFluxes.LeftElectric[field][dim][i] ,
