			      ...
			      char *PointValueNames[3][20], float BaseRadius)
{
  ...
  if (PointValueNames[0][0] == NULL) {
	...
	for (i = StartIndex[0]; i <= EndIndex[0]; i++, index++) 
	  if (BaryonField[NumberOfBaryonFields][index] == 0) {
	    ...
	    //dcc this line went to the above for Martin Laursen's vis stuff.
	    ...
	    for (dim = 0; dim < GridRank; dim++)
	      buffer[ibuf++] = float32(CenteredB[dim][index]);
#endif
  ...
  int BufferSize = 128, idmbuf = 0, istarbuf = 0, ParticleType;
  ...
  for (n = 0; n < NumberOfParticles; n++) {
	...
	density = 0;
      ...
      /* Determine particle type (0 for dm, 1 for star). */

      ParticleType = 0;
      if (NumberOfParticleAttributes > 0 && StarParticleCreation)
