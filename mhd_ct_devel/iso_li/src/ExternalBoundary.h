...
class ExternalBoundary
{
 private:
  ...
  //Note:  We aren't allowing cell-by-cell boundary setting just yet,
  //due to ambiguities in the prpoer way to set Magnetic Boundary.
  boundary_type MagneticBoundaryType[3][3][2];
...
//
  ...
  int SetMagneticBoundary(int FieldRank, int GridDims[], int GridOffset[],
                          ...
                          float *Field, int FieldType);
#ifdef EMF_BOUNDARY
  int SetExternalBoundary_EMF(int FieldRank, int GridDims[],
                                          int GridOffset[],
                                          int StartIndex[], int EndIndex[],
