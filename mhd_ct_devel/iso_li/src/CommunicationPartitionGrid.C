...
int CommunicationPartitionGrid(HierarchyEntry *Grid)
{
  ...
  if (MyProcessorNumber == ROOT_PROCESSOR)
    fprintf(stderr, "Partitioning Grid, MyProc %d\n", MyProcessorNumber);
  /* Declarations. */
  ...
  if (MyProcessorNumber == ROOT_PROCESSOR)
  {
    fprintf(stderr,"PartitionGrid (on all processors): Layout = %d %d %d\n", 
...
*/
  ...
  /*
  Pout( "  CommunicationPartitionGrid One: NumberOfBaryonFields ",
	Grid->GridData->ReturnNumberOfBaryonFields());
  */
  ...
  for (k = 0; k < Layout[2]; k++)
	...
	/*
	Pout( "  CommunicationPartitionGrid: NumberOfBaryonFields ", 
	      NewGrid->ReturnNumberOfBaryonFields(), OldGrid->ReturnNumberOfBaryonFields());
	*/
	/* Compute grid region. */
...
//        printf("K J I: %d %d %d  GC %d  NewProc %d\n",k,j,i,gridcounter,NewProc);
  ...
  if (RandomForcing == 1 && ParallelRootGridIO != 1)
    OldGrid->RemoveForcingFromBaryonFields(); //AK, dcc
