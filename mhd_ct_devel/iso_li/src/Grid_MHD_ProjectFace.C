				      ...
				      boundary_type RightFaceBoundaryCondition[])
{
  ...
  this->DebugCheck("MHD ProjectFace, before");
  //
  // Debug crap
  //

  if(MHD_Verbose) PoutF("Prong: PF: Parent Left Corner", 
	ParentGrid.GridLeftEdge[0], ParentGrid.GridLeftEdge[1], ParentGrid.GridLeftEdge[2]);
  if(MHD_Verbose) PoutF("Prong: PF: Parent Right Corner", 
	ParentGrid.GridRightEdge[0], ParentGrid.GridRightEdge[1], ParentGrid.GridRightEdge[2]);
  if(MHD_Verbose) PoutF("Prong: PF: Subgrid Left Corner", 
	GridLeftEdge[0], GridLeftEdge[1], GridLeftEdge[2]);
  if(MHD_Verbose) PoutF("Prong: PF: Subgrid Right Corner", 
	GridRightEdge[0], GridRightEdge[1], GridRightEdge[2]);
  Pout("Prong: PF: Procs (MyProc,SubProc,parentProc)"
       ,MyProcessorNumber,ProcessorNumber,ParentGrid.ProcessorNumber);

  
  this->DebugCheck("MHD_ProjectFace (before)");
  ...
  if (MyProcessorNumber != ProcessorNumber && 
    ...
    if(MHD_Verbose) Pout("Prong: PF: Exit, I have neither");
    return SUCCESS;
  }  
  ...
  if (CommunicationDirection == COMMUNICATION_SEND &&
    ...
    if(MHD_Verbose) Pout("Prong: PF: Send pass.  No comm needed, or I only have parent");
    return SUCCESS;
  }
  ...
  if (CommunicationDirection == COMMUNICATION_RECEIVE &&
      MyProcessorNumber != ParentGrid.ProcessorNumber &&
      ProcessorNumber != ParentGrid.ProcessorNumber){
    if(MHD_Verbose) Pout("Prong: PF: ReceivePass.  I don't have the subgrid.");
    return SUCCESS;
  }
  if (NumberOfBaryonFields == 0)
    return SUCCESS;

  if(WriteInThisF(10)==TRUE ){
    char basename[30];
    sprintf(basename, "data10%d%d.grid",MyProcessorNumber,dccCounter14);
    FILE *dccptr = fopen(basename, "a");
    ParentGrid.WriteGrid(dccptr, basename, 1);
    this->WriteGrid(dccptr, basename, 2);
    fclose(dccptr);
  ...
  for(shift[2]=-1;shift[2]<=1;shift[2]++)
	...
	SkipShift=FALSE;
	if(MHD_Verbose) Pout("Prong: Shift", shift[0],shift[1],shift[2]);
	...
	for(dim=0;dim<3;dim++){
	  ...
	  /*
	    ...
	    ParentStartIndex[dim] = nint((MovedSubLeft[dim] - 
	    ParentGrid.GridLeftEdge[dim])/
	    (ParentGrid.CellWidth[dim][0]))
	    + ParentGrid.GridStartIndex[dim];
	    
	    
	    //If this isn't the parent processor number, we'll be filling
	    //a dummy array that will then be communicated to the actual parent array.
	    //Thus, set the parent grid start to zero, so as not to waste space or violate memory
	    SubgridStartIndex[dim]=GridStartIndex[dim];
	...
	for(dim=0;dim<3;dim++){
	  ...
	  if( MovedSubRight[dim] < ParentGrid.GridLeftEdge[dim] - Some ){
	    if(MHD_Verbose) Pout("Prong: Right Too Left\n");
	    SkipShift=TRUE;
	    break;
	  }
	  if( (MovedSubLeft[dim] > ParentGrid.GridRightEdge[dim] + Some ) ){
	    if(MHD_Verbose) Pout("Prong: Left Too Right\n");
	    SkipShift=TRUE;
	    break;
	  }
	...
	for(dim=0;dim<3;dim++){
	  ...
	  if( fabs( MovedSubLeft[dim] - ParentGrid.GridRightEdge[dim] ) < Some ){
	    ...
	    Pout("Prong: Overlap: SubLeft.  ParentStart: ", 
		 ParentStartIndex[0],ParentStartIndex[1],ParentStartIndex[2]);
	    Pout("Prong: Overlap: SubLeft.  SubgridStart: ", 
		 SubgridStartIndex[0],SubgridStartIndex[1],SubgridStartIndex[2]);
	    if(MHD_Verbose) Pout("Prong: Overlap: Dim", Dim[0],Dim[1],Dim[2]);
	    ...
	    Pout("Prong: Overlap: SubRight.  ParentStart: ", 
		 ParentStartIndex[0],ParentStartIndex[1],ParentStartIndex[2]);
	    Pout("Prong: Overlap: SubRight.  SubgridStart: ", 
		 SubgridStartIndex[0],SubgridStartIndex[1],SubgridStartIndex[2]);
	    if(MHD_Verbose) Pout("Prong: Overlap: Dim", Dim[0],Dim[1],Dim[2]);
	...
	*/
	...
	if(ProcessorNumber == MyProcessorNumber){
	  if( MHD_ProjectE == TRUE ){
	    ...
	    if(MHD_Verbose) Pout("Prong: PF: Start Fill");
		...
		MHDeParentSize[field]*=MHDeParentDim[field][dim];
	      ...
	      if(MHD_Verbose) Pout("Prong: PF: MHDeParentDim (field, dims) ",field
		   , MHDeParentDim[field][0], MHDeParentDim[field][1], MHDeParentDim[field][2]);
	      if(MHD_Verbose) Pout("Prong: PF: MHDeDim (field, dims) ",field
		   , MHDeDim[field][0], MHDeDim[field][1], MHDeDim[field][2]);
	  ...
	  if(MHD_ProjectB == TRUE){
		...
		if(MHD_Verbose) Pout("Prong: WTF Projecting MHD field ", field);
		for(dim=0;dim<3;dim++){
	...
	/* If necessary, copy the projected field from the 'fake' ParentGrid to
	   the real one. */
	dccTempVerbosity = TRUE;
	if (ProcessorNumber != ParentGrid.ProcessorNumber) {
	  if (CommunicationDirection == COMMUNICATION_SEND )
	    Pout("Prong: Post Send");
	  if (CommunicationDirection == COMMUNICATION_RECEIVE)
	    Pout("Prong: Post receive");
	  for (dim = 0; dim < MAX_DIMENSION; dim++)
	...
	if( MyProcessorNumber != ParentGrid.ProcessorNumber &&
	  ...
	  for(field=0;field<3;field++){
	    ...
	    if( ParentGrid.ElectricField[field] != NULL){
	      ...
	      Pout("MPF: Deleting Magnetic Fiel (field, ParentProc)",field,ParentGrid.ProcessorNumber);
	    }
  ...
  if(WriteInThisF(12)==TRUE ){
    char basename[30];
    sprintf(basename, "data12%d%d.grid",MyProcessorNumber,dccCounter14++);
    FILE *dccptr = fopen(basename, "a");
    ParentGrid.WriteGrid(dccptr, basename, 1);
    this->WriteGrid(dccptr, basename, 2);
    fclose(dccptr);
