			     ...
			     int direction, int TransposeOnCompletion)
{
  ...
  for (dim = 0; dim < MAX_DIMENSION; dim++)
    DomainCellSize[dim] = 1.0/float(DomainDim[0]);  // DomainDim[dim], xh for 1d pancake
...
//    fprintf(stderr, "FFT(%d): initial -> strip1\n", MyProcessorNumber);
...
//    fprintf(stderr, "FFT(%d): FFT strip1\n", MyProcessorNumber);
...
//      fprintf(stderr, "FFT(%d): strip1 -> strip0\n", MyProcessorNumber);
...
//      fprintf(stderr, "FFT(%d): FFT strip0\n", MyProcessorNumber);
      for (j = 0; j < nffts; j++)
...
//      fprintf(stderr, "FFT(%d): Transpose on Completion\n", MyProcessorNumber);
  ...
  if (direction == FFT_INVERSE) {
    ...
    if (TransposeOnCompletion) {
//      fprintf(stderr, "FFT(%d): Transpose on Start\n", MyProcessorNumber);
