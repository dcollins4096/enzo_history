...
//    return SUCCESS;
  ...
  sprintf(PeakOutputName, "%s.L%1.1d", "PeakData", level);
	...
	if (FoundIt == TRUE && OutputAlready[n] == FALSE) {
	  ...
	  for (l = 0; l < level; l++)
		...
		if (radius < CellWidth[0][0]*POW(float(RefineBy), level-l))
		  fprintf(fptr, "%dl%d ", l, n1);
	      }

	  fprintf(fptr, "! %"GOUTSYM" ", Time);

	  fprintf(fptr, "%g %g %g ", PeakDensity[level][n], 
...
/*
	  if (SelfGravity && GravityResolution == 1 &&
	    ...
	    fprintf(fptr, "%g ", GravitatingMassFieldParticles[index]);
	  else
	    fprintf(fptr, "%g ", tiny_number);
*/
	  ...
	  if (SelfGravity && GravitatingMassFieldParticles != NULL) {
	    ...
	    gindex = (gdims[2]*GravitatingMassFieldParticlesDimension[1] +
		      ...
		      gdims[0];
	    fprintf(fptr, "%g ", GravitatingMassFieldParticles[gindex]);
	  }
	  else
	    fprintf(fptr, "%g ", tiny_number);
	  ...
	  for (l = 1; l < NumberOfBaryonFields; l++)
	    fprintf(fptr, "%g ", BaryonField[l][index]);
