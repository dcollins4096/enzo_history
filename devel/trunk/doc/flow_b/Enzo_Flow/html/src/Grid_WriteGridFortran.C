...
int grid::WriteGridFortran(FILE *fptr, char *base_name, int grid_id)
{
  ...
  if (NumberOfBaryonFields > 0) {
	...
	for (j = GridStartIndex[1]; j <= GridEndIndex[1]; j++)
      ...
      FORTRAN_NAME(fortwrite)(&FortranUnit, (float32 *)(temp), &GridRank, ActiveDim);
	...
	for (j = GridStartIndex[1]; j <= GridEndIndex[1]; j++)
      ...
      FORTRAN_NAME(fortwrite)(&FortranUnit, (float32*)(temperature), &GridRank, ActiveDim);
