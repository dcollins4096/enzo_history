...
//	 MyProcessorNumber, NumberOfGrids, GridsToSend);
  ...
  if (FirstTimeCalled) {
    MPI_Type_contiguous(sizeof(PackedGrid), MPI_BYTE, &MPI_PackedGrid);
  ...
  starttime = MPI_Wtime();
#endif
  MPI_Allgather(&GridsToSend, 1, MPI_INT, SharedListCount, 1, MPI_INT, 
		MPI_COMM_WORLD);
  ...
  MPI_Allgatherv(SendList, GridsToSend, MPI_PackedGrid, SharedList,
		 SharedListCount, SharedListDisplacements, MPI_PackedGrid,
		 MPI_COMM_WORLD);
  ...
  counter[13] ++;
  timer[33] += (endtime-starttime)*(endtime-starttime);
