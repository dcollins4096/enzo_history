...
//	      MyProcessorNumber, SendSize, ReceiveSize, FromProc, ToProc);
      if (MPI_Sendrecv((void*) SendBuffer, SendSize, DataType, ToProc, 
	       MPI_TRANSPOSE_TAG, (void*) ReceiveBuffer, ReceiveSize, 
	       DataType, FromProc, MPI_TRANSPOSE_TAG, MPI_COMM_WORLD, 
		       &status) != MPI_SUCCESS) {
	...
	return FAIL;
      ...
      counter[14] ++;
      timer[34] += (endtime-starttime)*(endtime-starttime);
