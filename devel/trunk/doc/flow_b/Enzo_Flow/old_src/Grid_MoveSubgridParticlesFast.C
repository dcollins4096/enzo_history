				   ...
				   int AllLocal)
{
  ...
  printf("MoveSubgridParticlesFast: %d\n", NumberOfParticles);
  ...
  if (MyProcessorNumber == ProcessorNumber) {
	...
	fprintf(stderr, "particle subgrid (%d/%d) out of range\n", subgrid,
		NumberOfSubgrids);
	return FAIL;
      }
...
*/
  ...
  if (MyProcessorNumber == ProcessorNumber)
	...
	if (ToGrids[subgrid]->ParticlePosition[0] != NULL ||
	  ...
	  fprintf(stderr, "Particles already in subgrid %d (n=%d, nm=%d)\n", 
	...
	if (debug) printf("MoveSubgridParticles: subgrid[%d] = %d\n", 
