...
int grid::WriteGrid(FILE *fptr, char *base_name, int grid_id)
{
  ...
  hsize_t     DMOutDims[MAX_DIMENSION];
...
//      fprintf(stderr, "Calling AddOverlappingParticleMassField\n");
//      this->AddOverlappingParticleMassField(this,Zero);
//      fprintf(stderr, "Past it\n");
	...
	EndIndex[dim] = nint((GridRightEdge[dim] - 
        ...
        bcsize = bcsize * (EndIndex[dim]-StartIndex[dim]+1+6);

//      fprintf(stderr, "%d %d %10.4f %10.4f %10.4f %10.4f\n", StartIndex[dim], EndIndex[dim],
...
//                                                             GravitatingMassFieldParticlesCellSize);
      ...
      bctemp = new float32[bcsize];

      for (dim = 0; dim < GridRank; dim++)
        DMOutDims[GridRank-dim-1] = EndIndex[dim]-StartIndex[dim]+1+6;
	...
	for (j = StartIndex[1]-3; j <= EndIndex[1]+3; j++)
      ...
      file_dsp_id = H5Screate_simple(GridRank, DMOutDims, NULL);
        if (io_log) fprintf(log_fptr, "H5Screate file_dsp_id: %d\n", file_dsp_id);
        assert( file_dsp_id != h5_error );
      ...
      h5_status = H5Dwrite(dset_id, float_type_id, H5S_ALL, H5S_ALL, H5P_DEFAULT, (VOIDP) bctemp);
        if (io_log) fprintf(log_fptr, "H5Dwrite: %d\n", h5_status);
        assert( h5_status != h5_error );
