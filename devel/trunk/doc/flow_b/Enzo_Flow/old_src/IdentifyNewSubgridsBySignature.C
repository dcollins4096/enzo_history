				   ...
				   int &NumberOfSubgrids)
{
  ...
  int dim, i, j, NumberOfNewGrids, GridEnds[MAX_NUMBER_OF_SUBGRIDS][2];
  ...
  while (index < NumberOfSubgrids) {
	...
	if (Subgrid->FindGridsByZeroSignature(dim, NumberOfNewGrids, GridEnds)
	    == FAIL) {
	...
	if (NumberOfNewGrids > MAX_NUMBER_OF_SUBGRIDS) {
	  fprintf(stderr, "Increase MAX_NUMBER_OF_SUBGRIDS.\n");
	  return FAIL;
	}
	...
	if (NumberOfNewGrids > 1) {
	  ...
	  /*	  if (debug) 
	    printf("Breaking by simple zero. new grids[%d]=%d break=%d\n",
	...
	/*	if (debug) 
	  printf("Breaking by zero-crossing. dim=%d  break=%d-%d/%d-%d\n",
		 StrongestDim, 
