		   ...
		   FLOAT WriteTime)
{
  ...
  sprintf(id, "%6.6d", filenumber);   /* create output # */
  strcat(name, id);
  ...
  fprintf(Summaryfptr, "Rank                       = %d\n", 
	  MetaData->TopGridRank);
  fprintf(Summaryfptr, "NumberOfProcessors         = %d\n",
	  NumberOfProcessors);
  ...
  fprintf(Summaryfptr, "#LengthCGSConversionFactor     = %g\n", LengthUnits);
  fprintf(Summaryfptr, "#MassCGSConversionFactor       = %g\n",
	  double(DensityUnits)*POW(double(LengthUnits), 3));
  fprintf(Summaryfptr, "#LengthMpcConversionFactor     = %g\n", 
	  LengthUnits/Mpc);
  fprintf(Summaryfptr, "#MassSolarMassConversionFactor = %g\n",
	  double(DensityUnits)*POW(double(LengthUnits), 3)/SolarMass);
  fprintf(Summaryfptr, "#VelocityCGSConversionFactor   = %g\n", VelocityUnits);
  fprintf(Summaryfptr, "#VelocityPositionFactor        = %"GOUTSYM"\n", a);

  for (type = 0; type < 3; type++) 
    if (NumberOfValuesPerPoint[type] > 0) {
      fprintf(Summaryfptr, "\n#Quantities for particle type %d ", type);
