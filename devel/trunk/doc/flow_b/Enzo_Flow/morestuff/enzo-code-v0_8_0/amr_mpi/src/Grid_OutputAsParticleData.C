...
int grid::OutputAsParticleData(FLOAT RegionLeftEdge[], FLOAT RegionRightEdge[],
			       ListOfParticles *ParticleList[2], 
			       float BaseRadius)
{
  ...
  int i,j,k, n, dim, field, findex, start[MAX_DIMENSION], stop[MAX_DIMENSION];
  ...
  ParticleList[0]->NumberOfValues = 3;
  ParticleList[1]->NumberOfValues = 2;
  ParticleList[0]->NumberOfParticles = 0;
  ParticleList[1]->NumberOfParticles = 0;
...
*/
  ...
  if (size > 0) {
    ...
    for (dim = 0; dim < GridRank; dim++) {
      ParticleList[1]->ParticlePosition[dim] = new float[size];
      ParticleList[1]->ParticleVelocity[dim] = new float[size];
    }
    ParticleList[1]->ParticleRadius    = new float[size];
    ParticleList[1]->ParticleValue[0]  = new float[size];
    ParticleList[1]->ParticleValue[1]  = new float[size];
    ParticleList[1]->ParticleIndex     = new int[size];
    ParticleList[1]->NumberOfParticles = size;

    /* Create Particles. */

    int FieldPosition[MAX_DIMENSION], count = 0, index;
    float density;
    for (n = 0; n < NumberOfParticles; n++) {
	...
	for (dim = 0; dim < GridRank; dim++) {
	  ParticleList[1]->ParticlePosition[dim][count] = 
	    float(ParticlePosition[dim][n]);
	  ParticleList[1]->ParticleVelocity[dim][count] = 
	    ParticleVelocity[dim][n] * VelocityConversion;
	}
      //      ParticleList[1]->ParticleRadius[count] = CellWidth[0][0]*
      //	max(POW(ParticleMass[n]/density, 0.3333), 3);
	ParticleList[1]->ParticleRadius[count] = BaseRadius *
	  POW(1.0/density, 0.3333);
	ParticleList[1]->ParticleValue[0][count] = 
	                       ParticleMass[n]*CellVolume*DensityConversion;
	ParticleList[1]->ParticleValue[1][count] = density;
	ParticleList[1]->ParticleIndex[count] = ParticleNumber[n];
  ...
  if (debug)
    printf("Grid %d %d %d: NumberOfParticles = %d %d\n", GridDimension[0],
