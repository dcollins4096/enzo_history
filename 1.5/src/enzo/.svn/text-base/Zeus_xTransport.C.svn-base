...
//        Compute mass flux
	...
	if (fabs(u[IDX(i,j,k)]) > 0.5*dx[i]/dt) {
	  printf("xt warning 1: u=%g,%g,%g  dstar=%g,%g,%g  i,j,k=%d,%d,%d\n",
                 u[IDX(i-1,j,k)],u[IDX(i,j,k)],u[IDX(i+1,j,k)], 
		 dstar[i-1], dstar[i],dstar[i+1],i,j,k);
	  printf("  d=%g,%g,%g  dx=%g,%g   dt=%g\n",
...
//    Update mass and energy fluxes
	...
	if (dnew/d[IDX(i,j,k)] < 0.15)
	  printf("zeus_x d warning: d,dnew=%g,%g  i,j,k=%d,%d,%d  f1=%g,%g\n",
	...
	if (e[IDX(i,j,k)] <= 0.0 || dnew <= 0.0) {
	  printf("zeus_x negative e or d error: d,e,dnew,dt=%g,%g,%g,%g\n",d[IDX(i,j,k)],e[IDX(i,j,k)],dnew,dt);
	  return FAIL;
	}
...
//    Convert momenta back to velocities
	...
	if (fabs(u[IDX(i,j,k)]) > dx[i]/dt) {
	  printf("zeus_x ux error: i,j,k=%d,%d,%d  ie,je,ke=%d,%d,%d  dx,dt=%g,%g\n", i,j,k,ie,je,ke,dx[i],dt);
          printf("               : u,d,d=%g,%g%g\n", u[IDX(i,j,k)],d[IDX(i,j,k)],d[IDX(i-1,j,k)]);
	  for (i1=1; i1 <= in; i1++)
	    printf("%d d,u,v,e,w,d-1,f2,u*,f1,uav=%g,%g,%g,%g,%g,%g,%g,%g,%g,%g\n", 
  ...
  if (rank > 1) {
	...
	for (i = is-1; i <= ie+1; i++) {
	  ...
	  if (fabs(v[IDX(i,j,k)]) > dx[i]/dt) {
	    printf("zeus_x vx warning: i,j,k=%d,%d,%d  ie,je,ke,jm1 = %d,%d,%d,%d\n",
		   i,j,k,ie,je,ke,jm1);
	    printf("zeus_x vx warning: v,d,d-1=%g,%g,%g  dx,dt=%g,%g\n", v[IDX(i,j,k)],d[IDX(i,j,k)],d[IDX(i,jm1,k)], dx[i], dt);
	  }
	  if (fabs(w[IDX(i,j,k)]) > dx[i]/dt) {
	    printf("zeus_x wx warning: i,j,k=%d,%d,%d  ie,je,ke,km1 = %d,%d,%d,%d\n",
		   i,j,k,ie,je,ke,km1);
	    printf("zeus_x wx warning: v,d,d-1=%g,%g,%g\n", w[IDX(i,j,k)],d[IDX(i,j,k)],d[IDX(i,j,km1)]);
	  }
	} // end: loop over i
      } // end: loop over j
    } // end: loop over k
