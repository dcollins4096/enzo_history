		     ...
		     float minsupecoef)
{
  ...
  for (i = 0; i < size; i++) {
	...
	fabs(w[i]) > dz[0]/dtFixed) {
      ...
      fprintf(stderr, "u,v,w,d,e=%g,%g,%g,%g,%g  dx=%g  dt=%g\n", 
  ...
  if (ZeusSource(d, e, u, v, w, p, 
    ...
    fprintf(stderr, "P(%d): Error in ZeusSource on step %d (dt=%g)\n", MyProcessorNumber,
	    nhy, dtFixed);
    fprintf(stderr, "  grid dims = %d %d %d\n", GridDimension[0], GridDimension[1], GridDimension[2]);
    return FAIL;
  }
  ...
  for(n=ixyz; n <= ixyz+GridRank-1; n++) {
    ...
    if (ret == FAIL) {
      fprintf(stderr, "P(%d): Error on ZeusTransport dim=%d (Cycle = %d, dt=%g)\n", 
