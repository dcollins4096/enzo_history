...
//    Compute mass flux
	...
	if (fabs(w[IDX(i,j,k)]) > 0.5*dz[k]/dt) {
	  printf("zt problem 1: w=%g,%g,%g  dstar=%g,%g,%g  i,j,k=%d,%d,%d\n",
                 w[IDX(i,j-1,k)],w[IDX(i,j,k)],w[IDX(i,j+1,k)], 
		 dstar[k-1], dstar[k],dstar[k+1],i,j,k);
	  printf("  d=%g,%g,%g  dz=%g,%g   dt=%g\n",
...
//    Update mass and energy fluxes
	...
	if (e[IDX(i,j,k)] <= 0.0 || dnew <= 0.0) {
	  printf("zeus_z negative e or d error: d,e,dnew,dt=%g,%g,%g,%g\n",d[IDX(i,j,k)],e[IDX(i,j,k)],dnew,dt);
	  return FAIL;
	}
...
//    Convert momenta back to velocities
	...
	if (fabs(w[IDX(i,j,k)]) > dz[k]/dt) {
	  printf("zeus_z uy error: i,j,k=%d,%d,%d  ie,je,ke=%d,%d,%d  dz,dt=%g,%g\n", i,j,k,ie,je,ke,dz[k],dt);
...
//  Convert v,w momenta back to velocities
  ...
  for (j = 0; j < jn; j++) {
	...
	if (fabs(u[IDX(i,j,k)]) > dz[k]/dt) {
	  printf("zeus_z ux warning: i,j,k=%d,%d,%d  ie,je,ke,im1 = %d,%d,%d,%d\n",
		 i,j,k,ie,je,ke,im1);
	  printf("zeus_z ux warning: u,d,d-1=%g,%g,%g\n", u[IDX(i,j,k)],d[IDX(i,j,k)],d[IDX(im1,j,k)]);
	}
	if (fabs(v[IDX(i,j,k)]) > dz[k]/dt) {
	  printf("zeus_z wx warning: i,j,k=%d,%d,%d  ie,je,ke,jm1 = %d,%d,%d,%d\n",
		 i,j,k,ie,je,ke,jm1);
	  printf("zeus_z wx warning: v,d,d-1=%g,%g,%g\n", v[IDX(i,j,k)],d[IDX(i,j,k)],d[IDX(i,jm1,k)]);
	}
      } // end: loop over k
    } // end: loop over i
  } // end: loop over j
