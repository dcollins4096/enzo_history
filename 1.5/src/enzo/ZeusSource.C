	       ...
	       int bottom, float minsupecoef)
{
  ...
  for (k = 0; k < kn; k++) {
	...
	for (i = 0; i < in; i++) {
	  ...
	  if (e[IDX(i,j,k)] <= 0.0 || d[IDX(i,j,k)] <= 0.0) {
	    printf("%d\n", IDX(i,j,k));
	    fprintf(stderr, "zeus_source1: e,d=%g,%g i,j,k=%d,%d,%d\n",
  ...
  for (k = ksm2; k <= kep1; k++) {
	...
	if (e[IDX(i,j,k)] <= 0.0 || d[IDX(i,j,k)] <= 0.0) {
	  fprintf(stderr, "zeus_div error: e,alpha,div=%g,%g,%g   i,j,k=%d,%d,%d\n",
                  e[IDX(i,j,k)],alpha, div[i],i,j,k);
	  fprintf(stderr, "%g %g dt=%g dx=%g\n", d[IDX(i,j,k)],p[IDX(i,j,k)],dt,dx[i]);
  ...
  for (k = ksm2; k <= kep2; k++) {
	...
	if (e[IDX(i,j,k)] <= 0.0 || d[IDX(i,j,k)] <= 0.0) {
	  fprintf(stderr, "zeus_source2: e,d=%g,%g  i,j,k=%d,%d,%d\n",
