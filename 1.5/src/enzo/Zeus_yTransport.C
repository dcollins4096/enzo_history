...
//    Compute mass flux
	...
	if (fabs(v[IDX(i,j,k)]) > 0.5*dy[j]/dt) {
	  printf("yt problem 1: u=%g,%g,%g  dstar=%g,%g,%g  i,j,k=%d,%d,%d\n",
                 v[IDX(i,j-1,k)],v[IDX(i,j,k)],v[IDX(i,j+1,k)], 
		 dstar[j-1], dstar[j],dstar[j+1],i,j,k);
	  printf("  d=%g,%g,%g  dy=%g,%g   dt=%g\n",
...
//    Update mass and energy fluxes
	...
	if (e[IDX(i,j,k)] <= 0.0 || dnew <= 0.0) {
	  printf("zeus_y negative e or d error: d,e,dnew,dt=%g,%g,%g,%g\n",d[IDX(i,j,k)],e[IDX(i,j,k)],dnew,dt);
	  return FAIL;
	}
...
//    Convert momenta back to velocities
	...
	if (fabs(v[IDX(i,j,k)]) > dy[j]/dt) {
	  printf("zeus_y uy error: i,j,k=%d,%d,%d  ie,je,ke=%d,%d,%d  dy,dt=%g,%g\n", i,j,k,ie,je,ke,dy[j],dt);
...
//  Convert v,w momenta back to velocities
  ...
  for (k = 0; k < kn; k++) {
	...
	if (fabs(u[IDX(i,j,k)]) > dy[j]/dt) {
	  printf("zeus_y ux warning: i,j,k=%d,%d,%d  ie,je,ke,im1 = %d,%d,%d,%d\n",
		 i,j,k,ie,je,ke,im1);
	  printf("zeus_y ux warning: u,d,d-1=%g,%g,%g\n", u[IDX(i,j,k)],d[IDX(i,j,k)],d[IDX(im1,j,k)]);
	}
	if (fabs(w[IDX(i,j,k)]) > dy[j]/dt) {
	  printf("zeus_y wx warning: i,j,k=%d,%d,%d  ie,je,ke,km1 = %d,%d,%d,%d\n",
		 i,j,k,ie,je,ke,km1);
	  printf("zeus_y wx warning: v,d,d-1=%g,%g,%g\n", w[IDX(i,j,k)],d[IDX(i,j,k)],d[IDX(i,j,km1)]);
	}
      } // end: loop over j
    } // end: loop over i
  } // end: loop over k
