...
// 
  ...
  if ( argc != 3 )
  {
    fprintf(stderr, "USAGE: ring ParticlePositionFile ParticleVelocityFile\n");
    MPI_Finalize();
    return FAIL;
  }
  ...
  if( ncpu*dbuff_size != TotalParticleCount)
  {
     fprintf(stderr,"Xring WARNING: TotalParticleCount (%d) is not a power of 2!\n", TotalParticleCount);
     ++dbuff_size;
  }
  ...
  dx = 1.0 / ((double) enzo_layout[0]);
  dy = 1.0 / ((double) enzo_layout[1]);
  dz = 1.0 / ((double) enzo_layout[2]);
  ...
  for (kk = 0; kk < enzo_layout[2]; kk++)
    for (jj = 0; jj < enzo_layout[1]; jj++)
      ...
      // coordinate to rank check
        ...
        Left[0] =  dx * (double) ii;
        Right[0] = dx * (double) (ii+1);
        Left[1] =  dy * (double) jj;
        Right[1] = dy * (double) (jj+1);
        Left[2] =  dz * (double) kk;
        Right[2] = dz * (double) (kk+1);
        ...
        for (m = 0; m < rank; m++)
          fprintf(log,"Grid %d    Left %16.8f   Right %16.8f\n",
...
/* Transmit left */
  ...
  for (ic = 0; ic < dims[2]; ic++)
  {
    ...
    if ( dpos == Left[dim] )
    {
       fprintf(stderr,"Particle on left boundary [%d] [%d] %16.8f\n", i, ic, buff[i][ic]);
       hits = hits + 1;
    }

    if ( dpos == Right[dim] )
    {
       fprintf(stderr,"Particle on right boundary [%d] [%d] %16.8f\n", i, ic, buff[i][ic]);
       hits = hits + 1;
    }
