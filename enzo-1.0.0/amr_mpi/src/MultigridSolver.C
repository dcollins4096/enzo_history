...
/*      if (Dims[dim][depth+1]*2-1 != Dims[dim][depth]+1) {
	fprintf(stderr, "Dims[%d]=%d not of form 2^j+1\n", dim, Dims[dim][0]);
	return FAIL;
      }
*/
  ...
  if (depth == MAX_DEPTH) {
    fprintf(stderr, "Depth(%d) > MAX_DEPTH\n", depth);
    return FAIL;
  }

  if (start_depth > bottom) {
    fprintf(stderr, "Start depth(%d) > bottom(%d)!\n", start_depth, bottom);
    return FAIL;
  }
...
//      FORTRAN_NAME(mg_prolong2)(defect[depth], RHS[depth+1], &Rank,
    ...
    for (depth = bottom-1; depth >= 0; depth--) {
      ...
      //    printf("prolonging Solution %d -> %d\n",Dims[0][depth+1],Dims[0][depth]);
...
//	 Dims[2][0], norm, mean, tol_check);
  ...
  if (tol_check > tolerance) {
    fprintf(stderr, "Too many iterations (%d): tol=%g, check=%g\n", iter, 
	    tolerance, tol_check);
    return FAIL;
  }
