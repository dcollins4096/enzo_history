...
//    CHECK_MPI_ERROR(MPI_Bsend(ParticleNumber+FromStart, FromNumber, MPI_INT,
...
//  }
    ...
    if (MyProcessorNumber == ToProcessor) {
      ...
      JBPERF_START_MPI_RECV("MPI_Recv",FromNumber,MPI_INT);
      CHECK_MPI_ERROR(MPI_Recv(&ToGrid->ParticleNumber[ToStart], FromNumber, 
			       MPI_INT, ProcessorNumber, MPI_SENDPARTINT_TAG, 
			       MPI_COMM_WORLD, &status));
      JBPERF_STOP_MPI_RECV("MPI_Recv",FromNumber,MPI_INT);

      for (dim = 0; dim < GridRank; dim++) {
	JBPERF_START_MPI_RECV("MPI_Recv",FromNumber, MY_MPIFLOAT);
	CHECK_MPI_ERROR(MPI_Recv(ToGrid->ParticlePosition[dim]+ToStart, 
				 FromNumber, MY_MPIFLOAT, ProcessorNumber, 
				 MPI_SENDPARTFLOAT_TAG, MPI_COMM_WORLD, 
				 &status));
	JBPERF_STOP_MPI_RECV("MPI_Recv",FromNumber, MY_MPIFLOAT);
      }
    }

    ZLAN_STOP_RECV(7);

    ZLAN_COUNT(8,TransferSize);
