...
/  modified1:  
/
/  PURPOSE: This routine reads the Self Gravity problem boundary data 
...
int SelfGravityProblem::ReadBoundary(FILE *fptr)
{
  ...
  if (fscanf(fptr, "GravityBoundaryRank = %"ISYM"\n", itest) != 1) {
    fprintf(stderr, "Error reading GravityBoundaryRank.\n");
    return FAIL;
  }
  if (itest[0] != rank) {
  ...
  for (dim=0; dim<rank; dim++) {
    if ((BdryType[dim] != 0) && (OnBdry[dim][0] || OnBdry[dim][1]))
      OwnIsoBoundary = true;
  }
  ...
  if (io_log) {
    ...
    fprintf(log_fptr, 
	    ...
	    MyProcessorNumber, BdryType[0], 
	    BdryType[1], BdryType[2]); 
