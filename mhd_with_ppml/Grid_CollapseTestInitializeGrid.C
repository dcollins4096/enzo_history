				     ...
				     )
{
  ...
  int DeNum, HINum, HIINum, HeINum, HeIINum, HeIIINum, HMNum, H2INum, H2IINum,
      DINum, DIINum, HDINum;
  int DensNum=-1, TENum=-1, GENum=-1, Vel1Num=-1, Vel2Num=-1,Vel3Num=-1;
  /* create fields */
...
#ifdef PPML
  if( MHD_Used == TRUE ){
    FieldType[BNum[0] = NumberOfBaryonFields++] = Magnetic1;
    FieldType[BNum[1] = NumberOfBaryonFields++] = Magnetic2;
    FieldType[BNum[2] = NumberOfBaryonFields++] = Magnetic3;
  }
...
#define NFW_POINTS 500
  ...
  if (SetupLoopCount == 0)
	...
	//<dbg>
	fprintf(stderr,"AllocateField %d\n",FieldType);
	//</dbg>
      }
#ifdef PPML
  if( HydroMethod == PPM_Local )
...
#ifdef PPML
	density = BackgroundDensity; //Initially 1.0;
#else
	density = 1.0;
#endif //PPML
...
//		float vel = sqrt(PointSourceGravityConstant/drad)/drad;
	...
	BaryonField[DensNum][n] = density*BaryonMeanDensity;
	...
	if (MultiSpecies > 0) {
	  ...
	  BaryonField[HIINum][n] = CosmologySimulationInitialFractionHII *
	    CoolData.HydrogenFractionByMass * BaryonField[DensNum][n] *
	    sqrt(OmegaMatterNow)/
	  ...
	  BaryonField[HeIINum][n] = CosmologySimulationInitialFractionHeII*
	    BaryonField[DensNum][n] * 4.0 * (1.0-CoolData.HydrogenFractionByMass);
	  BaryonField[HeIIINum][n] = CosmologySimulationInitialFractionHeIII*
	    BaryonField[DensNum][n] * 4.0 * (1.0-CoolData.HydrogenFractionByMass);
	  BaryonField[HeINum][n] =
	    (1.0 - CoolData.HydrogenFractionByMass)*BaryonField[DensNum][n] -
	  ...
	  if (MultiSpecies > 1) {
	    ...
	    BaryonField[H2INum][n] = CosmologySimulationInitialFractionH2I*
	      BaryonField[DensNum][n]*CoolData.HydrogenFractionByMass*POW(301.0,5.1)*
...
#ifdef PPML
	//I hate this ifdef'ed conditional. 
	if( EquationOfState == 0 ){
#endif //PPML
	  BaryonField[TENum][n] = temperature/TemperatureUnits/
	    ((Gamma-1.0)*mu);


	if (DualEnergyFormalism)
	  BaryonField[GENum][n] = BaryonField[1][n];
 
	if (HydroMethod != Zeus_Hydro)
	  for (dim = 0; dim < GridRank; dim++)
	    BaryonField[TENum][n] += 0.5*POW(BaryonField[ivel+dim][n], 2);
