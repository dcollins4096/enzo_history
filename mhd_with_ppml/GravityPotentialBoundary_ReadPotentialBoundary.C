...
/***********************************************************************
/
/  GRAVITY POTENTIAL BOUNDARY CLASS (READ THE BOUNDARY VALUES)
/
/  written by: Daniel R. Reynolds
/  date:       April, 2006
/  modified1:
/
/  PURPOSE: This routine reads the Potential Boundary data 
/           from the specified file(s).  If the preprocessor directive 
...
#ifdef ISO_GRAV
//  ReadPotentialBoundary obtains information regarding the HDF5 restart 
//  from the file fptr, including the HDF5 data file.  It then reads the 
//  local processor's boundary value information from the appropriate 
//  HDF5 data file (see comments at top of file).  The integer return 
//  value denotes SUCCESS or FAIL, as defined in the header file 
//  macros_and_parameters.h
int GravityPotentialBoundary::ReadPotentialBoundary(FILE *fptr)
{
  ...
  if (fscanf(fptr, "GravityBoundaryRank = %"ISYM"\n", itest) != 1) {
    fprintf(stderr, "Error reading BoundaryRank.\n");
    return FAIL;
  }
  if (itest[0] != BoundaryRank) {
    fprintf(stderr, "Incompatible restart GravityBoundaryRank = %"ISYM"\n",
	    itest[0]);
    return FAIL;
  }
 
  fscanf(fptr, "GravityDimensions =");
  if (ReadListOfInts(fptr, BoundaryRank, itest) == FAIL) {
    fprintf(stderr, "Error reading GravityDimensions.\n");
    return FAIL;
  }
  if ((itest[0] != GlobalDimension[0]) || 
      (itest[1] != GlobalDimension[1]) || 
      (itest[2] != GlobalDimension[2])) {
    fprintf(stderr, "Incompatible restart GravityDimensions = %"ISYM" %"ISYM" %"ISYM"\n",
	    itest[0], itest[1], itest[2]);
    return FAIL;
  }
 
  fscanf(fptr, "GravityBoundaryTypes =");
  if (ReadListOfInts(fptr, BoundaryRank, itest) == FAIL) {
    fprintf(stderr, "Error reading GravityBoundaryTypes.\n");
    return FAIL;
  }
  if ((itest[0] != BoundaryType[0]) || 
      (itest[1] != BoundaryType[1]) || 
      (itest[2] != BoundaryType[2])) {
    fprintf(stderr, "Incompatible restart GravityBoundaryTypes = %"ISYM" %"ISYM" %"ISYM"\n",
	    itest[0], itest[1], itest[2]);
    return FAIL;
  }
  ...
  // read HDF file name
  if (fscanf(fptr, "PotentialBoundaryFileName = %s\n", hdfname) != 1) {
    fprintf(stderr, "Error reading PotentialBoundaryFileName.\n");
    return FAIL;
  }
  ...
  if (io_log) {
    ...
    fprintf(log_fptr, "ReadPotentialBoundary start, proc %"ISYM"\n",
	    MyProcessorNumber);
  ...
  for (dim = 0; dim < BoundaryRank; dim++)
	...
	if (OwnBoundary[dim][face]) {
	  ...
	  this->SetGravityBoundaryValues(dim, face, 0, bdryface);
