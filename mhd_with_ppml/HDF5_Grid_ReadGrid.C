...
int grid::ReadGrid(FILE *fptr, int GridID)
{
  ...
  // (that's taken care of in WriteGrid
  ...
  if( HydroMethod == PPM_Local){
    if (fscanf(fptr, "PPML_NFaces = %"ISYM"\n",
	       &PPML_NFaces) != 1) {
      fprintf(stderr, "Error reading NumberOfBaryonFields.\n");
      return FAIL;
    }
    if (fscanf(fptr, "NumberOfFluidQuantities = %"ISYM"\n",
	       &NumberOfFluidQuantities) != 1) {
      fprintf(stderr, "Error reading NumberOfBaryonFields.\n");
      return FAIL;
    }
...
//  CAUTION - are the coordinates reversed?
    ...
    FLOAT_IO *temp = new FLOAT_IO[active_size];
  ...
  if (NumberOfParticles > 0) {
    ...
    switch(jj)
    {
      ...
      case 4:
        temp = new FLOAT_IO[NumberOfParticles];
        break;
 
      case 8:
        temp = new FLOAT_IO[NumberOfParticles*2];
        break;
 
      case 16:
        temp = new FLOAT_IO[NumberOfParticles*4];
        break;
