...
class GlobalStats; //For some global statistics
#endif //PPML
class grid
{
 private:
...
#ifdef PPML				 
			     ,float CollapseTestMagneticFieldx
			     ,float CollapseTestMagneticFieldy
			     ,float CollapseTestMagneticFieldz
#endif //PPML
);
...
#   include "Grid_AnalyzeClusters.h"
  ...
  //
  // Face Centered MHD stuff
  //

 private:
  float * MagneticField[MAX_FACE_FIELDS];
  float * ElectricField[MAX_EDGE_FIELDS];
  int NFaces, NEdges; //Will likely always be 3 for now.


  //Miscellaneous Access parameters.
  //I could get away with fewer, but they make things
  //easy to deal with.
  int MagneticDims[MAX_FACE_FIELDS][MAX_DIMENSION];
  int ElectricDims[MAX_EDGE_FIELDS][MAX_DIMENSION];
  int MHDAdd[MAX_FACE_FIELDS][MAX_DIMENSION];
  int MagneticSize[MAX_FACE_FIELDS];
  int ElectricSize[MAX_EDGE_FIELDS];
 ...
 public:
  ...
  //Nested Ifdefs!  Damn!
#ifdef PPML
  int PPML_MHD_Clone(int Direction); //Copies the FaceCentered Magnetic field to Psedo Face Centered Fields.
#endif //PPML
 ...
 private:
  int NumberOfFluidQuantities;   //Density, Energy, Velocity, Magnetic Fields, etc.
                                 //For non-PPML runs, identical to NumberOfBaryonFields.
  int PPML_NFaces;               //Number of faces.  Currently always 6, but will be different 
                                 //for lower dimensionalities
  int InterfaceStatesInitialized;//Flag for initialization.

 public:
  
  //Initialization routines: For all PPML runs
  int IdentifyPhysicalQuantities_2(IndexPointerMap &ind);
  int PPML_InitInterfaceDataGrid();
  int PPML_InitInterfaceTypesAndLabels();


  //For allocating fluxes. (not strictly a PPML function, but created along side PPML, so its in this ifdef block.
  int AllocateFluxes( int NumberOfSubgrids, fluxes *SubgridFluxes[] );

  //Initialization routines: Problem Specific
  int DiscontInitializeGrid(float DensityA, float DensityB,
			    float EnergyA,  float EnergyB,
			    float VelocityA[], float VelocityB[],
			    float BA[], float BB[], 
			    float Radius, float DiscontCenter[], int LongDimension,
  ...
  //The Algorithm routines: Monotonicity constraints.
  int PPML_Mono1();
  int PPML_Mono2();
  int PPML_Mono3();

  //The main driver: Compute fluxes, conservation law update, CT.
  int PPML_Update(int  CycleNumber, int level, int grid);

  //Viscosity wrapper.
  int PPML_AddViscosity( float * FluxX, float * FluxY, float * FluxZ );
  float MHD_Viscosity(float * Fluxes, float * Lhs, float * Rhs, int * index, 
			  int dim, int MHD_DiffusionMethodInput, float dT
			  , float **State, int **MapEtoS
		    );
		     


  //The external source term routine, called in PPML_Update.
  float ReturnSourceTerm(int Field, IndexPointerMap ind, int Index, int dim);

  int IdentifyMHDQuantities(int &DensNum, int &GENum, int &Vel1Num,
			    int &Vel2Num, int &Vel3Num, int &TENum,
  ...
  // Friends.
  friend class PPML_InterfacePointerBundle;
  friend class Athena;
