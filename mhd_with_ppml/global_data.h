...
EXTERN MHD_divbmethod MHD_DivB; //CT method. 
EXTERN float MHD_DivBparam;     //Parameters for various CT methods.
EXTERN MHD_Centering MHD_CenteringMethod;
EXTERN int MHD_WriteElectric;   //Write Electric Field?
#endif //MHDF

/* ppml variables (and other such stuff that went in at that time.) */
#ifdef PPML

//ATH || PPML
//NUMBER_OF_INTEGRATION_STEPS is a hold over from the RungeKutta integrator in Athena.
EXTERN int MHD_Recon[NUMBER_OF_INTEGRATION_STEPS]; //Reconstruction method.
...
EXTERN float MHD_DiffusionParameter; // Just like in ppm
EXTERN int MHD_DirectionalSplitting; //Also doesn't work.
EXTERN int PPML_SlopeLimiter[3]; //Hyun, Barth, PPM.  Switches each on on or off
EXTERN int PPML_EigenSystem;  //Doesn't do anything-- stopped working on that.
...
// Physics Flags
EXTERN int MHD_Used;                   // 0 = No. 1 = Yes.
EXTERN int EquationOfState;            //Determines the presence of an Energy variable
EXTERN float IsothermalSoundSpeed;     //for EquationOfState = 1

// Problem Flags
EXTERN float DiscontNormal[4];         //For oblique shocks.
EXTERN int PPML_InitInterfaceMethod;   // 0: done elsewhere. 1: Piecewise Constant. 2: PiecewiseLiner.
                                       // 3: Piecewise Quadratic (not done yet.)
// Global Counters (mostly for debugging.)
EXTERN int dccCounter01;      // 
EXTERN int dccCounter02;      // 
EXTERN int dccCounter03;      // 
EXTERN int dccCounter04;      // See SetDefaultGlobal for annotation of these counters.
EXTERN int dccCounter05;      //  
EXTERN int dccCounter06;      // 
EXTERN int dccCounter07;      // 
EXTERN int dccCounter08;      // 
EXTERN int dccCounter09;      // 
EXTERN int dccCounter10;      // 
EXTERN int dccCounter11;      // 

//Misc. Debugging controls.
EXTERN int MidWayDumpList[N_MidWayDumps];  //Calls WriteGrid at various points. Many options, ask about it!
EXTERN int WriteBoundary;                  //Writes the boundary. 0 or 1.
EXTERN int WriteAcceleration;              //Writes the acceleration. 0 or 1
EXTERN float FixedTimestep;                //Forces the timestep to be the values defined.
EXTERN int ProcessorTopology[3];           //Forces the proper number of grids in each direction.



//Long term memory counters.
EXTERN long long MEMPreviousCurrent, MEMPreviousGlobal;
...
#ifdef PPML
EXTERN int     RandomForcingNumberOfFields;
#endif //PPML
