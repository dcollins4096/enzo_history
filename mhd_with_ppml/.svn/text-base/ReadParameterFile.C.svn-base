...
int ReadParameterFile(FILE *fptr, TopGridData &MetaData, float *Initialdt)
{
  ...
  while (fgets(line, MAX_LINE_LENGTH, fptr) != NULL) {
    ...
    // int    ret += sscanf(line, "       = %"ISYM, &);
    // float  ret += sscanf(line, "      = %"FSYM, &);

    //Problem Parameters
    ret += sscanf(line, "PPML_InitInterfaceMethod       = %"ISYM, &PPML_InitInterfaceMethod);

    //Extraneous Output Flags:
    //MidWayDump: for dumping mid way.
    //WriteAcceleration: Writes the Acceleration Field (when it exists)
    //WriteBoundary: For making salads with your friends in the park.
    ret += sscanf(line, "MidWayDump = %"ISYM" %"ISYM" %"ISYM" %"ISYM" %"ISYM" %"ISYM" %"ISYM" %"ISYM" %"ISYM" %"ISYM"", MidWayDumpList,
		  MidWayDumpList +1,MidWayDumpList +2,MidWayDumpList +3,MidWayDumpList +4,MidWayDumpList +5,
		  MidWayDumpList +6,MidWayDumpList +7,MidWayDumpList +8,MidWayDumpList +9);
    ret += sscanf(line, "WriteBoundary       = %"ISYM, &WriteBoundary);
    ret += sscanf(line, "WriteAcceleration   = %"ISYM, &WriteAcceleration);
    ret += sscanf(line, "FixedTimestep      = %"FSYM, &FixedTimestep);
    ...
    ret += sscanf(line, "MHD_Recon              = %"ISYM" %"ISYM,
                  MHD_Recon,MHD_Recon+1);
    ret += sscanf(line, "MHD_Riemann            = %"ISYM" %"ISYM,
                  MHD_Riemann, MHD_Riemann+1);

    ret += sscanf(line, "PPML_SlopeLimiter      = %"ISYM" %"ISYM" %"ISYM,
		  PPML_SlopeLimiter,PPML_SlopeLimiter+1,PPML_SlopeLimiter+2);
    ret += sscanf(line, "PPML_EigenSystem       = %"ISYM, &PPML_EigenSystem);
    ret += sscanf(line, "MHD_DiffusionMethod            = %"ISYM" %"ISYM,
                  MHD_DiffusionMethod, MHD_DiffusionMethod+1);
    ret += sscanf(line, "MHD_DiffusionParameter = %"FSYM, &MHD_DiffusionParameter);
    ret += sscanf(line, "MHD_DirectionalSplitting = %"ISYM, &MHD_DirectionalSplitting);
#ifdef DCC_EXTERNAL_CYCLE_COUNT
    if( strstr(line,"LevelCycleCount") != 0 )
      ReadIntsFromString( strstr(line,"=") +1, LevelCycleCount, MAX_DEPTH_OF_HIERARCHY);      
#endif
...
#ifdef ISO_GRAV
    ret += sscanf(line, "GravityBoundaryFaces      = %"ISYM" %"ISYM" %"ISYM, 
		  MetaData.GravityBoundaryFaces,
		  MetaData.GravityBoundaryFaces+1,
		  MetaData.GravityBoundaryFaces+2);
    ret += sscanf(line, "GravityBoundaryRestart = %"ISYM,
		  &MetaData.GravityBoundaryRestart);
    if (sscanf(line, "GravityBoundaryName       = %s", dummy) == 1)
      MetaData.GravityBoundaryName = dummy;
#endif
...
#ifdef PPML
    ret += sscanf(line, "RandomForcingNumberOfFields       = %"ISYM, &RandomForcingNumberOfFields);
#endif //PPML
    ...
    if (sscanf(line, "CoolDataParamfile = %s", dummy) == 1)
  ...
  /* If RadiationHydrodynamics == TRUE, we don't want RadiativeCooling 
     to be on, since it is taken care of in the coupled solver. */
  if(RadiationHydrodynamics == TRUE){
    fprintf(stderr, "WARNING:  RadiationHydrodynamics = 1.  Forcing\n");
    fprintf(stderr, "WARNING:  RadiationFieldType = 0, and\n");
    fprintf(stderr, "WARNING:  RadiativeCooling = 0.\n");
...
#ifdef PPML
  //Not all parameter sets are compatable with eachother.
  if( ParameterCompatibilityCheck(MetaData) == FAIL ){
      fprintf(stderr, "Incompatable Parameters.\n");
      return FAIL;
  }    
#endif //PPML 
  return SUCCESS;
