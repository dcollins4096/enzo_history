...
int ExternalBoundary::WriteExternalBoundary(FILE *fptr, char *hdfname)
{
  ...
  if (NumberOfBaryonFields > 0) {
    ...
    // create HDF5 external boundary file
	...
	//  Create HDF5 dataset for External Boundary types
	...
	//  Create HDF5 dataset for External Boundary values
        if (io_log) fprintf(log_fptr, "H5Dcreate with Name = %s\n", dname2);
	...
	//  Create scalar-valued HDF5 attribute dataspace
	...
	//  Create/Write/Close an HDF5 attribute for Number of Baryon Fields
	...
	//  Create/Write/Close an HDF5 attribute for Boundary Rank
	...
	//  Create/Write/Close an HDF5 attribute for Index
	...
	//  Create/Write/Close an HDF5 attribute for External Boundary Size
	...
	//  Close scalar-valued HDF5 attribute dataspace
	...
	//  Create vector-valued HDF5 attribute dataspace
        attr_dsp_id = H5Screate_simple((Eint32) 1, &n_attr, NULL);
          if (io_log) fprintf(log_fptr, "H5Screate_simple: %"ISYM"\n", attr_dsp_id);
          assert( attr_dsp_id != h5_error );
 
	//  Create/Write/Close an HDF5 attribute for Boundary Dimensions
	...
	//  Close vector-valued HDF5 attribute dataspace
        h5_status = H5Sclose(attr_dsp_id);
          if (io_log) fprintf(log_fptr, "H5Sclose: %"ISYM"\n", h5_status);
          assert( h5_status != h5_error );
...
*/
	...
	for (field = 0; field < NumberOfBaryonFields; field++)
	  for (i = 0; i < 2; i++) {
	    ...
	    //  Independently write boundary types to parallel HDF5 file
	...
	// close HDF5 boundary type dataset
