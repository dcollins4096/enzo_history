...
/***********************************************************************
/
/  This routine fills in ghost cells for MGMPI grids at inter-processor
/  boundaries using nodal values for neighboring processors.  The 
/  handling of periodic/isolating BCs should be handled by the calling 
/  routine, which should set neighbor indices to MPI_PROC_NULL for 
/  non-communicating interfaces (i.e. isolating external boundaries).
/
		      ...
		      int x2r, int mdim0, int mdim1, int mdim2)
{
  ...
  /* set buffer sizes */
  MPI_Arg x0buffsize = mdim1*mdim2;
  MPI_Arg x1buffsize = mdim0*mdim2;
  MPI_Arg x2buffsize = mdim0*mdim1;
  ...
  /* Update x0L boundaries */
  {
    ...
    if (x0l != MPI_PROC_NULL) {
      if (MPI_Irecv(x0recvbuf, x0buffsize, DataType, MPI_Arg(x0l), 
		    msg_xch_x0l, MPI_COMM_WORLD, &id_recv_x0l) != 0) {
	fprintf(stderr,"MGMPIBdryExchange p%i: x0L receive error\n",myrank);
	return FAIL;
      }
    }
    ...
    if (x0r != MPI_PROC_NULL) {
      i=mdim0-2;
      for (idx=0, k=0; k<mdim2; k++)
	for (j=0; j<mdim1; j++, idx++)
	  x0sendbuf[idx] = mgmpivec[(k*mdim1 + j)*mdim0 + i];
      
      if (MPI_Isend(x0sendbuf, x0buffsize, DataType, MPI_Arg(x0r), 
		    msg_xch_x0l, MPI_COMM_WORLD, &id_send_x0l) != 0) {
	fprintf(stderr,"MGMPIBdryExchange p%i: x0R send error\n",myrank);
	return FAIL;
      }
    }
	...
	fprintf(stderr,"MGMPIBdryExchange p%i: x0L wait error\n",myrank);
	return FAIL;
      }
      ...
      for (idx=0, k=0; k<mdim2; k++)
	for (j=0; j<mdim1; j++, idx++)
	  mgmpivec[(k*mdim1 + j)*mdim0 + i] = x0recvbuf[idx];      
    }
  }
  ...
  /* Update x0R boundaries */
  {
    ...
    if (x0r != MPI_PROC_NULL) {
      if (MPI_Irecv(x0recvbuf, x0buffsize, DataType, MPI_Arg(x0r), 
		    msg_xch_x0r, MPI_COMM_WORLD, &id_recv_x0r) != 0) {
	fprintf(stderr,"MGMPIBdryExchange p%i: x0R receive error\n",myrank);
	return FAIL;
      }
    }
	...
	for (j=0; j<mdim1; j++, idx++)
      ...
      if (MPI_Isend(x0sendbuf, x0buffsize, DataType, MPI_Arg(x0l), 
		    msg_xch_x0r, MPI_COMM_WORLD, &id_send_x0r) != 0) {
	fprintf(stderr,"MGMPIBdryExchange p%i: x0L send error\n",myrank);
	return FAIL;
      }
    }
	...
	fprintf(stderr,"MGMPIBdryExchange p%i: x0R wait error\n",myrank);
	return FAIL;
      }
  ...
  /* Update x1L boundaries */
  {
    ...
    if (x1l != MPI_PROC_NULL) {
      if (MPI_Irecv(x1recvbuf, x1buffsize, DataType, MPI_Arg(x1l), 
		    msg_xch_x1l, MPI_COMM_WORLD, &id_recv_x1l) != 0) {
	fprintf(stderr,"MGMPIBdryExchange p%i: x1L receive error\n",myrank);
	return FAIL;
      }
    }
	...
	for (i=0; i<mdim0; i++, idx++)
	  x1sendbuf[idx] = mgmpivec[(k*mdim1 + j)*mdim0 + i];
      
      if (MPI_Isend(x1sendbuf, x1buffsize, DataType, MPI_Arg(x1r), 
		    msg_xch_x1l, MPI_COMM_WORLD, &id_send_x1l) != 0) {
	fprintf(stderr,"MGMPIBdryExchange p%i: x1R send error\n",myrank);
	return FAIL;
      }
    }
	...
	fprintf(stderr,"MGMPIBdryExchange p%i: x1L wait error\n",myrank);
	return FAIL;
      }
      ...
      for (idx=0, k=0; k<mdim2; k++)
	for (i=0; i<mdim0; i++, idx++)
	  mgmpivec[(k*mdim1 + j)*mdim0 + i] = x1recvbuf[idx];      
    }
  }
  ...
  /* Update x1R boundaries */
  {
    ...
    if (x1r != MPI_PROC_NULL) {
      if (MPI_Irecv(x1recvbuf, x1buffsize, DataType, MPI_Arg(x1r), 
		    msg_xch_x1r, MPI_COMM_WORLD, &id_recv_x1r) != 0) {
	fprintf(stderr,"MGMPIBdryExchange p%i: x1R receive error\n",myrank);
	return FAIL;
      }
    }
	...
	for (i=0; i<mdim0; i++, idx++)
      ...
      if (MPI_Isend(x1sendbuf, x1buffsize, DataType, MPI_Arg(x1l), 
		    msg_xch_x1r, MPI_COMM_WORLD, &id_send_x1r) != 0) {
	fprintf(stderr,"MGMPIBdryExchange p%i: x1L send error\n",myrank);
	return FAIL;
      }
    }
	...
	fprintf(stderr,"MGMPIBdryExchange p%i: x1R wait error\n",myrank);
	return FAIL;
      }
  ...
  /* Update x2L boundaries */
  {
    ...
    if (x2l != MPI_PROC_NULL) {
      if (MPI_Irecv(x2recvbuf, x2buffsize, DataType, MPI_Arg(x2l), 
		    msg_xch_x2l, MPI_COMM_WORLD, &id_recv_x2l) != 0) {
	fprintf(stderr,"MGMPIBdryExchange p%i: x2L receive error\n",myrank);
	return FAIL;
      }
    }
    ...
    if (x2r != MPI_PROC_NULL) {
      k=mdim2-2;
      for (idx=0, j=0; j<mdim1; j++)
	for (i=0; i<mdim0; i++, idx++)
	  x2sendbuf[idx] = mgmpivec[(k*mdim1 + j)*mdim0 + i];
      
      if (MPI_Isend(x2sendbuf, x2buffsize, DataType, MPI_Arg(x2r), 
		    msg_xch_x2l, MPI_COMM_WORLD, &id_send_x2l) != 0) {
	fprintf(stderr,"MGMPIBdryExchange p%i: x2R send error\n",myrank);
	return FAIL;
      }
    }
	...
	fprintf(stderr,"MGMPIBdryExchange p%i: x2L wait error\n",myrank);
	return FAIL;
      }
      ...
      for (idx=0, j=0; j<mdim1; j++)
	for (i=0; i<mdim0; i++, idx++)
	  mgmpivec[(k*mdim1 + j)*mdim0 + i] = x2recvbuf[idx];      
    }
  }
  ...
  /* Update x2R boundaries */
  {
    ...
    if (x2r != MPI_PROC_NULL) {
      if (MPI_Irecv(x2recvbuf, x2buffsize, DataType, MPI_Arg(x2r), 
		    msg_xch_x2r, MPI_COMM_WORLD, &id_recv_x2r) != 0) {
	fprintf(stderr,"MGMPIBdryExchange p%i: x2R receive error\n",myrank);
	return FAIL;
      }
    }
	...
	for (i=0; i<mdim0; i++, idx++)
      ...
      if (MPI_Isend(x2sendbuf, x2buffsize, DataType, MPI_Arg(x2l), 
		    msg_xch_x2r, MPI_COMM_WORLD, &id_send_x2r) != 0) {
	fprintf(stderr,"MGMPIBdryExchange p%i: x2L send error\n",myrank);
	return FAIL;
      }
    }
	...
	fprintf(stderr,"MGMPIBdryExchange p%i: x2R wait error\n",myrank);
	return FAIL;
      }
