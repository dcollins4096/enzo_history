...
#else
#ifdef PPML
int SolveHyperbolicEquations(HierarchyEntry **Grids, int NumberOfGrids, int CycleNumber, 
			     int NumberOfSubgrids[],fluxes ***SubgridFluxes, int level,
...
#ifdef PPML
  JBMEM_MESSAGE(MyProcessorNumber, "Enter EL");
#endif //PPML
  /* Declarations */
...
#ifdef PPML
  //cleans up outstanding mesages.
  CommunicationCheckOutstanding();

  if( LevelCycleCount[level] == 0 )
  if( PPML_InitInterfaceDataLoop(Grids, NumberOfGrids, level,
				 MetaData, Exterior, LevelArray[level]
#ifdef SIB2
				 ,SiblingList
#endif //SIB2
				 ) == FAIL){
      fprintf(stderr,"Failure in SolveHyperbolicEquations\n");
      return FAIL;
    }
...
#ifdef PPML
    //This here is for fixed timestep.
    dccCounter02= LevelCycleCount[level];
...
//  fprintf(stderr, "%"ISYM": EvolveLevel: Enter PrepareDensityField\n", MyProcessorNumber);
    ...
    if (SelfGravity)
#ifdef ISO_GRAV
      if (PrepareDensityField(LevelArray, SiblingList, level, 
			      MetaData, PotentialBdry, When) == FAIL) {
#else   // !ISO_GRAV
    ...
    if (SelfGravity)
#ifdef ISO_GRAV
      if (PrepareDensityField(LevelArray, level, MetaData, PotentialBdry, When) == FAIL) {
#else   // !ISO_GRAV
...
//  fprintf(stderr, "%"ISYM": EvolveLevel: Exit PrepareDensityField\n", MyProcessorNumber);
    ...
    if( ComputeGlobalStats(LevelArray,level,MetaData) == FAIL )
...
#ifdef PPML
      if( MidWayDumpCheck( 40 ) == TRUE ){
	char basename[30];
	sprintf(basename, "data40%d%d.grid",LevelCycleCount[level], level);
...
#ifdef PPML
      if( MidWayDumpCheck( 41 ) == TRUE ){
	char basename[30];
	sprintf(basename, "data41%d%d.grid",LevelCycleCount[level], level);
	FILE *dummy = fopen(basename, "a");    
	Grids[grid1]->GridData->WriteGrid(dummy, basename, grid1+1);
	fclose(dummy);
      }
...
#ifdef PPML
      //This is one of the more egrigious changes.  The main grid loop is broken into two distint pieces,
      //and SolveHydroEquations is moved into This call, SolveHyperbolicEquations.

    }//grid loop
    //This is the general wrapper for all things hyperbolic.
    //Calls PPM_DE, PPML_LR, Zeus Hydro (pure hydro solvers)
    //or PPML (MHD solver)

    if( SolveHyperbolicEquations(Grids, NumberOfGrids, LevelCycleCount[level], 
...
//      fprintf(stderr, "%"ISYM": Called RadiativeCooling\n", MyProcessorNumber);
      ...
      /* Include radiation-hydrodynamics-chemistry coupling. */
#ifdef RAD_HYDRO 
//      fprintf(stderr, "%"ISYM": Calling EvolveRadHydro\n", MyProcessorNumber);
 
#ifdef USE_JBPERF
      JBPERF_START("evolve-level-15.5"); // EvolveRadHydro()
#endif

      if (RadiationHydrodynamics) 
	if (EvolveRadHydro(Grids[grid1], FLDsolver) == FAIL) {
	  fprintf(stderr, "Error in grid->EvolveRadHydro.\n");
	  return FAIL;
	}
 
#ifdef USE_JBPERF
      JBPERF_STOP("evolve-level-15.5"); // EvolveRadHydro()
#endif
...
//      fprintf(stderr, "%"ISYM": Called UpdatePP\n", MyProcessorNumber);
...
#else   // !SIB3
#ifdef ISO_GRAV
      if (PrepareDensityField(LevelArray, level, MetaData, 
			      PotentialBdry, When) == FAIL) {
#else   // !ISO_GRAV
      if (PrepareDensityField(LevelArray, level, MetaData, When) == FAIL) {
#endif  // end ISO_GRAV
        fprintf(stderr, "Error in PrepareDensityField.\n");
	...
	Temp2 = Temp2->NextGridThisLevel; /* ugh: find last in linked list */
      ...
      if (WriteAllData(MetaData->DataDumpName, MetaData->DataDumpNumber++,
		       Temp2->GridHierarchyEntry, *MetaData, Exterior, PotentialBdry,
		       LevelArray[level]->GridData->ReturnTime()) == FAIL) {
#else
      if (WriteAllData(MetaData->DataDumpName, MetaData->DataDumpNumber++,
#ifdef ISO_GRAV
...
#ifdef ISO_GRAV
#ifdef RAD_HYDRO
      if (EvolveLevel(MetaData, LevelArray, level+1, dtThisLevel, 
		      Exterior, PotentialBdry, FLDsolver) == FAIL) {
#else
      if (EvolveLevel(MetaData, LevelArray, level+1, 
		      dtThisLevel, Exterior, PotentialBdry) == FAIL) {
#endif
#else
#ifdef RAD_HYDRO
      if (EvolveLevel(MetaData, LevelArray, level+1, dtThisLevel, 
