...
//Slightly redunant block of defines: much easier to read.
  ...
  while (fgets(line, MAX_LINE_LENGTH, fptr) != NULL) {
	...
	&& line[0] != '#')
      fprintf(stderr,
   "TurbSimInit: the following parameter line was not interpreted:\n%s\n", line);
...
#endif //PPML.  Note that this is no longer a necessary condition. 
				    ...
				    TopGridData &MetaData)
{
  ...
  char *DensityName = NULL, *TotalEnergyName = NULL, *GasEnergyName = NULL,
       ...
       *RandomForcingNames[MAX_DIMENSION];
#ifdef PPML
  char *MagneticNames[MAX_DIMENSION];
#endif //PPML
  for (dim = 0; dim < MAX_DIMENSION; dim++) {
    VelocityNames[dim] = NULL;
    RandomForcingNames[dim] = NULL;
