...
GlobalStats::GlobalStats(){
    ...
    N_Registered= N_Avg= N_RMS= N_Max= N_Min=0, N_Sum =0;
...
int GlobalStats::RegisterNewStat(char * name, char * output, GlobalStats_Comm comm){
  
  if( this->elem(name) == NULL){
    
    Stat_Obj * ThisStat  = new Stat_Obj(name, output, comm);  
    Stat[N_Registered++] = ThisStat;
...
Stat_Obj * GlobalStats::elem(char * name){
  Stat_Obj * found  = NULL;
  int i;
  ...
  //if( found == NULL ){
  //fprintf(stderr,"GlobalStats: Warning: value %s not found\n",name);
  //}
  return found;
}
void GlobalStats::print(FILE * FPTR){
  ...
  if( MyProcessorNumber == ROOT_PROCESSOR ){
    ...
    for( i=0; i<N_Registered; i++){
      ...
      //fprintf(stderr, "%d.) %s ",i+1,Stat[i]->Name);
      fprintf(FPTR, Stat[i]->OutputMethod,Stat[i]->value);
    }
    fprintf(FPTR,"\n");    

    /*
    fprintf(FPTR, "%d %9.6e %9.6e %9.6f %9.6f %4.2f %4.2f %4.2f %9.6f %9.6f %9.6f %9.6f %9.6f %9.6f %9.6f %9.6f %9.6f \n",
    CycleNumber, Time, Timestep, RMSVelocity, RMSMagneticField, 
    AvgMagneticField[0],AvgMagneticField[1],AvgMagneticField[2], RMSMachNumber,DensityVariance, 
    AvgKinetic, MaxDensity, MaxSpeed, MaxMagneticField, MinDensity,
    RMSAlvMach, AvgBeta);
  */    
    }//root proc
