...
int CommunicationPartitionGrid(HierarchyEntry *Grid)
{
  ...
  printf("Enter CommunicationPartitionGrid on CPU %"ISYM"\n", MyProcessorNumber);
#ifdef USE_MPI
  ...
  for (dim = 0; dim < MAX_DIMENSION; dim++) {
    ...
    for (i = 0; i < Layout[dim]; i++) {
      ...
      /* NOTE: FFT requires even grid sizes on dim==0 axis. */

      if (SelfGravity == TRUE && dim == 0) {
...
//      printf("GC = %"ISYM", K J I: %"ISYM" %"ISYM" %"ISYM"  NewProc = %"ISYM", ProcMap = %"ISYM"\n",gridcounter,k,j,i,NewProc,ProcMap);
 
	/* Move Grid from current processor to new Processor. */
 
#ifdef USE_OLD_DISTRIBUTION
