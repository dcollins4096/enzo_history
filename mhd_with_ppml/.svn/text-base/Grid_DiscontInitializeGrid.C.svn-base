...
int grid::DiscontInitializeGrid(float DensityA, float DensityB,
  ...
  
{
  ...
  int i, j, k, field, size=1, index, index2, dim, which = 0, vel;
  ...
  //  DiscontCenter is in Physical Units.
  //
  //
  
  for(i=0;i<GridRank;i++){
    BlastCenterLocal[i] = DiscontCenter[i];
  ...
  //Variable names.
  int Eeng, Eden, Ev[3], Egas, Eb[3]; 
  if (this->IdentifyMHDQuantities(Eden, Egas, Ev[0], Ev[1], 
				       Ev[2], Eeng,Eb[0],Eb[1],Eb[2]) == FAIL) {
    fprintf(stderr, "Error in IdentifyMHDQuantities.\n");
    return FAIL;
  }
  
  //For characteristic advection.  Right[field][wave]
  //80,81,82: square wave, left going alfven, along x,y, or z
  //70,71,72: sine wave, left going alfvven, along x,y,z
  ...
  for(k=InitStartIndex[2];k<=InitEndIndex[2]; k++)
	...
	case 8:
	  ...
	  if(InitStyle == 6) value = i-DEFAULT_GHOST_ZONES;
	  if(InitStyle == 7) value = j-DEFAULT_GHOST_ZONES;
	  if(InitStyle == 8) value = k-DEFAULT_GHOST_ZONES;
	  
	  BaryonField[ Eden ][index] = value +  0.1 ;
	  BaryonField[ Ev[0] ][index] = value + 0.2;
	  if( GridRank > 1 || MHD_Used == TRUE)
	    BaryonField[ Ev[1] ][index] = value + 0.3;
	  if( GridRank > 2 || MHD_Used == TRUE)
	    BaryonField[ Ev[2] ][index] = value + 0.4;
	  if( EquationOfState == 0)
	    BaryonField[ Eeng][index] = value + 0.5;
	  if( MHD_Used == TRUE ){
	    BaryonField[ Eb[0] ][index] = value + 0.6;
	    BaryonField[ Eb[0] ][index] = value + 0.7;
	    BaryonField[ Eb[0] ][index] = value + 0.8;
	  }
	...
	case 10:
	  ...
	  if( fraction == -1 ){
	    fprintf(stderr," DiscontInitializeGrid: problem with Volume Wrapper.\n");
	    return FAIL;
	  }
	  break;
	default:
	  ...
	  if( fraction == -1 ){
	    fprintf(stderr,"DiscontInitializeGrid: problem with cell centered blast style\n");
	    return FAIL;
	  }
	  break;
	...
	if( which == 0 ) {
	  ...
	  if( MHD_Used == TRUE ){
	    BaryonField[ Eb[0] ][index] = (1-fraction) * BA[0] + fraction*BB[0];
	    BaryonField[ Eb[1] ][index] = (1-fraction) * BA[1] + fraction*BB[1];
	    BaryonField[ Eb[2] ][index] = (1-fraction) * BA[2] + fraction*BB[2];
	  }
	}else{
	  continue;
	}
	...
	for(j=0; j<MagneticDims[field][1]; j++)
	  for(i=0; i<MagneticDims[field][0];i++){
	    ...
	    case 10:
	      ...
	      if( fraction == -1 ){
		fprintf(stderr,"DiscontInitializeGrid: problem with Area Wrapper.\n");
		return FAIL;
	      }
	    ...
	    default:
	      ...
	      if( fraction == -1 ){
		fprintf(stderr,"DiscontInitializeGrid: problem with Face Centered blast style.\n");
		return FAIL; 
	      }
	      break;
  ...
  if(WriteInThisF(99)==TRUE ){
    ...
    if( this->WriteGrid(dccptr,basename,1) == FAIL ){
      fprintf(stderr,"Robert Anton Wilson says Damn..\n");
      return FAIL;
    }
    fclose(dccptr);
  ...
  if( RandomForcing == TRUE ){
    ...
    for(dim=0; dim < GridRank; dim++){
      for (i = 0; i < size; i++)
	RandomForcingField[counter][i] = BaryonField[ Ev[dim] ][i];
      counter++;
    }
    if( HydroMethod == PPM_Local) {
	...
	for(dim=0; dim < GridRank; dim++){
	  for (i = 0; i < size; i++)
	    RandomForcingField[counter][i] = Face->All[face_part][Ev[dim] ][i];
	  counter++;
	}
    }//ppm_local
#else //PPML
...
float blaststyle(int i,int j, int k, int InitStyle, float BlastCenterLocal[], float Radius){
  ...
  default:
    which = -1;
    fprintf(stderr,"Discont: Invalid Init Style %d\n",InitStyle);
    break;
...
{  
  ...
  */
  ...
  right[6][2] = right[4][2]*betaz/betay;
  ...
  right[6][4] = right[4][4]*betaz/betay;
