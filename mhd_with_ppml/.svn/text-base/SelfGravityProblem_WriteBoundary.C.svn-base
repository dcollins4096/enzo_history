...
/***********************************************************************
/
/  Self Gravity Problem, write boundary conditions routine
/
...
int SelfGravityProblem::WriteBoundary(FILE *fptr, char *hdfname)
{
  ...
  if (MyProcessorNumber == 0) {
    ...
    WriteListOfInts(fptr, rank, BdryType);
  ...
  for (dim=0; dim<rank; dim++)
	...
	if (OnBdry[dim][face] && (BdryVals[dim][face] != NULL)) {
	  ...
	  for (j = 0; j < int(MemDims[1]); j++)
		...
		float32(BdryVals[dim][face][j*MemDims[0]+i]);
