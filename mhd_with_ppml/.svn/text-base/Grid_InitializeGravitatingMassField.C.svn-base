...
int grid::InitializeGravitatingMassField(int RefinementFactor)
{
  ...
  /* Determine the size of the mass grid we'll need.
     1) For isolated top grid, this is just the active grid size plus 
        the boundary zones plus one additional boundary zone
     2) For periodic top grid, this is just the active grid size (however,
        this defaults to SubGridIsolated instead)
  ...
  for (dim = 0; dim < GridRank; dim++) {
    ...
    if (GravityBoundaryType == TopGridIsolated) {
	 ...
	 plus the GravityBufferSize plus 1 on either side. -- D.R. */
      DimTemp = GridEndIndex[dim] - GridStartIndex[dim] + 1;
      BufferSize = GravityBufferSize+1;

      GravityGhostZones = BufferSize;  // store this for later -- D.R.
