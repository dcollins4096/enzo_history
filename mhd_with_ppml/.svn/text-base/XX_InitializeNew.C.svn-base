		  ...
		  float *Initialdt)
{
...
#ifdef PPML
  if (ProblemType == 100)
    ret = DiscontInitialize(fptr, Outfptr, TopGrid, MetaData, Exterior);
  ...
  if (ProblemType == 200)
    ret = RadHydroConstTestInitialize(fptr, Outfptr, TopGrid, MetaData);

  // 201) Radiation-Hydrodynamics test 2 -- stream test
  if (ProblemType == 201)
    ret = RadHydroStreamTestInitialize(fptr, Outfptr, TopGrid, MetaData);
#endif
