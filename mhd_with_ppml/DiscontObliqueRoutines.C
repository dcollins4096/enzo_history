...
//
//
int SetupNormal(float Normal[], float DiscontCenter[3], TopGridData & MetaData);
...
float VolumeWrapper(int i,int j,int k, int rank, float dx ,float dy, float dz,float Normal[]){
  ...
  if(rank == 1 ){
    fprintf(stderr,"Grid_DiscontInitializeGrid: Don't use InitStyle 10 with a 1d problem.\n");
    return -1;
  }
...
int SetupNormal(float Normal[], float DiscontCenter[3], TopGridData & MetaData){
  ...
  Normal[3] = -(Normal[0]*(DiscontCenter[0]-DomainLeftEdge[0]) + 
			Normal[1]*(DiscontCenter[1]-DomainLeftEdge[1]) + 
			Normal[2]*(DiscontCenter[2]-DomainLeftEdge[2])); 
  ...
  for( dim=1;dim<MetaData.TopGridRank;dim++){
	...
	SolverMin + (DomainRightEdge[dim] - DomainLeftEdge[dim])*(Normal[dim]/Normal[0] )  ){
      ...
      fprintf(stderr,"       In your case, you need %f\n",
	      SolverMin + (DomainRightEdge[dim] - DomainLeftEdge[dim]) *(Normal[dim]/(Normal[0]*CellWidth[dim]) ));
