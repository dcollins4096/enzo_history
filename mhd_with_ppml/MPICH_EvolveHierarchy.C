...
#ifdef PPML
  //Sick of accidentally writing in 32 bit.  
  if( HydroMethod == PPM_Local ){
    int I_need_64_bit_output = TRUE;
    if( I_need_64_bit_output == TRUE ){
#ifdef io4
      fprintf(stderr,"ERROR: IO set to 32 bit. You HATE 32 bit IO.\n");
      return FAIL;
#endif //io4
    }}
#endif //PPML 
...
#ifdef PPML
    if (MyProcessorNumber == ROOT_PROCESSOR) {
      fprintf(stderr,"STEP_INFO N= %d, DT = %.14g, T = %.14g\n", 
