			      ...
			      fluxes *SubgridFluxes[], int level)
{
  ...
  if( DEFAULT_GHOST_ZONES != 3 ){
    fprintf(stderr,"Catestrophic Error! Wrong number of ghost zones defined. Needs 3, you're using %d\n",
	    DEFAULT_GHOST_ZONES);
    return FAIL;
  }
  if( MidWayDumpCheck( 30 ) == TRUE ){
    ...
    if( this->WriteGrid(dummy, basename, 1) == FAIL ){
      fprintf(stderr, "Problem with Write Grid in SolveHydro.\n");
      return FAIL;
    }
    fclose(dummy);
  }
