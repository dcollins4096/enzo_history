...
; rough check of input data
  slice_data = dblarr(slice_size(0), slice_size(1))
  slice_data(*,*) = -1.e30
...
; use only grids that will show up on the image:
  b = 0.7*(max_right-min_left)/DOUBLE(slice_size)
...
;                index(0),index(1),index(2),0.,0.,0.))
      Left_point = FIX(DOUBLE(slice_size) *scale_up * $
                         ((u_g(i).Left_edge(other_subs)+ $
                           (i_l)*delta_dist(other_subs)-min_left) > 0.))
      Right_point= Left_point + $
        ROUND(DOUBLE(slice_size) *scale_up * $
              double(i_r-i_l+1)*delta_dist(other_subs)) < (slice_size-1)
