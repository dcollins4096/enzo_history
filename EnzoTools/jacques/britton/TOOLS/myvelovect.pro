; $Id: myvelovect.pro,v 1.1 2006/12/14 06:17:27 bwoshea Exp $
...
;
        ...
        y0 = min(y)
        y1 = max(y)
	x_step=double(x1-x0)/double(s(1))   ; Convert to double. Integer math
	y_step=double(y1-y0)/double(s(2))   ; could result in divide by 0
...
;
        s_good = size(good) 
        for i=0,s_good(3)-1 do begin     ;Each point
