; $Id: myslice.pro,v 1.1 2006/12/14 06:17:27 bwoshea Exp $
...
v = [ [(points mod !d.x_size) / double(!d.x_size)], $ ;Norm X
	[(points / !d.x_size)   / double(!d.y_size)], $ ;Norm Y
...
p = double([ev.x, ev.y])			;device coords of mouse
...
x = ev.x / double(!d.x_size)		;Normalized coords
y = ev.y / double(!d.y_size)
d1 = dims -1
...
if ev.id eq sl.draw THEN BEGIN		;mouse press?
  ...
  probe_it: if (n_elements(zb_last) le 1) THEN return
    set_plot,'Z'
    z = double(tvrd(ev.x, ev.y, /WORD, /CHANNEL))/65530. + .5 ;To 0-1
    set_plot,sl.gdev
...
sl = {  SLICER, base : 0L, $		;Main base
	...
	orthop : dblarr(4), $		;Plane eqn for ortho slices
	...
	p0 : dblarr(3,8), $		;Data coords of cube corners
	p1 : dblarr(3,8), $		;Device coords of cube corners
	pt_inverse : dblarr(4,4), $	;Inverse of !P.T
...
junk = WIDGET_DRAW(parent, XSIZE = sl_width, $
	YSIZE = sl_width * double(resolution(1)) / resolution(0))
