...
pro read_grid,gridindex,file_name,data,$
             byname=dataset_name
...
; THIS NO LONGER WORKS WITH PACKED AMR
...
;   gridindex=long(strmid(file_name,pos+5))-1l
  ...
  if (amr_is_packed eq 1) then begin
      groupData = 'Grid' + string(gridindex,format='(I08)') + '/' + dataset_name
      dataset_id = h5d_open(file_id,groupData)
  endif else begin
