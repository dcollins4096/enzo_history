...
PRO AMRslice, grid_in_info, data_in_dir ; GROUP=Group
@common_blocks.inc

device, retain=1,  BYPASS_TRANSLATION=0 

  if ((N_ELEMENTS(grid_info)   lt 1) OR  $
      (N_ELEMENTS(data_in_dir) lt 1) OR $
      (N_ELEMENTS(list_str) lt 1)) THEN BEGIN
      file_name = pickfile(FILTER = '*.hierarchy', $
                              Path=data_in_dir,   $
                           TITLE  = 'select hierarchy files', $
                           /MUST_EXIST, GET_PATH = data_in_dir, /NOCONFIRM)
      READ_GRID_INFO, data_in_dir, file_name, Grid_in_info, $
        list_str, mass_per_particle, divide_by_density, REDSHIFT = z_current
  ENDIF
