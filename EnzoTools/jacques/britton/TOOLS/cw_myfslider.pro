; $Id: cw_myfslider.pro,v 1.1 2006/12/14 06:17:27 bwoshea Exp $
...
;	integer only. This compound widget provides a doubleing point
;	slider.
;
; CATEGORY:
;	Compound widgets.
...
PRO myfslider_set_value, id, value
  ...
  WIDGET_CONTROL, state.slideid, $
	SET_VALUE = 1000000. * $
		(double(value(1)) - state.bot) / (state.top - state.bot)
  IF (state.labelid NE 0) THEN $
  	WIDGET_CONTROL, state.labelid, $
		SET_VALUE = STRING(DOUBLE(value(1)), format=state.format)

  WIDGET_CONTROL, stash, SET_UVALUE=state, /NO_COPY
END


FUNCTION myfslider_get_value, id
  ...
  ret = [state.bot, $
         ((tmp / 1000000.) * (state.top - state.bot)) + state.bot, $
         state.top]
...
FUNCTION myfslide_event, ev
  ...
  if (ev.id eq state.slideid) then begin
    ...
    ; Compute the doubleing point value
...
FUNCTION cw_myfslider, parent, $
  ...
  ; double slider is vertical or horizontal
  ...
  if (vert) then begin
    ...
    labelbase = mainbase
  endelse
  WIDGET_CONTROL, mainbase, SET_UVALUE = uval, EVENT_FUNC = 'myfslide_event', $
	PRO_SET_VALUE='MYFSLIDER_SET_VALUE', $
	FUNC_GET_VALUE='MYFSLIDER_GET_VALUE'
  ...
  IF (sup EQ 0) THEN $
    ...
    state.labelid = WIDGET_TEXT(labelbase, YOFFSET = l_yoff, $
		VALUE = STRING(DOUBLE(val), format=state.format), $
                edit=edit) $
