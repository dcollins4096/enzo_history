; $Id: cw_mybgroup.pro,v 1.1 2006/12/14 06:17:27 bwoshea Exp $
...
function CW_myBGROUP, parent, names, $
  ...
  base = WIDGET_BASE(next_base, COLUMN=column, FRAME=frame, $
	 ROW=row, SCROLL=scroll, SPACE=space, $
  ...
  for i = 0, n-1 do begin
    if (n_elements(font) eq 0) then begin
        junk_base(i) = WIDGET_BASE(base,/ROW,/NONEXCLUSIVE,map=0 )
        ids(i) = WIDGET_BUTTON(junk_base(i), value=names(i), UVALUE=i, /DYNAMIC_RESIZE )
    endif else begin
        junk_base(i) = WIDGET_BASE(base,/ROW,/NONEXCLUSIVE,map=0)
        ids(i) = WIDGET_BUTTON(junk_base(i), value=names(i), FONT=font, UVALUE=i, /DYNAMIC_RESIZE )
    endelse
  endfor
