; $Id: cw_myclr_index.pro,v 1.1 2006/12/14 06:17:27 bwoshea Exp $
;
...
function CW_MYCOLOR_INDEXE, ev		;Color index widget's event proc
...
CW_MYCOLOR_INDEXS, base, c	
ret =  { CW_MYCOLOR_INDEX, ID: base, TOP: ev.top, HANDLER: 0L, VALUE: c}
if state.efun eq '' then return, ret $
else return, CALL_FUNCTION(state.efun, ret)
end


function CW_MYCOLOR_INDEXG, id
	widget_control, widget_info(id, /child), get_uvalue = state
	return, state.value
end


pro CW_MYCOLOR_INDEXS, id, value		;Set color index widget value
...
function CW_MYCLR_INDEX, parent, LABEL = label, FRAME = frame, $
...
base = widget_base(parent, COLUMN=2, FRAME = frame, $
	EVENT_FUNC = 'CW_MYCOLOR_INDEXE', FUNC_GET_VALUE = 'CW_MYCOLOR_INDEXG', $
	PRO_SET_VALUE = 'CW_MYCOLOR_INDEXS', UVALUE = uvalue)
if n_elements(efun) le 0 then efun = ''
state = { CW_MYC_INDEX_STATE, $
	txt_id : 0L, rect_id : 0L, pwin_id : 0L, inited : 0, efun: efun, $
	scale : double(ncolors) / xsize, value : 0L, $
