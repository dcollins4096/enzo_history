; $Id: cw_myfslider.pro,v 1.1 2006/12/14 06:20:09 bwoshea Exp $
...
PRO myfslider_set_value, id, value
  ...
  WIDGET_CONTROL, state.slideid, $
	SET_VALUE = 1000000. * $
		(float(value(1)) - state.bot) / (state.top - state.bot)
  IF (state.labelid NE 0) THEN $
  	WIDGET_CONTROL, state.labelid, $
		SET_VALUE = STRING(FLOAT(value(1)), format=state.format)
