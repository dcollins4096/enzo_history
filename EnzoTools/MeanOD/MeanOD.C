...
inputs:
  halo x position (box units)
  halo y position (box units)
  halo z position (box units)
  halo virial radius (proper parsecs)

  calculates the mean overdensity of all gas within a multiple of the 
  virial radius of a given halo.
...
int main(int argc, char *argv[]){
  ...
  if(argc != 6){
    fprintf(stderr,"5 inputs needed!\n");
    fprintf(stderr,"meanod <filename> <xpos> <ypos> <zpos> <radius>\n");
    exit(-123);
  }

  inputfilename=argv[1];

  haloxpos = (double) atof(argv[2]);
  haloypos = (double) atof(argv[3]);
  halozpos = (double) atof(argv[4]);

  virial_radius = (double) atof(argv[5]);

  if(DEBUG){
    fprintf(stderr,"Inputs:  %d\n",argc);
    fprintf(stderr,"  xpos:  %.12lf\n",haloxpos);
    fprintf(stderr,"  ypos:  %.12lf\n",haloypos);
    fprintf(stderr,"  zpos:  %.12lf\n",halozpos);
    fprintf(stderr,"  virial radius:  %.12lf\n",virial_radius);
  }
  ...
  double lengthunits = boxsize / hubble * 1.0e+6/ (1.0+redshift);  // proper size of box in pc

  radius_boxunits = virial_radius / lengthunits;

  radius_5  =  5.0*radius_boxunits;
  radius_10 = 10.0*radius_boxunits;
  radius_20 = 20.0*radius_boxunits;
  radius_40 = 40.0*radius_boxunits;
  radius_80 = 80.0*radius_boxunits;

  if(DEBUG){
    fprintf(stderr,"Rvir in box units:      %.12lf\n",radius_boxunits);
    fprintf(stderr,"radius_5  (box units):  %.12lf\n",radius_5);
    fprintf(stderr,"radius_10 (box units):  %.12lf\n",radius_10);
    fprintf(stderr,"radius_20 (box units):  %.12lf\n",radius_20);
    fprintf(stderr,"radius_40 (box units):  %.12lf\n",radius_40);
    fprintf(stderr,"radius_80 (box units):  %.12lf\n",radius_80);
  }
  ...
  r5_dens = r10_dens = r20_dens = r40_dens = r80_dens = 0.0;
  r5_weight = r10_weight = r20_weight = r40_weight = r80_weight = 0.0;
  ...
  fprintf(stderr,"the mean baryon density is %e (from main.C)\n",meanoverdensity);
  ...
  fprintf(stderr,"r5_dens, weight:     %e  %e\n",r5_dens, r5_weight);
  fprintf(stderr,"r10_dens, weight:    %e  %e\n",r10_dens, r10_weight);
  fprintf(stderr,"r20_dens, weight:    %e  %e\n",r20_dens, r20_weight);
  fprintf(stderr,"r40_dens, weight:    %e  %e\n",r40_dens, r40_weight);
  fprintf(stderr,"r80_dens, weight:    %e  %e\n",r80_dens, r80_weight);
...
void OutputAllInformation(char *infilename){
  ...
  fprintf(output,"\n");
  fprintf(output,"r5  overdensity      = %e\n",r5_dens);
  fprintf(output,"r10 overdensity      = %e\n",r10_dens);
  fprintf(output,"r20 overdensity      = %e\n",r20_dens);
  fprintf(output,"r40 overdensity      = %e\n",r40_dens);
  fprintf(output,"r80 overdensity      = %e\n",r80_dens);
...
int GetCellInformationFromGrid(int gridnum,int total_number_grids, int whichroutine){
  ...
  if(DEBUG) fprintf(stderr,"in GetCellInformationFromGrid %d %d\n",gridnum,whichroutine);
...
int GetDensityInfo(int gridnum){
  ...
  double cellsize,cellvol,cellmass, ccx,ccy,ccz, H2frac, efrac, HMfrac,
    overdensity,entropy,jeansmass,HIIfrac, totalradius, massthisgrid, volumethisgrid,
    overdensitythisgrid;
  ...
  for(k=0; k < griddz[gridnum]; k++)
	...
	if(flagbuffer[cellindex] != -1){ // read just non-overlapped grid cells
	  ...
	  totalradius = sqrt(    pow( haloxpos-ccx   , 2.0) 
			       + pow( haloypos-ccy   , 2.0)
			       + pow( halozpos-ccz   , 2.0) );
	  ...
	  massthisgrid += cellmass;
	  volumethisgrid += cellvol;
	  overdensitythisgrid += overdensity*cellvol;

	  meanoverdensity += overdensity*cellvol;
	  meanoverdensity_weight += cellvol;
	  
	  if(totalradius <= radius_5){
	    r5_dens += overdensity*cellvol;
	    r5_weight += cellvol;
	  }

	  if(totalradius <= radius_10){
	    r10_dens += overdensity*cellvol;
	    r10_weight += cellvol;
	  }

	  if(totalradius <= radius_20){
	    r20_dens += overdensity*cellvol;
	    r20_weight += cellvol;
	  }

	  if(totalradius <= radius_40){
	    r40_dens += overdensity*cellvol;
	    r40_weight += cellvol;
	  }

	  if(totalradius <= radius_80){
	    r80_dens += overdensity*cellvol;
	    r80_weight += cellvol;
	  }
  ...
  if(VERBOSEDEBUG)
    fprintf(stderr,"GetDensityInfo:  %e  %e  %e  %e %e %e %e\n",
	    meanoverdensity, meanoverdensity_weight, meanoverdensity/meanoverdensity_weight, 
	    cellvol, massthisgrid, volumethisgrid, overdensitythisgrid/volumethisgrid);
