...
int main(int argc, char *argv[]){
  ...
  if(argc != 2){
    fprintf(stderr,"I need one argument!  gpp_testpart <paramfile>\n");
    exit(-123);
  }
  ...
  fprintf(outputfile,"%lf %lf %lf    %e %e %e    %lf %lf %lf    %e %e %e\n",
	  xpositions[0],ypositions[0],zpositions[0],
	  xvelocities[0],yvelocities[0],zvelocities[0],
	  xpositions[1],ypositions[1],zpositions[1],
	  xvelocities[1],yvelocities[1],zvelocities[1]);
...
int AddParticlesToArrays(int gridnum){
  ...
  float *particle_posx_buffer, *particle_posy_buffer, 
  ...
  if ( H5Tequal( typ_id, H5T_IEEE_F32BE ) )
    {
      ...
      particle_posx_buffer = new float[(int) size];
      particle_posy_buffer = new float[(int) size];
      particle_posz_buffer = new float[(int) size];
