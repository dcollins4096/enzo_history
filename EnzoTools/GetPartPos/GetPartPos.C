...
the original version of GetPartPos.C.  Reads in normal AMR hierarchies and
writes out a 3-column ASCII file with x,y,z positions in Enzo's internal 
position units.
...
int main(int argc, char *argv[]){
  ...
  numpart=atoi( argv[2] );   // number of particles (on an edge)


#ifdef PACK_AMR

  fprintf(stderr,"GetPartPos:  PACK_AMR is TURNED ON\n");

#else

  fprintf(stderr,"GetPartPos:  PACK_AMR is OFF\n");
  ...
  //totalparticlesfromuser=numpart*numpart*numpart;  
  ...
  fprintf(stderr,"writing file dmposinfo.dat\n") ;
    ...
    /*
  fwrite(xpositions,sizeof(float),totalparticlesfromuser,outputfile);
  fwrite(ypositions,sizeof(float),totalparticlesfromuser,outputfile);
  fwrite(zpositions,sizeof(float),totalparticlesfromuser,outputfile);
    */
  fclose(outputfile);
