...
// calculate mass filling factor!
#define Z_MFF_MIN  0.000001
#define Z1_MFF_MIN 0.000001
#define Z2_MFF_MIN 0.000001
...
int main(int argc, char *argv[]){
  ...
  float z_mff_sum, z1_mff_sum, z2_mff_sum;
  ...
  // keep track of mass filling factor
  z_mff_sum = z1_mff_sum = z2_mff_sum = 0.0;
  ...
  while( fgets(line, MAX_LINE_LENGTH, hierfile) != NULL){
	...
	if(tempbin<0 || densbin<0 || entbin < 0 || 
	  ...
	  if( tempbin >= NUMBEROFBINS ) tempbin = NUMBEROFBINS-1;
	  if( densbin >= NUMBEROFBINS ) densbin = NUMBEROFBINS-1;
	  if( entbin >= NUMBEROFBINS ) entbin = NUMBEROFBINS-1;
	...
	// increment mass filling factor # cells
	if(metallicity >= Z_MFF_MIN) z_mff_sum += densbuff[i] * cellvol;
	if(z1_metallicity >= Z1_MFF_MIN) z1_mff_sum += densbuff[i] * cellvol;
	if(z2_metallicity >= Z2_MFF_MIN) z2_mff_sum += densbuff[i] * cellvol;
	...
	if(entbin<0 || densbin<0 || entbin >= PIXELS || densbin >= PIXELS || tempbin<0 || tempbin >= PIXELS
	  ...
	  if( tempbin >= PIXELS ) tempbin = PIXELS-1;
	  if( densbin >= PIXELS ) densbin = PIXELS-1;
	  if( entbin >= PIXELS ) entbin = PIXELS-1;
  ...
  fprintf(outputfile,"Z: mass filling factor      %e\n\n",
	  (z_mff_sum/total_mass));
  ...
  fprintf(outputfile,"Z1: mass filling factor      %e\n\n",
	  (z1_mff_sum/total_mass));
  ...
  fprintf(outputfile,"Z2: mass filling factor      %e\n\n",
	  (z2_mff_sum/total_mass));
