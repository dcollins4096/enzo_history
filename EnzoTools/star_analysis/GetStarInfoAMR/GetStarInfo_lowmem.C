...
int AddParticlesToArrays(int gridnum){
  ...
  for(i=0; i<numberofparticles; i++)
    if(particle_crtime_buffer[i] > 0.0){
      fprintf(outputfile,"%f %f %f %e %e %e %e %f %f %e\n",
	      (float) particle_posx_buffer[i], (float) particle_posy_buffer[i], (float) particle_posz_buffer[i],
	      particle_xvel_buffer[i], particle_yvel_buffer[i], particle_zvel_buffer[i],
	      particle_mass_buffer[i]*cellvolume,particle_crtime_buffer[i],
	      particle_dyntime_buffer[i],particle_mfrac_buffer[i]);
      particle_count++;
    }
