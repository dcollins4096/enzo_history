...
Eint32 main(Eint32 argc, char *argv[])
{
  ...
  if (MyProcessorNumber == ROOT_PROCESSOR) {
    ...
    if (ENZO_VERSION_PRE)   printf ("-pre%d",  ENZO_VERSION_PRE);
    if (ENZO_VERSION_PATCH) printf ("-patch%d",ENZO_VERSION_PATCH);
    printf ("\n");
#ifdef TP_VELOCITY
    printf("Tracer particle velocity output: ON\n");
#else
    printf("Tracer particle velocity output: OFF\n");
#endif
    printf("=========================\n");
    fflush(stdout);
  }
