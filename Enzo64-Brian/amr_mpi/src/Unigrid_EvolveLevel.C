...
//      fprintf(stderr, "%"ISYM": Called UpdatePP\n", MyProcessorNumber);
	...
	for (int movieLevel = 0; movieLevel < MAX_DEPTH_OF_HIERARCHY; movieLevel++) {
	  if (LevelArray[movieLevel] == NULL) break;
	  delete [] Grids;
	  NumberOfGrids = GenerateGridArray(LevelArray, movieLevel, &Grids);
	  if (WriteStreamData(Grids, NumberOfGrids, MetaData,
			      LevelCycleCount[movieLevel], TRUE) == FAIL) {
