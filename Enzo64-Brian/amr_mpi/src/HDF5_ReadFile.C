...
// This routine reads only data from Inits: inits_type is 32- or 64-bit
  ...
  for ( dim=0; dim < Rank; dim++)
  {
    slab_offset[Rank-dim] = BufferOffset[dim];   // this is the correct version
    //slab_offset[Rank-dim] = BufferOffset[Rank-dim-1];   // this is the broken version
    slab_stride[Rank-dim] = 1;
    slab_count[Rank-dim] = EndIndex[dim]-StartIndex[dim]+1;  // this is the correct version
    //slab_count[Rank-dim] = EndIndex[Rank-dim-1]-StartIndex[Rank-dim-1]+1;  // this is the broken version
  ...
  for ( dim=0; dim < Slab_Rank; dim++)
  {
    if (io_log) fprintf(stderr, "%"ISYM" : File %s\n", MyProcessorNumber, name);
    if (io_log) fprintf(stderr, "%"ISYM" : Slab [%"ISYM"] Offset %"ISYM", Count %"ISYM", Stride %"ISYM", Block %"ISYM"\n",
            MyProcessorNumber, dim, (int) slab_offset[dim], (int) slab_count[dim],
                 (int) slab_stride[dim], (int) slab_block[dim]);
  }
  if (io_log) fprintf(stderr, "%"ISYM" : Xfer_size %"ISYM"\n", MyProcessorNumber, (int) xfer_size);
