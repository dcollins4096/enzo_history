...
int grid::ComputeCoolingTime(float *cooling_time)
{
  ...
  // place holder for metallicity field
  ...
  float TemperatureUnits = 1, DensityUnits = 1, LengthUnits = 1,
    VelocityUnits = 1, MassUnits = 1, TimeUnits = 1, aUnits = 1;
  ...
  // If Cloudy Cooling is on, get cooling time from there.

  if (CloudyCooling) {

    // Use hybrid Cloudy/MultiSpecies method if Multispecies is on.
    ...
    // Otherwise, use Cloudy-only cooling.

    else {

      if (solve_CloudyCooling_time(density,totalenergy,gasenergy,
				   velocity1,velocity2,velocity3,
				   metallicity,
				   cooling_time,
				   GridDimension,GridRank,dtFixed,
				   afloat,TemperatureUnits,aUnits,
				   DensityUnits)==FAIL) {
	fprintf(stderr,"Error in solve_CloudyCooling_time.\n");
	return FAIL;
      }
  ...
  // Else, get it the old-fashioned way.
    ...
    if (MultiSpecies)
      ...
      if(GadgetCoolingTime(density,totalenergy,gasenergy,velocity1,
			   velocity2,velocity3,
			   cooling_time,
