...
/* float circular_velocity = sqrt(BigGee*MassCGS/(TestRadius*LengthUnits)) / VelocityUnits; */
  ...
  float circular_velocity = sqrt(BigGee*MassTotal*MassUnits/(TestRadius*LengthUnits)) / VelocityUnits;
  float CentralVelocity = circular_velocity  * TestMass / MassTotal ;
  float TestVelocity = circular_velocity  * CentralMass / MassTotal ;
/* JRT 09/13/06  replace following line by */
