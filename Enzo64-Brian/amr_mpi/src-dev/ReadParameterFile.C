...
int ReadParameterFile(FILE *fptr, TopGridData &MetaData, float *Initialdt)
{
  ...
  while (fgets(line, MAX_LINE_LENGTH, fptr) != NULL) {
    ...
    ret += sscanf(line, "CloudyCooling = %"ISYM, &CloudyCooling);
    ret += sscanf(line, "CloudyCoolingGridRank = %"ISYM, 
		  &CloudyCoolingData.CloudyCoolingGridRank);
    if (sscanf(line, "CloudyCoolingGridRunFile = %s", dummy) == 1) {
      CloudyCoolingData.CloudyCoolingGridRunFile = dummy;
    }
    ret += sscanf(line, "IncludeCloudyHeating = %"ISYM,
		  &CloudyCoolingData.IncludeCloudyHeating);
    ret += sscanf(line, "CMBTemperatureFloor = %"ISYM, &CMBTemperatureFloor);
    ...
    ret += sscanf(line, "MinimumOverDensityForRefinement  = %"FSYM" %"FSYM" %"FSYM" %"FSYM" %"FSYM" %"FSYM" %"FSYM,
    ...
    ret += sscanf(line, "MinimumMassForRefinement  = %"FSYM" %"FSYM" %"FSYM" %"FSYM" %"FSYM" %"FSYM" %"FSYM,
    ...
    ret += sscanf(line, "MinimumMassForRefinementLevelExponent = %"FSYM" %"FSYM" %"FSYM" %"FSYM" %"FSYM" %"FSYM" %"FSYM,
  ...
  if(GadgetEquilibriumCooling == TRUE){
    ...
    fprintf(stderr, "          RadiationFieldType = MultiSpecies = 0 and\n");
    fprintf(stderr, "          RadiativeCooling = 1.\n");

    RadiationFieldType = MultiSpecies = 0;
    RadiativeCooling = 1;
  ...
  /* If set, initialize CloudyCooling stuff. */

  if (CloudyCooling > 0) {
    RadiationFieldType = 0;
    RadiativeCooling = 1;
    if ((MultiSpecies) && (MultiSpecies != 2)) {
      fprintf(stderr, "With CloudyCooling on, MultiSpecies must be either 0 or 2.\n");
      return FAIL;
    }
  ...
  if (MultiSpecies == 0 && RadiativeCooling > 0 && GadgetEquilibriumCooling == 0 && CloudyCooling == 0)
...
// 2 November 2006:  The override has been turned OFF by BWO, as per
// email conversation w/Alexei.  

/*
  if (MetaData.PPMDiffusionParameter != 0 && ProblemType != 60) {//AK
