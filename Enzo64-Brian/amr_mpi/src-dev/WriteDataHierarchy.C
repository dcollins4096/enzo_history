		       ...
		       char* base_name, int &GridID, FLOAT WriteTime)
{
  ...
  if (NextGridThisLevelID != 0) {
    GridID++;
    if (WriteDataHierarchy(fptr, MetaData, Grid->NextGridThisLevel, base_name, GridID,
			   WriteTime) == FAIL) {
  ...
  if (NextGridNextLevelID != 0) {
    GridID++;
    if (WriteDataHierarchy(fptr, MetaData, Grid->NextGridNextLevel, base_name, GridID,
			   WriteTime) == FAIL) {
      fprintf(stderr, "Error in WriteDataHierarchy(1).\n");
      return FAIL;
    }
