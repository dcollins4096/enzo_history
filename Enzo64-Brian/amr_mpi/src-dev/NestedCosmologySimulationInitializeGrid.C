...
int HDF5_ReadIntFile(char *name, int Rank, int Dims[], int StartIndex[],
...
//    if (io_log) fprintf(log_fptr, "NOP %"ISYM", RNK %"ISYM, NOP, RNK);
...
//      if (io_log) fprintf(log_fptr, "NCPU = 64 ==> coerced to 4**3\n");
      }
...
//{
  ...
  char *PPos = new char[strlen(pid)+lex+4+1];
  strcpy(PPos, "PPos");
  strcat(PPos, pid);
  if (lex > 0)
    strcat(PPos, Extension);
 
  char *PVel = new char[strlen(pid)+lex+4+1];
  strcpy(PVel, "PVel");
  strcat(PVel, pid);
  if (lex > 0)
    strcat(PVel, Extension);
 
  char *PPro = new char[strlen(pid)+lex+4+1];
  strcpy(PPro, "PPro");
  strcat(PPro, pid);
  if (lex > 0)
    strcat(PPro, Extension);
 
  char *PMass = new char[strlen(pid)+lex+5+1];
  strcpy(PMass, "PMass");
  strcat(PMass, pid);
  if (lex > 0)
    strcat(PMass, Extension);
 
  char *PType = new char[strlen(pid)+lex+5+1];
  strcpy(PType, "PType");
  strcat(PType, pid);
  if (lex > 0)
...
//      fcol(ParticleMass, NumberOfParticles, 10, log_fptr);
      ...
      if (CosmologySimulationParticleTypeName != NULL) {
        if (HDF5_ReadIntFile(CosmologySimulationParticleTypeName, 1, Dim, Start, End,
...
//      icol(ParticleType, NumberOfParticles, 10, log_fptr);
	...
	UniformParticleMass *= 8;
      ...
      if (debug) printf("NestedCosmologySimulationInitializeGrid: particle mass = %"GSYM"\n",
			UniformParticleMass);
