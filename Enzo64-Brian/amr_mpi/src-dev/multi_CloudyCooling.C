/********************************************************************************
 *                          multi_CloudyCooling.C                               *
 *                      Britton Smith - March, 2006                             *
 *                                                                              *
 * Adapted from solve_CloudyCooling.C, this routine takes cooling values from   *
 * tables produced by Cloudy, but also keeps track of all H,He species in order *
 * to calculate H2 cooling on the fly.                                          *
 *                                                                              *
 *      October, 2006: added feature to prevent gas from cooling below CMB      *
 *                     temperature by calculating cooling as C(T)-C(T_CMB).     *
 *                                                                              *
 *         June, 2006: added metallicity field to handle higher dimension of    *
 *                     interpolation.                                           *
 *******************************************************************************/
			...
			float aUnits,float DensityUnits,float TimeUnits)
{
  ...
  for (i = 0;i < size;i++) {
    ...
    logNumberDensity = log10((HI[i]+HII[i]+HM[i]+(H2I[i]+H2II[i])/2.0)*DensityUnits/mu/MH);
...
#define DONT_USE_GALLI_PALLA1999
	...
	edotMetals = -(pow(10,cooling)) * density[i];
	...
	if (CMBTemperatureFloor) {
	  ...
	  if (cooling > coolingAtTCMB) {
	    edotMetals += (pow(10,coolingAtTCMB)) * density[i];
	  }
	  else {
	    edotMetals = 0.0;
	  }
	...
	if (CloudyCoolingData.IncludeCloudyHeating) {
	  ...
	  edotMetals += ((pow(10,heating)) * density[i]);
	}
