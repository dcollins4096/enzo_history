...
//  CAUTION - are the coordinates reversed?
  ...
  if (NumberOfParticles > 0) {
	...
	fprintf(stderr,"ABBA:  %s\n",procfilename);
      ...
      fprintf(stderr,"log file name is: %s and my procesor is %d and io_log is %d\n", logname, 
	      MyProcessorNumber, io_log);
      fprintf(stderr,"procfilename is %s and ParticleFileName is %s and proc is %d\n",procfilename, name, MyProcessorNumber);
...
// Read ParticleType if present
  ...
  if (MyProcessorNumber == ProcessorNumber) 
    fprintf(stderr,"ACCA:  I'm done with grid %s on processor %d\n",procfilename,MyProcessorNumber);
