...
int grid::StarParticleHandler(int level)
{
  ...
  for (field = 0; field < NumberOfBaryonFields; field++)
    if (FieldType[field] >= ElectronDensity && FieldType[field] < FieldUndefined )
  ...
  if (StarParticleCreation > 0) {
    ...
    if (debug)
       fprintf(stderr,"StarParticle: Before Call: New StarParticles = %"ISYM"\n", NumberOfNewParticles);

 
#ifdef STAR1
    if (StarParticleCreation == 1)  //---- ORIGINAL STAR FORMATION ALGORITHM
    ...
    if (StarParticleCreation == 2)  //---- MODIFIED SF ALGORITHM ("STANDARD VERSION")
    ...
    if (StarParticleCreation == 3)  //---- UNIGRID ALGORITHM (NO JEANS MASS)
    ...
    if (StarParticleCreation == 4)  //---- KRAVTSOV SF ALGORITHM
    ...
    if (StarParticleCreation == 5){  //---- SPRINGEL & HERNQUIST SF ALGORITHM
	...
	coolingrate[coolindex] = GadgetCoolingRate( log10(temperature[coolindex]), 
						    cgsdensity,
						    electronguessptr, zred);
      }						  
...
#ifdef STAR1
  if (StarParticleFeedback == 1) {  //---- THIS IS THE ORIGINAL ENZO STAR FORMATION ALG.
  ...
  if (StarParticleFeedback == 2) {  //---- THIS IS THE MODIFIED STAR FORMATION ALGORITHM
  ...
  if (StarParticleFeedback == 3) {  //---- UNIGRID (NON-JEANS MASS) VERSION
  ...
  if (StarParticleFeedback == 4) {  //---- KRAVTSOV STAR FORMATION ALGORITHM
  ...
  if (StarParticleFeedback == 5) {  //---- SPRINGEL & HERNQUIST ALGORITHM
  ...
  BaryonField[NumberOfBaryonFields] = NULL;
    if (debug) fprintf(stderr,"StarParticle: end\n");
