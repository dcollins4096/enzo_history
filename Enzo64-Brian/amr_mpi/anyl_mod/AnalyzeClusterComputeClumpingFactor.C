					...
					float SphereRadius, char *Name)
{
  ...
  if (debug)
    printf("TotalNumberOfParticles = %"ISYM" ParticleCount = %"ISYM"\n",
  ...
  FLOAT a = 1, dadt, CurrentRedshift = 0, OmegaCurvatureNow = 0.0, Esquared,
        MeanToCriticalDensityRatio = 1;
  ...
  for (j = 0; j < NUMBER_OF_BINS; j++)
	...
	TotalMass += Mass;
      ...
      fprintf(fptr, "%"ISYM" %"GSYM" %"GSYM" %"GSYM" %"GSYM" %"GSYM" %"GSYM" %"GSYM" %"ISYM"\n", j, BinRadius[j],
  ...
  fprintf(fptr, "# mean clumping within r500 = %"GSYM"\n", MeanClump);
  fclose(fptr);
