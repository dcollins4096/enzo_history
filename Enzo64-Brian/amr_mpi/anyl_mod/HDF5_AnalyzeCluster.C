...
float angmom_mass_histogram_x[200],
  angmom_mass_histogram_y[200],
  angmom_mass_histogram_z[200],
  angmom_mass_histogram_ttl[200],
  angmom_histogram_values[200],
  angmom_histogram_log_min,
  angmom_histogram_log_max,
  angmom_histogram_max_radius,
  angmom_histogram_dl;

int num_angmom_histogram_bins; 
...
main(int argc, char *argv[])
{
  ...
  char Name[MAX_LINE_LENGTH], DiskImageName[MAX_LINE_LENGTH], DiskDataName[MAX_LINE_LENGTH];
  ...
  /* angular momentum declarations */
  num_angmom_histogram_bins = 200;
  angmom_histogram_log_min = 20.0;
  angmom_histogram_log_max = 26.0;
  angmom_histogram_max_radius = 10.0;  // in proper parsecs
  angmom_histogram_dl = (angmom_histogram_log_max-angmom_histogram_log_min)/
    ((float) num_angmom_histogram_bins);

  for(i=0; i<num_angmom_histogram_bins; i++){

    // set values in center of bin (for printout)
    angmom_histogram_values[i] = angmom_histogram_log_min +
      ( ((float) i) + 0.5)*angmom_histogram_dl;

    angmom_mass_histogram_x[i]=0.0;
    angmom_mass_histogram_y[i]=0.0;
    angmom_mass_histogram_z[i]=0.0;
    angmom_mass_histogram_ttl[i]=0.0;

  }

  /*
  printf("in HDF5_AnalyzeCluster.C:\n");
  printf("num_angmom_histogram_bins = %d\n",num_angmom_histogram_bins);
  printf("angmom_histogram_log_min = %e\n",angmom_histogram_log_min);
  printf("angmom_histogram_log_max = %e\n",angmom_histogram_log_max);
  printf("angmom_histogram_dl = %e\n",angmom_histogram_dl);
  printf("angmom_histogram_max_radius = %e\n\n",angmom_histogram_max_radius);
  fflush(stdout);
  */
  ...
  FLOAT a=1, dadt, CurrentRedshift = 0.0, OmegaCurvatureNow = 0.0, Esquared;
  ...
  for (int center = 0; center < NumberOfCenters; center++) {
    ...
    if (NumberOfCenters == 1)
      ...
      sprintf(Name, "%s%.3"ISYM, "AnalyzeCluster", center);
    ...
    if (rvir > OuterEdge || rvir == 0) {
      printf("warning: rvir (%"GSYM") > OuterEdge (%"GSYM")\n", rvir, OuterEdge);
      rvir = OuterEdge;
    }
    j1 = j;
    ...
    if (debug) printf("->finding sigma profiles\n");
    for (level = 0; level < MAX_DEPTH_OF_HIERARCHY; level++) {
      LevelHierarchyEntry *Temp = LevelArray[level];
      while (Temp != NULL) {
	if (Temp->GridData->AddToRadialProfileStats(Center, OuterEdge, MeanVelocity,
					       NumberOfPoints, ProfileRadius,
					       ProfileValue, ProfileWeight,
					       ProfileName, &parameters) 
	    == FAIL) {
	  fprintf(stderr, "Error in grid->AddToRadialProfileStats.\n");
	  exit(EXIT_FAILURE);
	}
	Temp = Temp->NextGridThisLevel;
      }
	...
	DiskVector[dim] = RvirValue[0][80+dim]/length;
      ...
      // new disk vector  -- calculate by avg. ang. mom vectory in inner parsec or so!

      float xweight=0.0, yweight=0.0, zweight=0.0;

      for(j=0; j<NumberOfPoints; j++)
	if( ProfileRadius[j]*BoxSize*1.0e+6 < 1.0){
	  xweight += ProfileValue[j][17];
	  yweight += ProfileValue[j][18];
	  zweight += ProfileValue[j][19];
	}

      // THIS IS ADDED BY BWO ON 21 APRIL 2006
      // FOR THE 12.5 KEV WDM model radial profiles!  (woot)
      /*
      fprintf(stderr,"*********** FIXED DISK VECTOR JACKASS **********\n");
      xweight = -53.0;
      yweight = -6.0;
      zweight = 4.0;
      */

      length = sqrt( xweight*xweight + yweight*yweight + zweight*zweight );

      DiskVector[0] = xweight / length;
      DiskVector[1] = yweight / length;
      DiskVector[2] = zweight / length;
	...
	LevelHierarchyEntry *Temp = LevelArray[level];
	while (Temp != NULL) {
	  Temp->GridData->AddToDiskProfileAngMomTrans(Center, rvir, MeanVelocity,
				       NumberOfPoints,
				       ProfileRadius,
				       ProfileValue, ProfileWeight,
				       ProfileName, &parameters,
...
// Size of each image is Dimm
      ...
      if (io_log) printf("H5Screate file_dsp_id %"ISYM"\n",file_dsp_id);
      ...
      if (io_log) printf("H5Fcreate File id: %"ISYM"\n",file_id);
      ...
      if (io_log) printf("H5Dcreate Dataset id: %"ISYM"\n",dset_id);
...
// Part are contiguous in the file, so stride = 1.
        ...
        if (io_log) printf("H5Sselect file slab: %"ISYM"\n",h5_status);
        assert( h5_status != h5_error );
 
        h5_status = H5Dwrite(dset_id, HDF5_R4, mem_dsp_id, file_dsp_id,  H5P_DEFAULT, (VOIDP) float_temp);
 
        if (io_log) printf("H5Dwrite %"ISYM"\n",h5_status);
        assert( h5_status != h5_error );
      ...
      if (io_log) printf("H5Sclose %"ISYM"\n",h5_status);
      assert( h5_status != h5_error );
 
      h5_status = H5Dclose(dset_id);
 
      if (io_log) printf("H5Dclose %"ISYM"\n",h5_status);
      assert( h5_status != h5_error );
 
      h5_status = H5Fclose(file_id);
 
      if (io_log) printf("H5Fclose %"ISYM"\n",h5_status);
      assert( h5_status != h5_error );
...
#define NUMBER_OF_FILES 6
    ...
    for (profile = 120; profile < 130; profile++)
      ProfileFile[profile] = 2;
     for (profile = 130; profile < 135; profile++)
      ProfileFile[profile] = 0;
    ProfileFile[135] = 2;
    for (profile = 136; profile < 139; profile++)
      ProfileFile[profile] = 0;


    /* Output global values. */
 
    fprintf(fptrs[0], "# Center             = %.10"GSYM" %.10"GSYM" %.10"GSYM"\n", Center[0],
	  Center[1], Center[2]);
    fprintf(fptrs[0], "# MaxBaryonValue     = %"GSYM"\n", MaxDensity);
    fprintf(fptrs[0], "# MeanVelocity (gas) = %"GSYM" %"GSYM" %"GSYM" (km/s)\n",
	  MeanVelocity[0][0], MeanVelocity[1][0], MeanVelocity[2][0]);
    fprintf(fptrs[0], "# MeanVelocity (dm ) = %"GSYM" %"GSYM" %"GSYM" (km/s)\n",
	  MeanVelocity[0][1], MeanVelocity[1][1], MeanVelocity[2][1]);
    fprintf(fptrs[0], "# MeanVelocity (tot) = %"GSYM" %"GSYM" %"GSYM" (km/s)\n",
	  MeanVelocity[0][2], MeanVelocity[1][2], MeanVelocity[2][2]);
    fprintf(fptrs[0], "#  (Within r = %"GSYM" Mpc)\n#\n",
	    MeanVelocityOuterEdge*BoxSize);
    fprintf(fptrs[0], "# L (gas)            = %"GSYM" %"GSYM" %"GSYM" (Mpc km/s)\n",
	    RvirValue[0][17], RvirValue[0][18], RvirValue[0][19]);
    fprintf(fptrs[0], "# L (dm)             = %"GSYM" %"GSYM" %"GSYM" (Mpc km/s)\n",
	    RvirValue[0][35], RvirValue[0][36], RvirValue[0][37]);
    fprintf(fptrs[0], "# InnerEdge = %"GSYM" Mpc  OuterEdge = %"GSYM" Mpc\n\n",
	  InnerEdge*BoxSize, OuterEdge*BoxSize);
 
    /* Output general information about simulation. */
 
    fprintf(fptrs[0], "# OmegaMatterNow     = %"GSYM"\n", OmegaMatterNow);
    fprintf(fptrs[0], "# OmegaLambdaNow     = %"GSYM"\n", OmegaLambdaNow);
    fprintf(fptrs[0], "# HubbleConstantNow  = %"GSYM"\n", HubbleConstantNow);
    fprintf(fptrs[0], "# CurrentRedshift    = %"GSYM"\n", CurrentRedshift);
    fprintf(fptrs[0], "# AMR file name      = %s\n\n", argv[1]);
 
    /* Output rvir information. */
 
    fprintf(fptrs[0], "# rvir               = %"GSYM" (Mpc)\n", rvir*BoxSize);
    fprintf(fptrs[0], "# mvir               = %"GSYM" (M_solar)\n", mvir);
    fprintf(fptrs[0], "# VirialDensity      = %"GSYM" (to critical density)\n",
	    parameters.virial_dens);
    fprintf(fptrs[0], "# mvir (gas,dm,star) = %"GSYM" %"GSYM" %"GSYM" (M_solar)\n",
	    mvir_gas, mvir - mvir_gas - mvir_star, mvir_star);
    fprintf(fptrs[0], "# spin (gas, dm)     = (%"GSYM", %"GSYM")\n", spin_gas, spin_dm);
    fprintf(fptrs[0], "# Tvir predicted (K) = %"GSYM"\n", Tvir);
    if (parameters.XrayTableFileName != NULL) {
      fprintf(fptrs[0], "# XrayLowerCutoffkeV = %"GSYM"\n",
	      parameters.XrayLowerCutoffkeV);
      fprintf(fptrs[0], "# XrayUpperCutoffkeV = %"GSYM"\n",
	      parameters.XrayUpperCutoffkeV);
      fprintf(fptrs[0], "# XrayTableFileName  = %s\n",
	      parameters.XrayTableFileName);
    }
    fprintf(fptrs[0], "# ColdTempCutoff (K) = %"GSYM"\n\n",
	    parameters.ColdTemperatureCutoff);
	...
	fprintf(fptrs[ProfileFile[profile]], "%12.5"GSYM" ", RvirValue[0][profile]);
    ...
    for (i = 0; i < NUMBER_OF_FILES; i++)
      if (fptrs[i] != NULL) {
	fprintf(fptrs[i], "#\n# COLUMN   DESCRIPTION\n");
	fprintf(fptrs[i], "#   %3"ISYM"    %s\n", 1, "bin central radius (Mpc)");
	fprintf(fptrs[i], "#   %3"ISYM"    %s\n", 2, "bin outer radius (Mpc)");
      }
 
    for (profile = 0; profile < MAX_PROFILES; profile++)
      if (ProfileName[profile] != NULL) {
	fprintf(fptrs[ProfileFile[profile]], "#   %3"ISYM"    %s\n",
    ...
    for (j = 0; j < NumberOfPoints; j++) {
      ...
      for (i = 0; i < NUMBER_OF_FILES; i++)
	if (fptrs[i] != NULL)
	  fprintf(fptrs[i], "%12.5"GSYM" %12.5"GSYM" ",
		  rmid, ProfileRadius[j+1]*BoxSize);
 
      for (profile = 0; profile < MAX_PROFILES; profile++)
	if (ProfileName[profile] != NULL)
	  fprintf(fptrs[ProfileFile[profile]], "%12.5"GSYM" ",
		  (ProfileValue[j][profile] == 0) ?
		  tiny_number : ProfileValue[j][profile]);
 
      fprintf(fptrs[0], "%12.5"GSYM" %12.5"GSYM" %12.5"GSYM" ", ProfileWeight[j][0],
	      (POW(ProfileRadius[j+1], 3) - POW(ProfileRadius[j], 3) ) *
  ...
  angmomfileptr=fopen("angmom_histogram.txt","w");

  for(i=0; i < num_angmom_histogram_bins ; i++)
    fprintf(angmomfileptr,"%e   %e   %e   %e   %e\n",
	    pow(10.0,angmom_histogram_values[i]),
	    angmom_mass_histogram_x[i],
	    angmom_mass_histogram_y[i],
	    angmom_mass_histogram_z[i],
	    angmom_mass_histogram_ttl[i]);

  fclose(angmomfileptr);
