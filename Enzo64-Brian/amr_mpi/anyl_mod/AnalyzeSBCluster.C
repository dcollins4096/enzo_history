...
//      POW(BoxSize, 3) * 4.0*pi/3.0;
  ...
  fprintf(fptr, "# MaxBaryonPosition  = %"GSYM" %"GSYM" %"GSYM"\n", MaxPosition[0],
	  MaxPosition[1], MaxPosition[2]);
  fprintf(fptr, "# MaxBaryonValue     = %"GSYM"\n", MaxDensity);
  fprintf(fptr, "# MeanVelocity (gas) = %"GSYM" %"GSYM" %"GSYM" (km/s)\n",
	  MeanVelocity[0][0], MeanVelocity[1][0], MeanVelocity[2][0]);
  fprintf(fptr, "# MeanVelocity (dm ) = %"GSYM" %"GSYM" %"GSYM" (km/s)\n",
	  MeanVelocity[0][1], MeanVelocity[1][1], MeanVelocity[2][1]);
  fprintf(fptr, "# MeanVelocity (tot) = %"GSYM" %"GSYM" %"GSYM" (km/s)\n",
	  MeanVelocity[0][2], MeanVelocity[1][2], MeanVelocity[2][2]);
  fprintf(fptr, "#  (Within r = %"GSYM" Mpc\n#\n", MeanVelocityOuterEdge*BoxSize);
  fprintf(fptr, "# InnerEdge = %"GSYM" Mpc  OuterEdge = %"GSYM" Mpc\n",
	  InnerEdge*BoxSize, OuterEdge*BoxSize);
 
  /* Output r200 information. */
 
  fprintf(fptr, "# r200              = %"GSYM" (Mpc)\n", r200*BoxSize);
  for (profile = 0; profile < MAX_PROFILES; profile++)
    if (ProfileName[profile] != NULL)
      fprintf(fptr, "%12.5"GSYM" ", R200Value[0][profile]);
  fprintf(fptr, "\n");
  fprintf(fptr, "#\n");
  ...
  for (j = 0; j < MAX_BINS; j++) {
    ...
    fprintf(fptr, "%12.5"GSYM" ",0.5*(ProfileRadius[j]+ProfileRadius[j+1])*BoxSize);
 
    for (profile = 0; profile < MAX_PROFILES; profile++)
      if (ProfileName[profile] != NULL)
	if (ProfileValue[j][profile] == 0)
	  fprintf(fptr, "%12.5"GSYM" ", tiny_number);
	else
	  fprintf(fptr, "%12.5"GSYM" ", ProfileValue[j][profile]);
 
    fprintf(fptr, "%12.5"GSYM" %12.5"GSYM" %12.5"GSYM" ", ProfileWeight[j][0],
	    (POW(ProfileRadius[j+1], 3) - POW(ProfileRadius[j], 3) ) *
	    POW(BoxSize, 3) * 4.0*pi/3.0,
	    ProfileWeight[j][10]);
    fprintf(fptr, "\n");
