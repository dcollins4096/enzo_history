...
int FindField(int field, int farray[], int numfields);
 ...
 extern float angmom_mass_histogram_x[200],
...
extern int num_angmom_histogram_bins;
			     ...
			     AnalyzeClusterParameters *parameters)
{
  ...
  float radius2, delx, dely, delz, InertiaWeight, radial_vel, AngWeight,
        velx, vely, velz, vel[MAX_DIMENSION], mu, number_density, meanvelgas;
  const double SolarMass = 1.989e33, Mpc = 3.086e24, kboltz = 1.38e-16,
               mh = 1.67e-24;

   float angmom_x,angmom_y,angmom_z,angmom_ttl,radius_in_parsecs;

  int angmom_x_bin, angmom_y_bin, angmom_z_bin, angmom_ttl_bin;
  ...
  for (k = GridStartIndex[2]; k <= GridEndIndex[2]; k++) {
	...
	// get radius in proper parsecs
	radius_in_parsecs = sqrt(radius2)*LengthUnits/3.0857e+18;
	...
	if (radius2 <= SphereRadius*SphereRadius)
	  for (n = 0; n < NumberOfBins; n++)
		...
		ProfileName[1] = "v_rms_gas_3d (km/s)";
	      ...
	      /* 130)  mean gas velocity (overall) - use results from previous
		 bit for my convenience */
	      meanvelgas = 0.0;
	      for (dim = 0; dim < GridRank; dim++)
		meanvelgas += vel[dim] * vel[dim];
	      meanvelgas = sqrt(meanvelgas);
	      
	      ProfileValue[n][130] += gas_mass * meanvelgas;
	      ProfileWeight[n][130] += gas_mass;
	      if (ProfileName[130] == NULL) 
		ProfileName[130] = "v_mean_gas_3d (km/s)";
		...
		POW(BaryonField[DensNum][index], Gamma-1.0) * gas_mass;
		...
		ProfileWeight[n][m] += gas_mass;
	      ...
	      // Brian's stuff
	      if(radius_in_parsecs <= angmom_histogram_max_radius){

		angmom_x = fabs( (vel[1]*delz - vel[2]*dely));
		angmom_y = fabs( (-vel[0]*delz + vel[2]*delx));
		angmom_z = fabs( (vel[0]*dely - vel[1]*delx));

		angmom_ttl = sqrt(angmom_x*angmom_x + angmom_y*angmom_y + angmom_z*angmom_z);

		angmom_x_bin = ((int) ((log10(angmom_x*LengthUnits*VelocityUnits)-angmom_histogram_log_min)/angmom_histogram_dl));
		if(angmom_x_bin < 0) angmom_x_bin = 0;
		if(angmom_x_bin >= num_angmom_histogram_bins) angmom_x_bin = num_angmom_histogram_bins-1;
		angmom_mass_histogram_x[angmom_x_bin] += gas_mass;

		/*
		  printf("angmom_x_test:  %e %e %e %e %e %e %d\n",
		  angmom_x,
		  angmom_x*LengthUnits*VelocityUnits,
		  log10(angmom_x*LengthUnits*VelocityUnits),
		  log10(angmom_x*LengthUnits*VelocityUnits)-angmom_histogram_log_min,
		  (log10(angmom_x*LengthUnits*VelocityUnits)-angmom_histogram_log_min)/angmom_histogram_dl,
		  ((log10(angmom_x*LengthUnits*VelocityUnits)-angmom_histogram_log_min)/angmom_histogram_dl),
		  ((int) ((log10(angmom_x*LengthUnits*VelocityUnits)-angmom_histogram_log_min)/angmom_histogram_dl)));
		*/

		angmom_y_bin = ((int) ((log10(angmom_y*LengthUnits*VelocityUnits)-angmom_histogram_log_min)/angmom_histogram_dl));
		if(angmom_y_bin < 0) angmom_y_bin = 0;
		if(angmom_y_bin >= num_angmom_histogram_bins) angmom_y_bin = num_angmom_histogram_bins-1;
		angmom_mass_histogram_y[angmom_y_bin] += gas_mass;

		angmom_z_bin = ((int) ((log10(angmom_z*LengthUnits*VelocityUnits)-angmom_histogram_log_min)/angmom_histogram_dl));
		if(angmom_z_bin < 0) angmom_z_bin = 0;
		if(angmom_z_bin >= num_angmom_histogram_bins) angmom_z_bin = num_angmom_histogram_bins-1;
		angmom_mass_histogram_z[angmom_z_bin] += gas_mass;

		angmom_ttl_bin = ((int) ((log10(angmom_ttl*LengthUnits*VelocityUnits)-angmom_histogram_log_min)/angmom_histogram_dl));
		if(angmom_ttl_bin < 0) angmom_ttl_bin = 0;
		if(angmom_ttl_bin >= num_angmom_histogram_bins) angmom_ttl_bin = num_angmom_histogram_bins-1;
		angmom_mass_histogram_ttl[angmom_ttl_bin] += gas_mass;

		/*
		  printf("angmom vals:  %e %e %e %e %e %d %d %d %d %d\n",angmom_x,angmom_y,angmom_z,
		  angmom_ttl,LengthUnits*VelocityUnits,
		  angmom_x_bin,angmom_y_bin,angmom_z_bin,angmom_ttl_bin,sizeof(float));
		*/

	      }  // end of  if(radius_in_parsecs <= angmom_histogram_max_radius){
		...
		if (!ProfileName[83]) ProfileName[83] = "Metallicity fraction";
	      }
		...
		(mh*mu);
	      ProfileValue[n][21] += sqrt(radius2*LengthUnits*LengthUnits/
					  sound_speed2) * gas_mass;
		...
		*double(VelocityUnits*VelocityUnits);
	      ...
	      ProfileValue[n][136] += float(thispressure) * gas_mass;
	      ProfileWeight[n][136] += gas_mass;
	      if (ProfileName[136] == NULL) ProfileName[136] = "Pressure";
