...
main(int argc, char *argv[])
{
  ...
  if (debug)
    printf("Analysis region: Left = %"GSYM" %"GSYM" %"GSYM"   Right = %"GSYM" %"GSYM" %"GSYM"\n",
  ...
  for (i = 0; i < NumberOfTempBins; i++)
    printf("%"GSYM"-%"GSYM" K:  %"GSYM"   %"GSYM"  %"GSYM"\n", TempBinEdge[i], TempBinEdge[i+1],
  ...
  printf("Total mass, Volume = %"GSYM"  %"GSYM"\n", TempBinInfo[0], TempBinInfo[1]);
  ...
  for (i = 0; i < NumberOfTempBins; i++)
    fprintf(fptr, "     %"GSYM"-%"GSYM"    ", TempBinEdge[i], TempBinEdge[i+1]);
  fprintf(fptr, "\n");
 
  for (j = 0; j < NumberOfDensityBins; j++) {
    fprintf(fptr, "%"GSYM"  ", POW(10.0, DensBinStart +
      (float(j)+0.5)*(DensBinEnd - DensBinStart)/float(NumberOfDensityBins)));
    for (i = 0; i < NumberOfTempBins; i++)
      fprintf(fptr, "  %"GSYM" %"GSYM"  ", TempDensInfo[i*NumberOfDensityBins+j],
