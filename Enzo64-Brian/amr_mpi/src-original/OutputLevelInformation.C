			   ...
			   LevelHierarchyEntry *LevelArray[])
{
  ...
  if (isRoot) fprintf(fptr, "%"GOUTSYM"  %"ISYM" %"ISYM" %"GSYM" %"GSYM" %"GSYM" %"GSYM"   ",
  ...
  for (level = 0; level <= MaximumRefinementLevel; level++) {
    if (isRoot) fprintf(fptr, "%"ISYM" %"GSYM" %"GSYM" %"GSYM" %"GSYM" %"ISYM"   ",
	    Grids[level], float(Memory[level])/1.049e6,
