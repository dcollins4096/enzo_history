...
/  date:       November, 1994
/  modified:   Many times by AK, DC, RH, JB...
/
/  PURPOSE:
/
************************************************************************/
...
//
// MHD things.  Not actually used right now.  Needs more later.
//
  ...
  int MHDAdd[3][3]; //How much to add to Baryon Dimensions.

  int MHDBlastInitializeGrid(float Density0, float Density1,
			     float Energy0,  float Energy1,
			     float Velocity0[], float Velocity1[],
			     float B0[], float B1[], float Radius, float MHDBlastCenter[],
			     float PerturbAmplitude, int PerturbMethod, int InitStyle);
