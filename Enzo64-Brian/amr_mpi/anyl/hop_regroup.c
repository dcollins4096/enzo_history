...
void parsecommandline(float dens_outer, Controls *c)
{
	...
	if (!strcmp(argv[narg],"-root")) {
	    ...
	    narg++; if (narg>=argc) usage();
	    sscanf(argv[narg],"%"FSYM,&(c->densthresh)); narg++;
	    if (c->qdenscut!=0) c->qdenscut=1;
	}
	else if (!strcmp(argv[narg],"-dsaddle")) {
	    narg++; if (narg>=argc) usage();
	    sscanf(argv[narg],"%"FSYM,&(c->saddle_thresh)); narg++;
	}
	else if (!strcmp(argv[narg],"-dpeak")) {
	    narg++; if (narg>=argc) usage();
	    sscanf(argv[narg],"%"FSYM,&(c->peak_thresh)); narg++;
	}
	else if (!strcmp(argv[narg],"-mingroup")) {
	    narg++; if (narg>=argc) usage();
	    sscanf(argv[narg],"%"ISYM,&(c->mingroupsize)); narg++;
	}
...
/* I think this will work even if saddledensthresh<densthresh */
{
    ...
    if (sscanf(line, "%"ISYM, &ngroups)!=1) myerror("Error in gmerge format.");
