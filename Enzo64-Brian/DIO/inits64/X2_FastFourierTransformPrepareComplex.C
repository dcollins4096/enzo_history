...
//->    FLOAT *tempbuffer = new FLOAT[big];
    fprintf(stderr, "DON'T Allocate %lld FLOATs for FFT temp buffer\n", big);
    fprintf(stderr, "Call Prefort\n");

    mm_p1 = (char *) sbrk(0);
    mm_delta = mm_p1-mm_p0;
    fprintf(stderr, "Mem before Prefort %Ld\n", mm_delta);
    ...
    FORTRAN_NAME(prefort)(buffer, buffer, &Dim[0], &Dim[1], &Dim[2],
...
//->    delete tempbuffer;
    ...
    mm_p1 = (char *) sbrk(0);
    mm_delta = mm_p1-mm_p0;
    fprintf(stderr, "Mem after Prefort %Ld\n", mm_delta);

    fprintf(stderr, "Exit Prefort\n");
