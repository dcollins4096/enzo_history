		  ...
		  int Refinement, int StartIndex[3], int Species)
{
  ...
  fprintf(stderr, "GenerateField: Allocate 2x %"ISYM" bytes for Temp\n", 8 * size );
 
    mm_p1 = (char *) sbrk(0);
    mm_delta = mm_p1-mm_p0;
    fprintf(stderr, "Mem before new Temp %Ld\n", mm_delta);
 
  if ((Temp = new FLOAT[2*size]) == NULL) {
    ...
    mm_p1 = (char *) sbrk(0);
    mm_delta = mm_p1-mm_p0;
    fprintf(stderr, "Mem after new Temp %Ld\n", mm_delta);
  ...
  for (i = 0; i < size; i++)
  {
    ...
    mm_p1 = (char *) sbrk(0);
    mm_delta = mm_p1-mm_p0;
    fprintf(stderr, "Mem before MFK %Ld\n", mm_delta);
  ...
  if (Rank == 3)
    ...
    mm_p1 = (char *) sbrk(0);
    mm_delta = mm_p1-mm_p0;
    fprintf(stderr, "Mem after MFK %Ld\n", mm_delta);

    mm_p1 = (char *) sbrk(0);
    mm_delta = mm_p1-mm_p0;
    fprintf(stderr, "Mem before FFT %Ld\n", mm_delta);
  ...
  if (debug) printf("transform complete\n");
    ...
    mm_p1 = (char *) sbrk(0);
    mm_delta = mm_p1-mm_p0;
    fprintf(stderr, "Mem after FFT %Ld\n", mm_delta);
  ...
  if (Extract) {
	...
	MoveBy[dim] = NewCenter[dim] - (MaxDims[dim]/2 - 1);
      if (llabs(MoveBy[dim]) % Refinement != 0) {
  ...
  fprintf(stderr, "GenerateField: De-Allocate Temp\n");
 
    mm_p1 = (char *) sbrk(0);
    mm_delta = mm_p1-mm_p0;
    fprintf(stderr, "Mem before delete Temp %Ld\n", mm_delta);
 
  delete [] Temp;
 
    mm_p1 = (char *) sbrk(0);
    mm_delta = mm_p1-mm_p0;
    fprintf(stderr, "Mem after delete Temp %Ld\n", mm_delta);
