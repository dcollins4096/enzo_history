...
extern "C" void read_random_number_table_(int *nx, int *slice, double tab[])
#endif
#if defined(SP2) || defined(HP)
extern "C" void read_random_number_table(int *nx, int *slice, double tab[])
#endif
{
...
// Data in memory is considered 1D, stride 1, with zero offset
  ...
  h5_status = H5Dread(dset_id, mem_type_id, mem_dsp_id, file_dsp_id,  H5P_DEFAULT, tab);
    assert( h5_status != h5_error );
