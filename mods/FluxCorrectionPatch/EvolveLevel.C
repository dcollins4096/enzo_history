...
#ifdef JB_OPT_FLUXES_FIX
    //dcc Now setting SUBlingList = NULL here; crashes with FC off OW.
    /*
    SUBlingList = new LevelHierarchyEntry*[NumberOfGrids];
    fprintf(stderr, "dccSUB level %d \n", level);
    for(int list=0;list<NumberOfGrids;list++)
      SUBlingList[list] = NULL;
    */

    fprintf(stderr,"dccSUB level %d FluxCorrection %d\n", level, FluxCorrection);
    if( FluxCorrection){
    ...
    for(grid=0;grid<NumberOfGrids;grid++){
      NextMonkey = SUBlingList[grid];
      while (NextMonkey != NULL) {
	fprintf(stderr,"SGcheckEL%d: SUBling[%d]->Grid pointer %p\n",
		MyProcessorNumber, grid, NextMonkey->GridData);
