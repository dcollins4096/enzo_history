...
)
{
  ...
  for (grid = 0; grid < NumberOfGrids; grid++) {
    ...
    NextEntry = SUBlingList[grid];
    
    while (NextEntry != NULL && FluxCorrection) {
      /* make sure this isn't a "proper" subgrid */
      if( !(NextEntry->GridHierarchyEntry->ParentGrid == Grids[grid]) ){
	/* Project subgrid's refined fluxes to the level of this grid. */
  ...
  for (grid = 0; grid < NumberOfGrids; grid++) {
    ...
    /* Loop over subgrids for this grid using the hierarchy. */
