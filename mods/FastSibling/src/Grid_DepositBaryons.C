...
int grid::DepositBaryons(grid *TargetGrid, FLOAT DepositTime)
{
  ...
  for (dim = 0; dim < GridRank; dim++) {
    ...
    if (TargetGrid != this && GridStart[dim] < 0) {
      fprintf(stderr, "GridStart[%d] = %g < 0.\n", dim,GridStart[dim]);
      return FAIL;
    }

    if (RegionDim[dim] < 2) {
      fprintf(stderr, "RegionDim[%d] = %d < 2\n", dim, RegionDim[dim]);
      return FAIL;
    }

  }

  /* Allocate the local density patch to be deposited unless this is the
     receive-phase in which case the patch was allocated on this processor
     already in the post-receive phase. */
  ...
  if (ProcessorNumber != TargetGrid->ProcessorNumber) {
    ...
    if (MyProcessorNumber == ProcessorNumber)
      CommunicationBufferedSend(dens_field, size, MPI_FLOAT, 
	...
	CommunicationDirection == COMMUNICATION_SEND_RECEIVE)
      MPI_Recv(dens_field, size, MPI_FLOAT, ProcessorNumber, 
	       MPI_SENDREGION_TAG, MPI_COMM_WORLD, &status);

    if (MyProcessorNumber == TargetGrid->ProcessorNumber &&
	CommunicationDirection == COMMUNICATION_POST_RECEIVE) {
      MPI_Irecv(dens_field, size, MPI_FLOAT, ProcessorNumber, 
