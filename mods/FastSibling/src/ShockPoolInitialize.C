		       ...
		       TopGridData &MetaData)
{
  ...
  while (fgets(line, MAX_LINE_LENGTH, fptr) != NULL) {
    ...
    ret += sscanf(line, "ShockPoolAngle = %f", &ShockPoolAngle);
    ret += sscanf(line, "ShockPoolMachNumber = %f", &ShockPoolMachNumber);

    ret += sscanf(line, "ShockPoolDensity = %f", &ShockPoolDensity);
    ret += sscanf(line, "ShockPoolPressure = %f", &ShockPoolPressure);
    ret += sscanf(line, "ShockPoolVelocity1 = %f", &ShockPoolVelocity[0]);
    ret += sscanf(line, "ShockPoolVelocity2 = %f", &ShockPoolVelocity[1]);
    ret += sscanf(line, "ShockPoolVelocity3 = %f", &ShockPoolVelocity[2]);

    ret += sscanf(line, "ShockPoolSubgridLeft = %"FSYM, &ShockPoolSubgridLeft);
    ret += sscanf(line, "ShockPoolSubgridRight = %"FSYM, &ShockPoolSubgridRight);
