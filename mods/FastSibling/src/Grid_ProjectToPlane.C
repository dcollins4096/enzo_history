			 ...
			 float XrayUpperCutoffkeV, char *XrayTableFileName)
{
  ...
  if (debug) printf("ProjectToGrid: start = %d/%d (%5.3f)  stop = %d/%d (%5.3f)  GridLeft/Right = %5.3"FSYM"/%5.3"FSYM"\n",
...
// printf("%d PFStart = %d %d  PFStop = %d %d, a/bdim = %d/%d level = %g\n", 
// MyProcessorNumber, PFStart[adim], PFStart[bdim], PFStop[adim],PFStop[bdim],
...
#define GENERATE_ENTROPY_FIELDS

#ifdef GENERATE_ENTROPY_FIELDS

  if (NumberOfBaryonFields > 0) {

    float *temp_field = new float[size];

    /* 10) Compute surface density of low-entropy material. */

    float entropy;
    for (i = 0; i < size; i++) {
      temp_field[i] = 0;
      if (BaryonField[DensNum][i] > 0) {
	entropy = (temperature[i]/1.16e7)/
	       pow(BaryonField[DensNum][i]*double(DensityUnits)*0.88/mh,0.666);
	if (entropy < 150.0)
	  temp_field[i] = BaryonField[DensNum][i];;
      }
    }

    if (NumberOfBaryonFields > 0)
      FORTRAN_NAME(projplane)(
                          temp_field, NULL, BaryonField[NumberOfBaryonFields], 
			         &One, &ProjectionSmooth,
			  GridDimension, GridDimension+1,
                              GridDimension+2, CellWidth[0],
                          ProjectedField[9], ProjectedFieldDims+adim, 
                              ProjectedFieldDims+bdim, &ProjectedFieldCellSize,
                          &ProjectionDimension, &One, &DensityConversion,
                          GridLeftEdge, GridFarLeftEdge, GridRightEdge,
                              ProjectedFieldLeftEdge, ProjectedFieldRightEdge,
    ...
    /* 11) Compute mass-weighted entropy. */

    for (i = 0; i < size; i++) {
      temp_field[i] = 0;
      if (BaryonField[DensNum][i] > 0)
	temp_field[i] = (temperature[i]/1.16e7)/
	       pow(BaryonField[DensNum][i]*double(DensityUnits)*0.88/mh,0.666)
               *BaryonField[DensNum][i]*DensityConversion;
    }

    ConversionFactor = 1;
    if (NumberOfBaryonFields > 0)
      FORTRAN_NAME(projplane)(temp_field, NULL,
                          BaryonField[NumberOfBaryonFields], 
			         &One, &ProjectionSmooth,
			  GridDimension, GridDimension+1,
                              GridDimension+2, CellWidth[0],
                          ProjectedField[10], ProjectedFieldDims+adim, 
                              ProjectedFieldDims+bdim, &ProjectedFieldCellSize,
                          &ProjectionDimension, &One, &ConversionFactor,
                          GridLeftEdge, GridFarLeftEdge, GridRightEdge,
                              ProjectedFieldLeftEdge, ProjectedFieldRightEdge,
    ...
    /* 12) Compute mass-weighted inverse cooling-time. */

    if (RadiativeCooling) {
      this->ComputeCoolingTime(temp_field);
      for (i = 0; i < size; i++) {
	temp_field[i] = fabs(temp_field[i]*TimeUnits);
	temp_field[i] = BaryonField[DensNum][i]*DensityConversion/
                        max(min(temp_field[i], 1.0e35), 1.0e-35);
	}
	...
	FORTRAN_NAME(projplane)(temp_field, NULL,
			  ...
			  GridDimension, GridDimension+1,
                              GridDimension+2, CellWidth[0],
                          ProjectedField[11], ProjectedFieldDims+adim, 
                              ProjectedFieldDims+bdim, &ProjectedFieldCellSize,
                          &ProjectionDimension, &One, &DensityConversion,
                          GridLeftEdge, GridFarLeftEdge, GridRightEdge,
                              ProjectedFieldLeftEdge, ProjectedFieldRightEdge,
    ...
    } // end: if (RadiativeCooling)

    delete temp_field;

  } // end: if (NumberOfBaryonFields > 0)

#endif /* GENERATE_ENTROPY_FIELDS */

  /* 13/14/15/16/17/18) OVII/OVIII/SiXIII/SiXIV/FeXXV/FeXXVI column density. */

  if (NumberOfBaryonFields > 0 && NumberOfProjectedFields > 12) {
    float *elemental_density = new float[size];
    ConversionFactor = LengthUnits*CellLength;

    for (i = 0; i < NumberOfProjectedFields-12; i++) {
//      if (this->ComputeElementalDensity(temperature, elemental_density, i)
	...
	return FAIL;
      ...
      FORTRAN_NAME(projplane)(elemental_density, NULL, 
			  ...
			  GridDimension, GridDimension+1,
                              GridDimension+2, CellWidth[0],
                          ProjectedField[12+i], ProjectedFieldDims+adim, 
                              ProjectedFieldDims+bdim, &ProjectedFieldCellSize,
                          &ProjectionDimension, &One, &ConversionFactor,
                          GridLeftEdge, GridFarLeftEdge, GridRightEdge,
                              ProjectedFieldLeftEdge, ProjectedFieldRightEdge,
