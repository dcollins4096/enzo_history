...
int grid::CopyZonesFromGrid(grid *OtherGrid, FLOAT EdgeOffset[MAX_DIMENSION])
{
  ...
  /*      fprintf(stderr, "P(%d) [%g %g %g  %g %g %g] from %d phase = %d   %g %g %g\n",
	  MyProcessorNumber,
	  GridLeftEdge[0], GridLeftEdge[1], GridLeftEdge[2],
	  OtherGrid->GridLeftEdge[0], 
	  OtherGrid->GridLeftEdge[1], 
	  OtherGrid->GridLeftEdge[2], 
	  OtherGrid->ProcessorNumber,
	  CommunicationDirection, EdgeOffset[0], EdgeOffset[1], EdgeOffset[2]); */
