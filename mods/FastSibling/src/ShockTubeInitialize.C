...
int ShockTubeInitialize(FILE *fptr, FILE *Outfptr, HierarchyEntry &TopGrid)
{
  ...
  while (fgets(line, MAX_LINE_LENGTH, fptr) != NULL) {
    ...
    ret += sscanf(line, "ShockTubeBoundary = %"FSYM, &ShockTubeBoundary);

    ret += sscanf(line, "ShockTubeLeftDensity = %"FSYM, &ShockTubeDensity[0]);
    ret += sscanf(line, "ShockTubeLeftPressure = %"FSYM, &ShockTubePressure[0]);
    ret += sscanf(line, "ShockTubeLeftVelocity = %"FSYM, &ShockTubeVelocity[0]);

    ret += sscanf(line, "ShockTubeRightDensity = %"FSYM, &ShockTubeDensity[1]);
    ret += sscanf(line, "ShockTubeRightPressure = %"FSYM, &ShockTubePressure[1]);
    ret += sscanf(line, "ShockTubeRightVelocity = %"FSYM, &ShockTubeVelocity[1]);
  ...
  if (MyProcessorNumber == ROOT_PROCESSOR) {
    fprintf(Outfptr, "ShockTubeDirection     = %d\n", ShockTubeDirection);
    fprintf(Outfptr, "ShockTubeBoundary      = %f\n\n", ShockTubeBoundary);

    fprintf(Outfptr, "ShockTubeLeftDensity   = %f\n", ShockTubeDensity[0]);
    fprintf(Outfptr, "ShockTubeLeftPressure  = %f\n", ShockTubePressure[0]);
    fprintf(Outfptr, "ShockTubeLeftVelocity  = %f\n\n", ShockTubeVelocity[0]);

    fprintf(Outfptr, "ShockTubeRightDensity  = %f\n", ShockTubeDensity[1]);
    fprintf(Outfptr, "ShockTubeRightPressure = %f\n", ShockTubePressure[1]);
    fprintf(Outfptr, "ShockTubeRightVelocity = %f\n\n", ShockTubeVelocity[1]);
  }
