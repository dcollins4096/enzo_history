...
int ReadAllData(char *filename, HierarchyEntry *TopGrid, TopGridData &tgd,
		    ExternalBoundary *Exterior, int MaximumLevel = -1);
...
int main(int argc, char *argv[])
{
  ...
  float Initialdt              = 0;
  //  printf("sizeof(grid,fluxes,float) = %d %d %d\n", sizeof(grid), sizeof(fluxes), sizeof(float));
  ...
  if (project || OutputAsParticleDataFlag || extract || 
    ...
    if (ReadAllData(ParameterFile, &TopGrid, MetaData, &Exterior, RegionLevel) == FAIL) {
  ...
  fprintf(filePtr, "total interval time used between RebuildHierarchy is %ef (%d times).\n", timer[0], counter[0]);
