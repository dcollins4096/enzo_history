			  ...
			  TopGridData &MetaData, ExternalBoundary &Exterior)
{
  ...
  while (fgets(line, MAX_LINE_LENGTH, fptr) != NULL) {
    ...
    ret += sscanf(line, "ShockInABoxBoundary = %"FSYM, &ShockInABoxBoundary);

    ret += sscanf(line, "ShockInABoxLeftDensity = %f", 
		  &ShockInABoxDensity[0]);
    ret += sscanf(line, "ShockInABoxLeftPressure = %f", 
		  &ShockInABoxPressure[0]);
    ret += sscanf(line, "ShockInABoxLeftVelocity = %f", 
		  &ShockInABoxVelocity[0]);

    ret += sscanf(line, "ShockInABoxRightDensity = %f", 
		  &ShockInABoxDensity[1]);
    ret += sscanf(line, "ShockInABoxRightPressure = %f", 
		  &ShockInABoxPressure[1]);
    ret += sscanf(line, "ShockInABoxRightVelocity = %f", 
		  &ShockInABoxVelocity[1]);

    ret += sscanf(line, "ShockInABoxSubgridLeft = %"FSYM, 
		  &ShockInABoxSubgridLeft);
    ret += sscanf(line, "ShockInABoxSubgridRight = %"FSYM,
  ...
  if (MyProcessorNumber == ROOT_PROCESSOR) {
    fprintf(Outfptr, "ShockInABoxDirection     = %d\n", ShockInABoxDirection);
    fprintf(Outfptr, "ShockInABoxBoundary      = %"GOUTSYM"\n\n", 
	    ShockInABoxBoundary);

    fprintf(Outfptr, "ShockInABoxLeftDensity   = %f\n", ShockInABoxDensity[0]);
    fprintf(Outfptr, "ShockInABoxLeftPressure  = %f\n", 
	    ShockInABoxPressure[0]);
    fprintf(Outfptr, "ShockInABoxLeftVelocity  = %f\n\n", 
	    ShockInABoxVelocity[0]);

    fprintf(Outfptr, "ShockInABoxRightDensity  = %f\n", ShockInABoxDensity[1]);
    fprintf(Outfptr, "ShockInABoxRightPressure = %f\n", 
	    ShockInABoxPressure[1]);
    fprintf(Outfptr, "ShockInABoxRightVelocity = %f\n\n", 
	    ShockInABoxVelocity[1]);
  }
