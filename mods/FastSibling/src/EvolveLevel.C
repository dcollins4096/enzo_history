		...
		int level, float dtLevelAbove, ExternalBoundary *Exterior)
{
  ...
  PerformanceTimers[23] += ReturnWallTime() - time1;
  ...
  while (dtThisLevelSoFar < dtLevelAbove) {
	...
	if (Grids[grid1]->GridData->ReturnProcessorNumber()==MyProcessorNumber){
	  ...
	  fprintf(stdout, "level = %d %d %d  Vel %f %f %f  DMVel %f %f %f  CofM %"FSYM" %"FSYM" %"FSYM"  DMCofM %f %f %f\n",
...
*/
    ...
    // if (NumberOfSubgrids[grid1] > 1 && MetaData->OutputFirstTimeAtLevel > 0)
