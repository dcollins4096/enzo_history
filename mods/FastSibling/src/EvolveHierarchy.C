...
*/
  ...
  /* Open the OutputLevelInformation and PerformanceInformation files. */

  FILE *LevelInfofptr = NULL, *PerformanceInfofptr = NULL;
  if (MyProcessorNumber == ROOT_PROCESSOR) {
    LevelInfofptr = fopen("OutputLevelInformation.out", "w");
    fclose(LevelInfofptr);
    PerformanceInfofptr = fopen("PerformanceInformation.out", "w");
    fclose(PerformanceInfofptr);
  }
  ...
  while (!Stop) {
    ...
    /* Output level and performance information to log file. */

    if (MyProcessorNumber == ROOT_PROCESSOR) {
      LevelInfofptr = fopen("OutputLevelInformation.out", "a");
      PerformanceInfofptr = fopen("PerformanceInformation.out", "a");
      OutputLevelInformation(LevelInfofptr, MetaData, LevelArray);
      fclose(LevelInfofptr);
    }
    OutputPerformanceInformation(PerformanceInfofptr, MetaData);
    if (MyProcessorNumber == ROOT_PROCESSOR)
      fclose(PerformanceInfofptr);
	...
	if (CosmologyOutputRedshift[i] != -1)
	  dt = min(1.01*(CosmologyOutputRedshiftTime[i]-MetaData.Time), dt);
    for (i = 0; i < MAX_TIME_ACTIONS; i++)
      if (TimeActionTime[i] > 0 && TimeActionType[i] > 0)
	dt = min(1.01*(TimeActionTime[i] - MetaData.Time), dt);
	...
	MetaData.TimeLastDataDump += MetaData.dtDataDump;
     dt = min(1.01*(MetaData.TimeLastDataDump + MetaData.dtDataDump -
		       MetaData.Time), dt);
    }
