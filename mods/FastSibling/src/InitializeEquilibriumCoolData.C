...
int InitializeEquilibriumCoolData(FLOAT Time)
{
  ...
  while (fgets(line, MAX_LINE_LENGTH, fptr) != NULL) 
    if (line[0] != '#')
      if (sscanf(line, "%f %f %f", &temp, &dummy, 
  ...
  if (debug) {
    printf("InitializeEqCoolData: NumberOfTemperatureBins = %d\n", 
		    CoolData.NumberOfTemperatureBins);
    printf("InitializeEqCoolData: TemperatureStart = %g\n", 
		    CoolData.TemperatureStart);
    printf("InitializeEqCoolData: TemperatureEnd = %g\n", 
