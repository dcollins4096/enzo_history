				  ...
				  float *elemental_density, int Type)
{
  ...
  default:
    fprintf(stderr, "Unrecognized element type: %d\n", Type);
    return FAIL;
  }
  ...
  if (Type < 6) {
	...
	for (i = 0; i < TableSize[0]; i++, n++)
	  if (fscanf(fptr, "%f %f %f %f", 
	    ...
	    fprintf(stderr, "Error reading table %d %d\n", i, j);
	    return FAIL;
	  }
      fclose(fptr);
      TableRead = TRUE;
...
#endif /* USE_DENSIT_DEPENDENT_METALLICITY */
