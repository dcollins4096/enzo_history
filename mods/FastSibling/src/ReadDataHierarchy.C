                      ...
                      int CurrentLevel, int MaximumReadLevel)
{
  ...
  int ReadGridData = TRUE;
  if (CurrentLevel > MaximumReadLevel && MaximumReadLevel >= 0)
     ReadGridData = FALSE;
  if (Grid->GridData->ReadGrid(fptr, ReadGridData) == FAIL) {
  ...
  if (NextGridThisLevelID != 0) {
    ...
    if (ReadDataHierarchy(fptr, Grid->NextGridThisLevel, NextGridThisLevelID,
			  ParentGrid, CurrentLevel, MaximumReadLevel) == FAIL) {
  ...
  if (NextGridNextLevelID != 0) {
    ...
    if (ReadDataHierarchy(fptr, Grid->NextGridNextLevel, NextGridNextLevelID,
			  Grid, CurrentLevel+1, MaximumReadLevel)
	== FAIL) {
