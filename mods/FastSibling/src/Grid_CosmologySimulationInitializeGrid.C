...
int ReadHDFFile(char *name, int Rank, int Dims[], int StartIndex[],
...
/*          * pow(BaryonField[0][i]/CosmologySimulationOmegaBaryonNow,Gamma-1) 
  ...
  if (CosmologySimulationParticlePositionName != NULL && ReadData) {
	...
	int *IntTempField = (int *) TempField;
	if (sizeof(int) != sizeof(float)) {
	  fprintf(stderr, "read assumption failure.\n");
	  return FAIL;
	}
	//if (READFILE(CosmologySimulationParticleTypeName, 1, Dim, Start, End,
	//		     Zero, TempField) == FAIL) {
	DFSDgetdims(CosmologySimulationParticleTypeName, &TempInt,
		    TempIntArray, MAX_DIMENSION);
	if (DFSDgetdata(CosmologySimulationParticleTypeName, TempInt,
			TempIntArray, (VOIDP) IntTempField) == HDF_FAIL) {
	  fprintf(stderr, "Error reading particle masses.\n");
	  return FAIL;
	}
	for (n = 0, i = 0; i < TotalParticleCount; i++)
	  if (Mask[i])
	    ParticleType[n++] = IntTempField[i];
      }
	...
	/* Here we use the float routine to read int data -- should be fixed.*/

	if (sizeof(float) != sizeof(int)) {
	  fprintf(stderr, "int-float size mismatch\n");
	  return FAIL;
	}
	if (READFILE(CosmologySimulationParticleTypeName, 1, Dim, Start, End,
		     Zero, (float *) ParticleType, &tempbuffer) == FAIL) {
	  fprintf(stderr, "Error reading particle masses.\n");
	  return FAIL;
	}
      }
