		       ...
		       TopGridData &MetaData)
{
  ...
  while (fgets(line, MAX_LINE_LENGTH, fptr) != NULL) {
    ...
    ret += sscanf(line, "TestGravityDensity = %f", &TestGravityDensity);
    ret += sscanf(line, "TestGravitySubgridLeft = %"FSYM, 
		  &TestGravitySubgridLeft);
    ret += sscanf(line, "TestGravitySubgridRight = %"FSYM, 
		  &TestGravitySubgridRight);
    ret += sscanf(line, "TestGravityNumberOfParticles = %d",
		  &TestGravityNumberOfParticles);
    ret += sscanf(line, "TestGravityUseBaryons = %d",
  ...
  if (MyProcessorNumber == ROOT_PROCESSOR) {
    fprintf(Outfptr, "TestGravityDensity           = %f\n", 
	    TestGravityDensity);
    fprintf(Outfptr, "TestGravitySubgridLeft       = %"GOUTSYM"\n", 
	    TestGravitySubgridLeft);
    fprintf(Outfptr, "TestGravitySubgridRight      = %"GOUTSYM"\n", 
	    TestGravitySubgridRight);
    fprintf(Outfptr, "TestGravityNumberOfParticles = %d\n",
	    TestGravityNumberOfParticles);
    fprintf(Outfptr, "TestGravityUseBaryons        = %d\n\n",
	    TestGravityUseBaryons);
  }
