...
static int Work1dLastSize = 0;
static float *Work1d = NULL;
				...
				int Dimension[], int dir, int type)
{
  ...
  if (Rank == 1) {
    if (type == REAL_TO_COMPLEX) {
      if (Work1dLastSize != Dimension[0]) {
	delete [] Work1d;
	Work1d = (float *) RTOC(1di)(Dimension[0], Work1d);
	Work1dLastSize = Dimension[0];
      }
      if (dir == FFT_FORWARD)
	RTOC(1du)(dir*-1, Dimension[0], buffer, 1, Work1d);
      else {
	RTOC_INVERSE(1du)(dir*-1, Dimension[0], buffer, 1, Work1d);
	RTOC_SCALE(1d)(Dimension[0], 1.0/float(Dimension[0]), buffer, 1);
      }
    } else {
      if (Work1dLastSize != Dimension[0]*2) {
	delete [] Work1d;
	Work1d = (float *) CTOC(1di)(Dimension[0], NULL);
	Work1dLastSize = Dimension[0]*2;
      }
      CTOC(1d)(dir*-1, Dimension[0], CMPLX_CAST buffer, 1, CMPLX_CAST Work1d);
      if (dir == FFT_INVERSE)
