...
extern "C" void FORTRAN_NAME(star_maker3)(int *nx, int *ny, int *nz,
             ...
             int *nmax, FLOAT *xstart, FLOAT *ystart, FLOAT *zstart, 
     		 int *ibuff, 
	     hydro_method *imethod, float *massthresh, int *level, int *np,
...
int grid::StarParticleHandler(int level)
{
  ...
  float *temperature = NULL, *dmfield = NULL, *cooling_time = NULL;
  ...
  for (field = 0; field < NumberOfBaryonFields; field++)
    if (FieldType[field] >= ElectronDensity && FieldType[field] <= Metallicity)
  ...
  if (StarParticleCreation > 0) {
    ...
    if (StarParticleCreation == 3)
      MaximumNumberOfNewParticles = 10; /* not many expected */
    ...
    /* This is the main star formation scheme. */
    ...
    /* This is a modified star formation scheme. */
    ...
    if (StarParticleCreation == 3 && level == MaximumRefinementLevel) {
	...
	if (CellFlaggingMethod[method] == 2)
      ...
      FORTRAN_NAME(star_maker3)(
       ...
       &MaximumNumberOfNewParticles, CellLeftEdge[0], CellLeftEdge[1],
          CellLeftEdge[2], &GhostZones, 
       &HydroMethod, &SinkParticleMassThreshold, &level, &NumberOfNewParticles,
       tg->ParticlePosition[0], tg->ParticlePosition[1], 
          tg->ParticlePosition[2], 
       tg->ParticleVelocity[0], tg->ParticleVelocity[1], 
       tg->ParticleVelocity[2], tg->ParticleMass, 
       tg->ParticleAttribute[0], tg->ParticleAttribute[1],
       &NumberOfParticles, ParticlePosition[0], ParticlePosition[1], 
          ParticlePosition[2], 
       ParticleVelocity[0], ParticleVelocity[1], 
          ParticleVelocity[2], ParticleMass, ParticleAttribute[0]);

      /* Output sink particle id numbers and masses. */

      static int SinkRoutineTimesCalled = 0;
      if (SinkRoutineTimesCalled++ % 10 == 0)
	for (int n=0; n < NumberOfParticles; n++)
	  if (ParticleAttribute[0][n] > 0)
	    printf("%d %g %g\n", ParticleNumber[n], Time, ParticleMass[n]*pow(CellWidthTemp, 3));
    ...
    if (NumberOfNewParticles > 0) {
      ...
      if (debug) 
	printf("StarParticle: New StarParticles = %d\n", NumberOfNewParticles);
