		     ...
		     LevelHierarchyEntry *LevelArray[], int level)
{
  ...
  double time1, time2 = ReturnWallTime(), time3;
  ...
  timer[20] += (starttime - tmptime)*(starttime - tmptime);
  counter[0] ++;
  ...
  PerformanceTimers[24] += ReturnWallTime() - time1;
  ...
  PerformanceTimers[25] += ReturnWallTime() - time1;
...
//    if (debug) ReportMemoryUsage("Memory usage report: Rebuild 3");
	...
	if (FindSubgrids(GridHierarchyPointer[j], i) == FAIL) {
      ...
      PerformanceTimers[26] += ReturnWallTime() - time1;
	...
	SubgridHierarchyPointer[j]->GridData->DebugCheck("Rebuild child");
      ...
      PerformanceTimers[30] += ReturnWallTime() - time1;
	...
	PerformanceTimers[29] += ReturnWallTime() - time3;
  ...
  timer[21] += (endtime - starttime)*(endtime - starttime);
  counter[1] ++;
#endif /* MPI_INSTRUMENTATION */

  PerformanceTimers[0] += ReturnWallTime() - time2;
