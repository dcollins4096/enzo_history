//  GRID CLASS (FIND AND SAVE PROFILE FOR THE MHD SPHERICAL INFALL TEST)
...
int grid::MHDSphericalInfallGetProfile(int level, int ReportLevel)
{
  ...
  sprintf(ProfileName, "%s.L%1.1d.%4.4d", "SphericalInfallProfile", level, 
  ...
  fprintf(fptr, "# l %d t = %f L = %g %g %g %g d(max) = %g S(min) = %g v_dmax = %g div_v_max = %g  dt/dt_divv = %g\n",
	  level, Time, 
...
//	  MaxVel[0], MaxVel[1], MaxVel[2], MinVel[0], MinVel[1], MinVel[2]);
  ...
  if (ReportLevel > 1) {
    ...
    for (i = Index[0]; i < GridDimension[0]; i++) {
      fprintf(fptr, "%f ", CellLeftEdge[0][i] + 0.5*CellWidth[0][i] - 
