...
// 2.) Probably should have a glass of whiskey ready.
...
AthenaObj::AthenaObj(grid * Grid){
  ...
  for( dim=0; dim< MAX_DIMENSION; dim++){
    ...
    NFaces = 3;//Grid->NFaces; Might get expanded in the future.
    NEdges = 3;//Grid->NEdges; 
    ...
    AthStart[dim] = 1;                        //Grid->GridStartIndex[dim];
    AthEnd[dim] = Grid->GridDimension[dim]-2; //Grid->GridEndIndex[dim];
  }
  ...
  NumberOfFluidQuantities = ( ( MHD_Used == TRUE) ? 7 : 4 );
  NumberOfFluidQuantities += ( ( EquationOfState == 0 ) ? 1 : 0 );
  //Grid->NumberOfFluidQuantities;

  // Copies for maximum confusion.  
  ...
  for(dim=0;dim<GridRank; dim++){
    MagneticSize[dim] = Grid->MagneticSize[dim];
    ElectricSize[dim] = Grid->ElectricSize[dim];
...
if(ComovingCoordinates==1){
 FLOAT a, dadt;
    if (CosmologyComputeExpansionFactor(Grid->Time+(FLOAT)0.5*dtFixed, &a, &dadt)
        == FAIL) {
  ...
  *(MHDBlastNormal[dim]/(MHDBlastNormal[0]*CellWidth[dim][0]) );
  ...
  fprintf(stderr,"AthenaConstructor Setup ERROR: Not enough ghost zones.  To accomidate ShiftedPeriodic, and to avoid\n");
