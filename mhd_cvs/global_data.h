...
EXTERN float LoadBalancingRatio;
EXTERN char * AnalysisParameterFileName;
EXTERN int ForceAnalysisOnly;
#ifdef DCC_EXTERNAL_CYCLE_COUNT
EXTERN int LevelCycleCount[MAX_DEPTH_OF_HIERARCHY];
#endif //!DCC_EXTERNAL_CYCLE_COUNT
EXTERN boundary_type TopLeftBoundary[MAX_DIMENSION];  //Kind of a hack, 
EXTERN boundary_type TopRightBoundary[MAX_DIMENSION]; //but better than really invasive changes.
...
EXTERN int GlobalStatsCycleSkip, GlobalStatsLastCycleDump;
EXTERN float GlobalStatsDT, GlobalStatsLastTimeDump;
