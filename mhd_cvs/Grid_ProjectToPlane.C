			 ...
			 float XrayUpperCutoffkeV, char *XrayTableFileName)
{
  ...
  if (debug) printf("ProjectToGrid: start = %d/%d (%5.3f)  stop = %d/%d (%5.3f)  GridLeft/Right = %5.3"PSYM"/%5.3"PSYM"\n",
...
#ifdef HAOXU
   if(MHD_Used){
     if (MHDCosmologyGetUnits(&DensityUnits, &LengthUnits,
&TemperatureUnits,   &TimeUnits, &VelocityUnits, Time,&BFieldUnits) ==
FAIL) {
  ...
  if (NumberOfBaryonFields > 0) {
...
#ifdef HAOXU  //Project B fields
    if(MHD_Used){
   FORTRAN_NAME(projplane)(CenteredB[0], NULL,
                             BaryonField[NumberOfBaryonFields], &One,
                             &ProjectionSmooth,
                          GridDimension, GridDimension+1,
                              GridDimension+2, CellWidth[0],
                          ProjectedField[NumberOfProjectedFields], ProjectedFieldDims+adim,
                              ProjectedFieldDims+bdim, &ProjectedFieldCellSize,
                          &ProjectionDimension, &One, &BFieldUnits,
   ...
   FORTRAN_NAME(projplane)(CenteredB[1], NULL,
                             BaryonField[NumberOfBaryonFields], &One,
                             &ProjectionSmooth,
                          GridDimension, GridDimension+1,
                              GridDimension+2, CellWidth[0],
                          ProjectedField[NumberOfProjectedFields+1], ProjectedFieldDims+adim,
                              ProjectedFieldDims+bdim, &ProjectedFieldCellSize,
                          &ProjectionDimension, &One, &BFieldUnits,
                          GridLeftEdge, GridFarLeftEdge, GridRightEdge,
                              ProjectedFieldLeftEdge, ProjectedFieldRightEdge,
   ...
   FORTRAN_NAME(projplane)(CenteredB[2], NULL,
                             BaryonField[NumberOfBaryonFields], &One,
                             &ProjectionSmooth,
                          GridDimension, GridDimension+1,
                              GridDimension+2, CellWidth[0],
                          ProjectedField[NumberOfProjectedFields+2], ProjectedFieldDims+adim,
                              ProjectedFieldDims+bdim, &ProjectedFieldCellSize,
                          &ProjectionDimension, &One, &BFieldUnits,
                          GridLeftEdge, GridFarLeftEdge, GridRightEdge,
                              ProjectedFieldLeftEdge, ProjectedFieldRightEdge,
   ...
   FORTRAN_NAME(projplane)(BaryonField[Vel1Num], NULL,
                             BaryonField[NumberOfBaryonFields], &One,
                             &ProjectionSmooth,
                          GridDimension, GridDimension+1,
                              GridDimension+2, CellWidth[0],
                          ProjectedField[NumberOfProjectedFields+3], ProjectedFieldDims+adim,
                              ProjectedFieldDims+bdim, &ProjectedFieldCellSize,
                          &ProjectionDimension, &One, &VelocityUnits,
                          GridLeftEdge, GridFarLeftEdge, GridRightEdge,
                              ProjectedFieldLeftEdge, ProjectedFieldRightEdge,
                          &start, &stop, &LeftCellFraction,&RightCellFraction);

   FORTRAN_NAME(projplane)(BaryonField[Vel2Num], NULL,
                             BaryonField[NumberOfBaryonFields], &One,
                             &ProjectionSmooth,
                          GridDimension, GridDimension+1,
                              GridDimension+2, CellWidth[0],
                          ProjectedField[NumberOfProjectedFields+4], ProjectedFieldDims+adim,
                              ProjectedFieldDims+bdim, &ProjectedFieldCellSize,
                          &ProjectionDimension, &One, &VelocityUnits,
                          GridLeftEdge, GridFarLeftEdge, GridRightEdge,
                              ProjectedFieldLeftEdge, ProjectedFieldRightEdge,
   ...
   FORTRAN_NAME(projplane)(BaryonField[Vel3Num], NULL,
                             BaryonField[NumberOfBaryonFields], &One,
                             &ProjectionSmooth,
                          GridDimension, GridDimension+1,
                              GridDimension+2, CellWidth[0],
                          ProjectedField[NumberOfProjectedFields+5], ProjectedFieldDims+adim,
                              ProjectedFieldDims+bdim, &ProjectedFieldCellSize,
                          &ProjectionDimension, &One, &VelocityUnits,
                          GridLeftEdge, GridFarLeftEdge, GridRightEdge,
                              ProjectedFieldLeftEdge, ProjectedFieldRightEdge,
    ...
    /* 6) Faraday Rotation Measurement */
                                                                                                                                                             
    float *rm = new float[size];
    for (i = 0; i < size; i++)
      rm[i] = BaryonField[DensNum][i]*CenteredB[ProjectionDimension][i];
                                                                                                                                                             
    double  mh = 1.67e-24, me = 9.11e-28,kpc=3.08e21;
                                                                                                                                                             
    ConversionFactor = 812*double(DensityUnits)*0.88/mh*double(BFieldUnits)
                       *double(LengthUnits)*CellLength/kpc*1.0e6;
        
                                                                                                                                                           
    if (NumberOfBaryonFields > 0)
      FORTRAN_NAME(projplane)(rm, NULL, BaryonField[NumberOfBaryonFields],
                              &One, &ProjectionSmooth,
                          GridDimension, GridDimension+1,
                              GridDimension+2, CellWidth[0],
                          ProjectedField[NumberOfProjectedFields+6], ProjectedFieldDims+adim,
                              ProjectedFieldDims+bdim, &ProjectedFieldCellSize,
    ...
    /* 7) Magnetic Energy Density */

    float *be = new float[size];
    for (i = 0; i < size; i++)
      be[i] =
0.5/4.0/3.14159*(CenteredB[0][i]*CenteredB[0][i]+CenteredB[1][i]*CenteredB[1][i]+CenteredB[2][i]*CenteredB[2][i])*(BFieldUnits*BFieldUnits);

      ConversionFactor = 1.0;

    if (NumberOfBaryonFields > 0)
      FORTRAN_NAME(projplane)(be, NULL, BaryonField[NumberOfBaryonFields],
                              &One, &ProjectionSmooth,
                          GridDimension, GridDimension+1,
                              GridDimension+2, CellWidth[0],
                          ProjectedField[NumberOfProjectedFields+7],ProjectedFieldDims+adim,
                              ProjectedFieldDims+bdim,&ProjectedFieldCellSize,
                          &ProjectionDimension, &One, &ConversionFactor,
                          GridLeftEdge, GridFarLeftEdge, GridRightEdge,
                              ProjectedFieldLeftEdge,ProjectedFieldRightEdge,
  ...
  if (GravitatingMassFieldParticles != NULL) {
    ...
    for (dim = 0; dim < MAX_DIMENSION; dim++) {
      Offset[dim] = -nint((GridFarLeftEdge[dim] - 
                          ...
                          CellWidth[dim][0]);
//      printf("Offset[%d] = %d  GFLE = %g  GMFPLE = %g\n", dim, Offset[dim], 
//	     GridFarLeftEdge[dim],GravitatingMassFieldParticlesLeftEdge[dim]);
  ...
  /*  printf("PFStart = %d %d  PFStop = %d %d, a/bdim = %d/%d level = %g\n", 
