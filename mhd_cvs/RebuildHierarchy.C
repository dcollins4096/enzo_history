...
//    if (debug) ReportMemoryUsage("Memory usage report: Rebuild 3");
    ...
    ////wall_time("End RH_SendOld");
    ////wall_time("End RH_SendOld");
    Pout("RH:  InterpolateValues");
    
    //wall_time("Start RH_Interpolate");
    for (j = 0; j < subgrids; j++) {
    ...
    //wall_time("End RH_Interpolate");

      /* Copy from old grids. */
      
    ////wall_time("Start RH_CopyOld");      
    ////wall_time("Start RH_CopyOld");      
      for (j = 0; j < subgrids; j++) {
