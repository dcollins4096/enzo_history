...
/***********************************************************************
/
/  INITIALIZE A MHD COSMOLOGY SIMULATION
/
			       ...
			       HierarchyEntry &TopGrid, TopGridData &MetaData)
{
  ...
  for (gridnum = 0; gridnum < CosmologySimulationNumberOfInitialGrids; 
    ...
    if (GridsList[gridnum]->GridData->MHDCosmologySimulationInitializeGrid(
			     CosmologySimulationOmegaBaryonNow,
			       CosmologySimulationOmegaCDMNow,
			       CosmologySimulationInitialTemperature,
      ...
      fprintf(stderr, "Error in grid->MHDCosmologySimulationInitializeGrid.\n");
      return FAIL;
    }
...
void MHDRecursivelySetParticleCount(HierarchyEntry *GridPoint, int *Count);
				    ...
				    TopGridData &MetaData)
{
  ...
  while (Temp != NULL) {
    ...
    if (Temp->GridData->MHDCosmologySimulationInitializeGrid(
			     CosmologySimulationOmegaBaryonNow,
			       CosmologySimulationOmegaCDMNow,
			       CosmologySimulationInitialTemperature,
			     DensityName, TotalEnergyName,
...
//    printf("NewLocalParticleCount: %d\n", LocalNumberOfParticles );
  ...
  Temp = TopGrid;
  int ParticleCount = 0;
  MHDRecursivelySetParticleCount(Temp, &ParticleCount);
  if (debug)
...
void MHDRecursivelySetParticleCount(HierarchyEntry *GridPoint, int *Count)
{
  ...
  if (GridPoint->NextGridThisLevel != NULL)
    MHDRecursivelySetParticleCount(GridPoint->NextGridThisLevel, Count);

  if (GridPoint->NextGridNextLevel != NULL)
    MHDRecursivelySetParticleCount(GridPoint->NextGridNextLevel, Count);
