...
/* read until out of lines */
  ...
  while (fgets(line, MAX_LINE_LENGTH, fptr) != NULL) {
    ...
    ret += sscanf(line, "LoadBalancing          = %d", &LoadBalancing);
    ret += sscanf(line, "LoadBalancingRatio     = %"FSYM, &LoadBalancingRatio);
...
#endif
#ifdef ATHENA
    ret += sscanf(line, "MaximumSubgridSize          = %d",&MaximumSubgridSize);
    ret += sscanf(line, "MHD_Equation                = %d",&MHD_Equation);
    ret += sscanf(line, "GlobalStatsCycleSkip        = %d",&GlobalStatsCycleSkip);
    ret += sscanf(line, "GlobalStatsLastCycleDump    = %d",&GlobalStatsLastCycleDump);
    ret += sscanf(line, "GlobalStatsDT               = %"FSYM,&GlobalStatsDT);
    ret += sscanf(line, "GlobalStatsLastTimeDump     = %"FSYM,&GlobalStatsLastTimeDump);
   ...
   ret += sscanf(line,"WriteBiermannTerm     = %d",&WriteBiermannTerm);
    ...
    ret += sscanf(line, "ExternalBoundaryIO = %d", &ExternalBoundaryIO);

    ret += sscanf(line, "ExternalBoundaryTypeIO = %d", &ExternalBoundaryTypeIO);

    ret += sscanf(line, "ExternalBoundaryValueIO = %d", &ExternalBoundaryValueIO);
    ...
    /* Analysis stuff */
    if( sscanf(line, "AnalysisParameterFileName = %s",dummy ) == 1){
      AnalysisParameterFileName = dummy;
    }
...
//  The following is an over-ride to force PPMDiffusion OFF
  ...
  //AMR with MHD needs the periodicity only relatively deep in the call stack.
  //Making a global value avoids changing 12 funciton calls to propogate 2 bytes of information.
  for(dim=0;dim<MAX_DIMENSION;dim++){
    TopLeftBoundary[dim] =  MetaData.LeftFaceBoundaryCondition[dim];
    TopRightBoundary[dim] = MetaData.RightFaceBoundaryCondition[dim];

  }

  ReadAnalysisParameterFile(AnalysisParameterFileName, fptr, MetaData);
