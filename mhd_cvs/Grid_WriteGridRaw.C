...
int grid::WriteGridRaw(FILE *fptr, char *base_name, int grid_id)
{
  ...
  char *name = new char[strlen(base_name)+strlen(id)+5];
  ...
  if (NumberOfBaryonFields > 0) {
	...
	for (j = GridStartIndex[1]; j <= GridEndIndex[1]; j++)
      ...
      if (fwrite((void*) temp, sizeof(float32), size, Rawfptr) != size) {
  ...
  if (NumberOfParticles > 0) {
	...
	temp[i] = float32(ParticlePosition[dim][i]);
      ...
      if (fwrite((void*) temp, sizeof(float32), NumberOfParticles, 
    ...
    if (fwrite((void*) tempint, sizeof(int32), NumberOfParticles, 
	       Rawfptr) != NumberOfParticles) {
      perror("error in fwrite");
      return FAIL;
    }
