			      ...
			      fluxes *SubgridFluxes[], int level, int grid)
{
  ...
  if (ProcessorNumber != MyProcessorNumber)
    return SUCCESS;
  wall_time("Start SolveHydro");

  //dcc

  fprintf(stderr,"=== GPFS SolveHydroEquations CycleNumber = %d Level = %d proc= %d g = %d dtFixed = %15.12f ",
  CycleNumber, level, MyProcessorNumber, grid,dtFixed);

  fprintf(stderr," Left = %4.1f %4.1f %4.1f ===\n",64*GridLeftEdge[0], 64*GridLeftEdge[1], 64*GridLeftEdge[2]);

  char basename[20];  
  int MyGridNumber = 1; //can be set to processor, level, grid.  whatever.
  if( WriteInThisF(30) == TRUE) {
    sprintf(basename, "data30%04d%d%d.grid",CycleNumber, level, MyProcessorNumber);
  ...
  if( WriteInThisF(39) == TRUE) {
    sprintf(basename, "data39%04d%d%d.grid",CycleNumber, level, MyProcessorNumber);
