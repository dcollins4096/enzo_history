		       ...
		       TopGridData &MetaData)
{
  ...
  for (lev = 0; lev < MaximumRefinementLevel; lev++) {
    ...
    if (debug)
      printf("Implosion:: Level[%d]: NumberOfSubgridZones[0] = %d\n", lev+1, 
	...
	RightEdge[dim]   = ImplosionSubgridRight;
      ...
      if (Subgrid[lev]->GridData->InitializeUniformGrid
	  (ImplosionDensity,      ImplosionTotalEnergy,
	   ImplosionTotalEnergy,  ImplosionVelocity) == FAIL) {
  ...
  if (MyProcessorNumber == ROOT_PROCESSOR) {
    fprintf(Outfptr, "ImplosionDensity         = %f\n"  , ImplosionDensity);
    fprintf(Outfptr, "ImplosionPressure        = %f\n"  , ImplosionPressure);
    fprintf(Outfptr, "ImplosionDiamondDensity  = %f\n", 
	    ImplosionDiamondDensity);
    fprintf(Outfptr, "ImplosionDiamondPressure = %f\n", 
	    ImplosionDiamondPressure);
  }
