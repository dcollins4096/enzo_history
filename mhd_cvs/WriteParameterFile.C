...
int WriteParameterFile(FILE *fptr, TopGridData &MetaData)
{
  ...
  if(ComovingCoordinates){
    if(MHD_Used){
        if (MHDCosmologyGetUnits(&DensityUnits, &LengthUnits, &TemperatureUnits,
                          &TimeUnits, &VelocityUnits, MetaData.Time,&BFieldUnits) == FAIL) {
  ...
  fprintf(fptr, "TimeLastAnalysis      = %"GOUTSYM"\n", MetaData.TimeLastAnalysis);
  fprintf(fptr, "dtAnalysis            = %"GOUTSYM"\n", MetaData.dtAnalysis);
  ...
  fprintf(fptr, "CycleSkipGlobalDataDump = %d\n\n", //AK
  ...
  /* MHD stuff.  Right here in the middle. */

  fprintf(fptr, "MHD_Equation                = %d\n", MHD_Equation);
  fprintf(fptr, "LoadBalancing               = %d\n", LoadBalancing);
  fprintf(fptr, "MaximumSubgridSize          = %d\n", MaximumSubgridSize);
  fprintf(fptr, "LoadBalancingRatio          = %"GOUTSYM"\n", LoadBalancingRatio);

  fprintf(fptr, "GlobalStatsCycleSkip        = %d\n",GlobalStatsCycleSkip);
  fprintf(fptr, "GlobalStatsLastCycleDump    = %d\n",GlobalStatsLastCycleDump);
  fprintf(fptr, "GlobalStatsDT               = %"GOUTSYM"\n",GlobalStatsDT);
  fprintf(fptr, "GlobalStatsLastTimeDump     = %"GOUTSYM"\n",GlobalStatsLastTimeDump);


#ifdef ATHENA
  fprintf(fptr, "EquationOfState        = %d\n", EquationOfState);
  fprintf(fptr, "MHD_Flattening        = %d\n", MHD_Flattening);
  ...
  fprintf(fptr, "MHD_ElectricRecon      =  %d\n", MHD_ElectricRecon);
  fprintf(fptr, "MHD_DivBparam           =  %"GOUTSYM"\n", MHD_DivBparam);
  fprintf(fptr, "MHD_FixedTimestep      =  %"GOUTSYM"\n", MHD_FixedTimestep);
  fprintf(fptr, "MHD_PLM_Slope          = ");
  WriteListOfInts(fptr, MAX_NUMBER_OF_BARYON_FIELDS+2, MHD_PLM_Slope);
  fprintf(fptr, "MHD_ReconField         = %d %d\n", MHD_ReconField[0], MHD_ReconField[1]);
#endif //ATHENA 
 fprintf(fptr, "MHD_Used               = %d\n", MHD_Used);
  fprintf(fptr, "MHD_SendFace           = %d\n", MHD_SendFace);
  fprintf(fptr, "MHD_FluxCorrection     = %d\n", MHD_FluxCorrection);
  fprintf(fptr, "MHD_CenteringMethod    = %d\n", MHD_CenteringMethod);
  fprintf(fptr, "MHD_DivB               = %d\n", MHD_DivB);
  fprintf(fptr, "MHD_InterpolationMethod    = %d\n", MHD_InterpolationMethod);
  fprintf(fptr, "MHD_Hack2d             = %d\n", MHD_Hack2d);

  fprintf(fptr, "WriteBoundary          = %d\n", WriteBoundary);
  fprintf(fptr, "MHD_WriteFace          = %d\n", MHD_WriteFace);
  fprintf(fptr, "MHD_WriteCentered      = %d\n", MHD_WriteCentered);
  fprintf(fptr, "MHD_WriteCurrent       = %d\n", MHD_WriteCurrent);
  fprintf(fptr, "MHD_WriteAcceleration = %d\n", MHD_WriteAcceleration);
  fprintf(fptr, "MHD_WriteElectric      = %d\n", MHD_WriteElectric);
  fprintf(fptr, "MHD_WriteDivB          = %d\n", MHD_WriteDivB);
  fprintf(fptr, "MHD_pout               = %d\n", MHD_pout);
  fprintf(fptr,"ProcessorTopology      = %d %d %d \n",
	  ProcessorTopology[0], ProcessorTopology[1], ProcessorTopology[2]);
  fprintf(fptr, "huge_number            = %e\n", huge_number);
  fprintf(fptr, "tiny_number            = %e\n", tiny_number);
  fprintf(fptr, "tiny_density           = %e\n", tiny_density);

#ifdef NSS
  fprintf(fptr, "MHD_Eps                = %f %f %f %f\n",
	  MHD_Eps[0],MHD_Eps[1],MHD_Eps[2],MHD_Eps[3]);
#endif

   fprintf(fptr, "tiny_pressure            = %e\n", tiny_pressure);
  ...
  if(MHD_Used){
    for(dim = 0;dim<3;dim++){
      if (MHDLabel[dim])
	fprintf(fptr, "MHDLabel[%d]              = %s\n", dim, MHDLabel[dim]);
      if (MHDUnits[dim])
	fprintf(fptr, "MHDUnits[%d]              = %s\n", dim, MHDUnits[dim]);
    }
    for(dim = 0;dim<3;dim++){
      if (MHDcLabel[dim])
	fprintf(fptr, "MHDcLabel[%d]              = %s\n", dim, MHDcLabel[dim]);
      if (MHDcUnits[dim])
	fprintf(fptr, "MHDcUnits[%d]              = %s\n", dim, MHDcUnits[dim]);
    }
    for(dim = 0;dim<3;dim++){
      if (MHDeLabel[dim])
	fprintf(fptr, "MHDeLabel[%d]              = %s\n", dim, MHDeLabel[dim]);
      if (MHDeUnits[dim])
	fprintf(fptr, "MHDeUnits[%d]              = %s\n", dim, MHDeUnits[dim]);
    }
#ifdef HAOXU
     fprintf(fptr, "#MagneticFieldCGSConversionFactor = %g\n",BFieldUnits);
#endif
    for(dim = 0;dim<3;dim++){
      if (CurrentLabel[dim])
	fprintf(fptr, "CurrentLabel[%d]              = %s\n", dim, CurrentLabel[dim]);
    }
  }
  fprintf(fptr, "\n");

#ifdef EMF_BOUNDARY
   fprintf(fptr, "EMF_Boundary             = %d \n\n", EMF_Boundary);
#endif // EMF_BOUNDARY
  ...
  fprintf(fptr, "RandomForcing               = %d\n", RandomForcing);     //AK
  fprintf(fptr, "RandomForcingEdot           = %g\n", RandomForcingEdot); //AK
  ...
  fprintf(fptr, "RefineByJeansLengthUnits  = %d\n", 
  ...
  fprintf(fptr, "ExternalBoundaryIO              = %d\n", ExternalBoundaryIO);
  fprintf(fptr, "ExternalBoundaryTypeIO          = %d\n", ExternalBoundaryTypeIO);
  fprintf(fptr, "ExternalBoundaryValueIO         = %d\n", ExternalBoundaryValueIO);
  ...
  /* write out analysis parameters */
  fprintf(    fptr, "AnalysisBaseName               = %s\n",
  ...
  if( MetaData.ProjectionParams ){
	...
	if( MetaData.ProjectionParams->ProjectionTitle[p] != NULL )
	  fprintf(fptr, "ProjectionTitle[%d]  = %s\n",p,
		  MetaData.ProjectionParams->ProjectionTitle[p] );
	
	fprintf(fptr,"ProjectionAxis[%d]  = %d\n",p, MetaData.ProjectionParams->ProjectionAxis[p]);
	fprintf(fptr,"ProjectionLevel[%d]  = %d\n",p, MetaData.ProjectionParams->ProjectionLevel[p]);
	fprintf(fptr,"ProjectionRightEdge[%d]  = %f %f %f\n",
	...
	fprintf(fptr,"ProjectionLeftEdge[%d]  = %f %f %f\n",
	...
	fprintf(fptr,"ProjectionFieldList[%d] = ",p);
	for( int q=0;q<MAX_PROJ_FIELDS;q++)
	  if( MetaData.ProjectionParams->fields[p][q] != FieldUndefined )
	    fprintf(fptr," %s", get_field_name(MetaData.ProjectionParams->fields[p][q]));
	fprintf(fptr,"\n");
	
      }
    }

  fprintf(stderr,"ProjectionOutputNumber = %d\n",MetaData.ProjectionParams->output_number);
  }
...
#ifdef HAOXU
    /* Write ShockPool parameters to parameter output file */

  if (ProblemType == 3) {
    fprintf(fptr, "ShockPoolAngle        = %f\n"  , ShockPoolAngle);
    fprintf(fptr, "ShockPoolMachNumber   = %f\n\n", ShockPoolMachNumber);

    fprintf(fptr, "ShockPoolDensity      = %f\n"  , ShockPoolDensity);
    fprintf(fptr, "ShockPoolPressure     = %f\n"  , ShockPoolPressure);
    fprintf(fptr, "ShockPoolVelocity1    = %f\n"  , ShockPoolVelocity[0]);
    fprintf(fptr, "ShockPoolVelocity2    = %f\n"  , ShockPoolVelocity[1]);
    fprintf(fptr, "ShockPoolVelocity3    = %f\n\n", ShockPoolVelocity[2]);
  ...
  if (MetaData.PDFGeneratorParams != NULL){
     fprintf( fptr, "PDFAnalysis                     = 1\n");
     fprintf( fptr, "PDFGeneratorMaximumLevel        = %"ISYM"\n", 
	      MetaData.PDFGeneratorParams->max_level);
     fprintf( fptr, "PDFGeneratorNumberOfPDFBins     = %"ISYM"\n", 
	      MetaData.PDFGeneratorParams->num_pdf_bins);
     fprintf( fptr, "PDFGeneratorCurrentOutputNumber = %"ISYM"\n", 
	      MetaData.PDFGeneratorParams->output_number);    
   }else{
     fprintf( fptr, "PDFAnalysis                    = 0\n");
   }

  if (MetaData.ProjectionParams != NULL){
     fprintf( fptr, "ProjectionAnalysis             = 1\n");
   }else{
     fprintf( fptr, "ProjectionAnalysis             = 0\n");
   }
