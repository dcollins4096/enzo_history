...
if(ComovingCoordinates==1){
   ...
   ElectricField[field][i] /= a[2];  
    ...
    int dbgPPML = 0; //for debugging PPML: shifts all fluxes 'ahead' by 1.
    for( dim=0; dim<3; dim++){
	...
	sprintf(basename, "data%d%d%d.grid",WriteInThisNumber,CycleNumber,level, grid);
		/*WriteInThisNumber, CycleNumber, level);*/
    ...
    if( WriteInThisF(WriteInThisNumber) == TRUE ){
      sprintf(basename, "data%d%d%d.grid",WriteInThisNumber,CycleNumber, level);
