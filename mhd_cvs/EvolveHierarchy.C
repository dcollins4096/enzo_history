		    ...
		    LevelHierarchyEntry *LevelArray[], float Initialdt)
{
...
#ifdef PRGIO_HACK
  //PRGIOhack is for changing the number of grids.  See my web page.
  if( PRGIOhack == TRUE){
    fprintf(stderr," ******************************\n");
    fprintf(stderr," ********* PRGIO HACK *********\n");
    
    if(ParallelRootGridIO == 0){
      ParallelRootGridIO = 1;
      fprintf(stderr," ****** Turning PRGIO on ******\n");
      
    } else{
      ParallelRootGridIO = 0;
      fprintf(stderr," ****** Turning PRGIO off *****\n");
    }
    fprintf(stderr," ******************************\n");     
  }//endif
#endif //  PRGIO_HACK
  /* MHD Exits.  MHD only supports certain things, currently.  If you're trying to 
     access somehting that isn't supported, exit. The exit routine will give more info. */

  if( MHDExit(TopGrid.GridData, &MetaData) == FAIL ) return FAIL;
  ...
  if (MetaData.Time        >= MetaData.StopTime ){
    Stop = TRUE;
    fprintf(stderr,"Right out of the gate, Time (%f) > Stop Time (%f)\n I bet you didn't mean that.\n"
	    ,MetaData.Time, MetaData.StopTime); 
  }
  if (MetaData.CycleNumber >= MetaData.StopCycle){ 
    Stop = TRUE;
    fprintf(stderr,"Right out of the gate, Cycle (%d) > Stop cydle (%d)\n I bet you didn't mean that.\n"
	    ,MetaData.CycleNumber, MetaData.StopCycle); 
  }
  ...
  //fprintf(stderr, "np %d EHDBG 03 \n", MyProcessorNumber);
  MPI_Barrier(MPI_COMM_WORLD);
  
  /* Check for output. */
  JBMEM_MESSAGE(MyProcessorNumber,"jb: BeforeCheckForOutput");
  ...
  JBMEM_MESSAGE(MyProcessorNumber,"jb: AfterCheckForOutput");
...
*/
  ...
  //fprintf(stderr, "np %d EHDBG 05 \n", MyProcessorNumber);
  //MPI_Barrier(MPI_COMM_WORLD);


    /* Compute minimum timestep on the top level. This is done here in order
     to ensure dtParent is set properly for the MHD updates.*/
  ...
  JBMEM_MESSAGE(MyProcessorNumber,"jb: AfterRebuildHierarchy");
  ...
  //fprintf(stderr, "np %d EHDBG 06 \n", MyProcessorNumber);
  //MPI_Barrier(MPI_COMM_WORLD);

  /* ====== MAIN LOOP ===== */
  //(Flow chart directive)
  //dcf for all time dcg 
  while (!Stop) {
    wall_time_flush();
    wall_time("Start MainLoop");
    if( MyProcessorNumber == ROOT_PROCESSOR) {
      fprintf(stderr, "================= Evolve Hierarchy: New Step.= %d ======\n",MetaData.CycleNumber );
    }


    /* Output level information to log file. */
    
    JBMEM_MESSAGE(MyProcessorNumber,"jb: EnterTimestepLoop");
    if (MyProcessorNumber == ROOT_PROCESSOR) {
...
#endif
    //if (debug) { //dcc always on
    if (MyProcessorNumber == ROOT_PROCESSOR) {
      fprintf(stderr, "TopGrid cycle = %d dt = %.14g     time = %.14g", 
	      MetaData.CycleNumber,dt, MetaData.Time);
      if (ComovingCoordinates) {
	FLOAT a, dadt;
    ...
    JBMEM_MESSAGE(MyProcessorNumber,"jb: CallEvolveLevel");
    ...
    JBPERF_ADVANCE;
    JBPERF_WRITE;
    JBMEM_MESSAGE(MyProcessorNumber,"jb: BackInEvolveHierarchy");
    //=================================================================
    ...
    JBMEM_MESSAGE(MyProcessorNumber,"jb: BetweenTimeActionAndOutput");
    ...
    JBMEM_MESSAGE(MyProcessorNumber,"jb: AfterOutput");
    ...
    JBMEM_MESSAGE(MyProcessorNumber,"jb: AfterReduceFragmentation");

#endif /* REDUCE_FRAGMENTATION */

    JBMEM_MESSAGE(MyProcessorNumber,"jb: EndOfMainLoop");
    wall_time("End MainLoop");
  } // ===== end of main loop ====
  //Flow chart directives:
  //dcf endfor dcg 
