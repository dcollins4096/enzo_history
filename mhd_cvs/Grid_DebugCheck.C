...
int grid::DebugCheck(char *message)
{
  ...
  for (field = 0; field < NumberOfBaryonFields; field++)
	...
	for(i=0;i<GridDimension[0];i++){
	  ...
	  if( Shit == FALSE ){
	    if( isnan(BaryonField[field][index]) ){
	      fprintf(stderr, "DebugCheck[%s](Proc %d nan): BaryonField[%d][%d, %d, %d]= %g Left = %13g %13g %13g Width %13g\n\n", message, 
		      MyProcessorNumber, field, i,j,k, BaryonField[field][index],
		      GridLeftEdge[0],GridLeftEdge[1],GridLeftEdge[2], CellWidth[0][0]);
	      Shit=TRUE;
	    }
	    if( isinf(BaryonField[field][index]) ){
	      fprintf(stderr, "DebugCheck[%s](Proc %d inf): BaryonField[%d][%d, %d, %d]= %g Left = %13g %13g %13g Width %13g\n\n", message, 
		      MyProcessorNumber, field, i,j,k, BaryonField[field][index],
		      GridLeftEdge[0],GridLeftEdge[1],GridLeftEdge[2], CellWidth[0][0]);
	      Shit=TRUE;
	    }
