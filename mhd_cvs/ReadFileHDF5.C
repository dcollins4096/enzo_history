...
// This routine reads only data from Inits: inits_type is 32- or 64-bit
  ...
  if (io_log) fprintf(log_fptr, "RHDF size of inits_type is %d\n", ii);
  ...
  fprintf(stderr, "RHDF H5Fopen %s on CPU %d\n", name, MyProcessorNumber);
  ...
  if (Npart != component_rank_attr) {
    fprintf(stderr, "Npart (%d) and Component_Rank (%d) do not agree!\n",
	    Npart, component_rank_attr);
    return FAIL;
  }

  if (TempInt != Rank) {
    fprintf(stderr, "Rank mismatch in %s. field_rank_attr = %d, Rank = %d\n", name,
	    TempInt, Rank);
    return FAIL;
  }
  ...
  if (ParallelRootGridIO == FALSE)
	...
	for(int UsefulInfo=0; UsefulInfo<Rank;UsefulInfo++){
	  fprintf(stderr,"    field_dims_attr[%d] = %d, %d - %d +1 = %d\n",
		  UsefulInfo,TempIntArray[UsefulInfo], EndIndex[UsefulInfo], StartIndex[UsefulInfo], 
		  EndIndex[UsefulInfo]-StartIndex[UsefulInfo]+1);
	}
	return FAIL;
      }
  ...
  if (io_log) fprintf(log_fptr, "Allocate %d inits_types for *tempbuffer\n", size);
  ...
  for (dim=0; dim < Rank; dim++)
  {
    ...
    for ( dim = 1; dim < Slab_Rank; dim++ )
    {
      ...
      file_count[dim] = TempIntArray[Rank-dim];  // field dimensions
