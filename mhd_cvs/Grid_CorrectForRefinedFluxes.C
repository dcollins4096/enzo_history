...
(Gamma-1.0)/POW(BaryonField[DensNum][index],Gamma-1.0);
                      BaryonField[GENum][index+Offset] *=
(Gamma-1.0)/POW(BaryonField[DensNum][index+Offset],Gamma-1.0);
                    }
                  }
       if(MHD_Equation == 1){
        BaryonField[TENum][index] += 0.5*(1.0/a-1.0)*
       (CenteredB[0][index]*CenteredB[0][index]+ 
        CenteredB[1][index]*CenteredB[1][index]+ 
        CenteredB[2][index]*CenteredB[2][index]);
         BaryonField[TENum][index+Offset] += 0.5*(1.0/a-1.0)*
       (CenteredB[0][index+Offset]*CenteredB[0][index+Offset]+
        CenteredB[1][index+Offset]*CenteredB[1][index+Offset]+
        CenteredB[2][index+Offset]*CenteredB[2][index+Offset]);
       } //MHD_Equation
      }//MHD_Used 
...
#ifdef HAOXU
   float temp1, temp2;
// if( FieldType[field] == InternalEnergy && HydroMethod == 6 )
//                  continue;
#endif
	      for (k = Start[2]; k <= End[2]; k++){
		for (j = Start[1]; j <= End[1]; j++){
 ...
 if ((FieldTypeIsDensity(FieldType[field]) == TRUE ||
                      ...
                      fprintf(stderr,"ERROR: Before CorrectForRefinedFluxes \n");
  ...
  temp1 = BaryonField[field][FieldIndex];
  temp2 = BaryonField[field][FieldIndex+Offset];
#endif //HAOXU
			...
			(BaryonField[field][FieldIndex] <= 0 ||
			 BaryonField[field][FieldIndex+Offset] <= 0)) {
		      ...
		      fprintf(stderr,"      value = %f, +offset %f\n", 
			      ...
			      BaryonField[field][FieldIndex+Offset]);
#ifdef HAOXU
      BaryonField[field][FieldIndex] = temp1;
      BaryonField[field][FieldIndex+Offset] = temp2;
      fprintf(stderr,"      value = %f, +offset %f\n",
                              BaryonField[field][FieldIndex],
                              BaryonField[field][FieldIndex+Offset]);
#endif /* HAOXU */
    ...
    if( CorrectLeftBaryonField || CorrectRightBaryonField ){
      ...
      for (k = Start[2]; k <= End[2]; k++)
                  for (j = Start[1]; j <= End[1]; j++) { 
                    ...
                    for (i = Start[0]; i <= End[0]; i++, index++) {
                      BaryonField[GENum][index] *=
     POW(BaryonField[DensNum][index],Gamma-1.0)/(Gamma-1.0);
                      BaryonField[GENum][index+Offset] *=
     POW(BaryonField[DensNum][index+Offset],Gamma-1.0)/(Gamma-1.0);
                    }
                  }
        if(MHD_Equation == 1){
        BaryonField[TENum][index] += 0.5*(1.0-1.0/a)*
       (CenteredB[0][index]*CenteredB[0][index]+
        CenteredB[1][index]*CenteredB[1][index]+
        CenteredB[2][index]*CenteredB[2][index]); 
         BaryonField[TENum][index+Offset] += 0.5*(1.0-1.0/a)*      
       (CenteredB[0][index+Offset]*CenteredB[0][index+Offset]+
        CenteredB[1][index+Offset]*CenteredB[1][index+Offset]+
        CenteredB[2][index+Offset]*CenteredB[2][index+Offset]);
       } //MHD_Equation
...
#ifdef HAOXU
/*              if(MHD_Used == TRUE){
                 int temp_ge1 = (BaryonField[TENum][i1]-0.5*(pow(CenteredB[0][i1],2)+pow(CenteredB[1][i1],2)+pow(CenteredB[2][i1],2))
                               - 0.5*BaryonField[DensNum][i1]*( BaryonField[Vel1Num][i1] * BaryonField[Vel1Num][i1]
                                                               +BaryonField[Vel2Num][i1] * BaryonField[Vel2Num][i1]
                                                               +BaryonField[Vel3Num][i1] * BaryonField[Vel3Num][i1]))/BaryonField[DensNum][i1];
                 int temp_ge2 = (BaryonField[TENum][i2]-0.5*(pow(CenteredB[0][i2],2)+pow(CenteredB[1][i2],2)+pow(CenteredB[2][i2],2))
                               - 0.5*BaryonField[DensNum][i2]*( BaryonField[Vel1Num][i2] * BaryonField[Vel1Num][i2]
                                                               +BaryonField[Vel2Num][i2] * BaryonField[Vel2Num][i2]
                                                               +BaryonField[Vel3Num][i2] * BaryonField[Vel3Num][i2]))/BaryonField[DensNum][i2];
                 BaryonField[GENum][i1] = max(BaryonField[GENum][i1],temp_ge1);
                 BaryonField[GENum][i2] = max(BaryonField[GENum][i2],temp_ge2);
          }
*/
#endif
...
//change the total energy to the form of MHDENZO
	...
	for( field=0;field<3;field++){
	  for( int dim2=0; dim2<3; dim2++){
	    if( RefinedFluxes->LeftElectric[field][dim2] != NULL ){
	      delete [] RefinedFluxes->LeftElectric[field][dim2];
	      RefinedFluxes->LeftElectric[field][dim2] = NULL;
	    }
	    if( RefinedFluxes->RightElectric[field][dim2] != NULL ){
	      delete [] RefinedFluxes->RightElectric[field][dim2];
	      RefinedFluxes->RightElectric[field][dim2] = NULL;
	    }
