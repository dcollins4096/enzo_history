...
/***********************************************************************
/
/  GRID CLASS (INITIALIZE THE GRID FOR A MHD COLLAPSE TEST)
/
/
...
int grid::MHDCollapseTestInitializeGrid(int NumberOfSpheres,
...
#define NFW_POINTS 500
  float NFWMass[NFW_POINTS], NFWPressure[NFW_POINTS], NFWTemp[NFW_POINTS], x1,
        NFWDensity[NFW_POINTS], NFWSigma[NFW_POINTS];
  ...
  for (i = 0; i < NFW_POINTS; i++) {
    ...
    fprintf(fptr, "%d %"GOUTSYM" %g %g %g %g %g\n", i, NFWRadius[i], 
  ...
  for (k = 0; k < GridDimension[2]; k++)
	...
	density = BackgroundDensity; //0.298;
...
//		float vel = sqrt(PointSourceGravityConstant/drad)/drad;
	...
	if( EquationOfState == 0 ){
	  ...
	  if (DualEnergyFormalism)
	    BaryonField[2][n] = BaryonField[1][n];
          ...
          BaryonField[1][n] *=BaryonField[0][n];
	  if (HydroMethod != Zeus_Hydro){
	    for (dim = 0; dim < GridRank; dim++)
	      BaryonField[1][n] += 0.5*POW(BaryonField[vel+dim][n], 2)*BaryonField[0][n];
	  }
#ifdef ATHENA
	}
#endif //ATHENA
	...
	if (SphereUseParticles)
	  if (i >= GridStartIndex[0] && i <= GridEndIndex[0] &&
		...
		if (sigma != 0)
		  for (dim = 0; dim < GridRank; dim++)
		    ParticleVelocity[dim][npart] += 
		      gasdev_mhd()*sigma/VelocityUnits;
  ...
  if (SphereUseParticles && debug)
    printf("CollapseTestInitialize: NumberOfParticles = %d\n", 
	   NumberOfParticles);

    //Initialize Magnetic and Electric Fields
  
 if(0){    //try to initialize magnetic field weighted by density,don't work yet
   ...
   for( k=0;k<ElectricDims[field][2];k++)
    for( j=0;j<ElectricDims[field][1];j++)
      for( i=0;i<ElectricDims[field][0];i++){
        ...
        if(field==0)
          ElectricField[field][index]=MHDCollapseTestMagneticFieldz*0.25*j*(*CellWidth[1])*
       (BaryonField[0][i+ElectricDims[field][0]*(j%ElectricDims[field][1]+ElectricDims[field][1]*(k%ElectricDims[field][2]))]
+BaryonField[0][i+ElectricDims[field][0]*(j+ElectricDims[field][1]*((k-1+ElectricDims[field][2])%ElectricDims[field][2]))]
+BaryonField[0][i+ElectricDims[field][0]*((j-1+ElectricDims[field][1])%ElectricDims[field][1]+ElectricDims[field][1]*k)]
+BaryonField[0][i+ElectricDims[field][0]*((j-1+ElectricDims[field][1])%ElectricDims[field][1]+ElectricDims[field][1]*((k-1+ElectricDims[field][2])%ElectricDims[field][2]))]);
       
        if(field==1)
          ElectricField[field][index]=MHDCollapseTestMagneticFieldx*0.25*k*(*CellWidth[2])* 
       (BaryonField[0][i%ElectricDims[field][0]+ElectricDims[field][0]*(j+ElectricDims[field][1]*(k%ElectricDims[field][2]))]  
+BaryonField[0][i+ElectricDims[field][0]*(j+ElectricDims[field][1]*((k-1+ElectricDims[field][2])%ElectricDims[field][2]))]
+BaryonField[0][(i-1+ElectricDims[field][0])%ElectricDims[field][0]+ElectricDims[field][0]*(j+ElectricDims[field][1]*k)]
+BaryonField[0][(i-1+ElectricDims[field][0])%ElectricDims[field][0]+ElectricDims[field][0]*(j+ElectricDims[field][1]*((k-1+ElectricDims[field][2])%ElectricDims[field][2]))]);
 
         if(field==2)
          ElectricField[field][index]=MHDCollapseTestMagneticFieldy*0.25*i*(*CellWidth[0])*
       (BaryonField[0][i%ElectricDims[field][0]+ElectricDims[field][0]*(j%ElectricDims[field][1]+ElectricDims[field][1]*k)]
+BaryonField[0][i+ElectricDims[field][0]*((j-1+ElectricDims[field][1])%ElectricDims[field][1]+ElectricDims[field][1]*k)]
+BaryonField[0][(i-1+ElectricDims[field][0])%ElectricDims[field][0]+ElectricDims[field][0]*(j+ElectricDims[field][1]*k)]
+BaryonField[0][(i-1+ElectricDims[field][0])%ElectricDims[field][0]+ElectricDims[field][0]*((j-1+ElectricDims[field][1])%ElectricDims[field][1]+ElectricDims[field][1]*k)]);
}
     }
...
/*
 for(field=0;field<=2;field++){
   ...
   for( k=0;k<ElectricDims[field][2];k++)
    for( j=0;j<ElectricDims[field][1];j++)
      for( i=0;i<ElectricDims[field][0];i++){
        index=k+ElectricDims[field][2]*(j+ElectricDims[field][1]*i);
          
        if(field==0)
          ElectricField[field][index]=MHDCollapseTestMagneticFieldz*0.25*j*(*CellWidth[1])*
       (BaryonField[0][k%ElectricDims[field][2]+ElectricDims[field][2]*(j%ElectricDims[field][1]+ElectricDims[field][1]*i)]  
+BaryonField[0][k+ElectricDims[field][2]*((j-1+ElectricDims[field][1])%ElectricDims[field][1]+ElectricDims[field][1]*i)]
+BaryonField[0][(k-1+ElectricDims[field][2])%ElectricDims[field][2]+ElectricDims[field][2]*(j+ElectricDims[field][1]*i)]
+BaryonField[0][(k-1+ElectricDims[field][2])%ElectricDims[field][2]+ElectricDims[field][2]*((j-1+ElectricDims[field][1])%ElectricDims[field][1]+ElectricDims[field][1]*i)]);
                          
        if(field==1)
          ElectricField[field][index]=MHDCollapseTestMagneticFieldx*0.25*k*(*CellWidth[2])*
       (BaryonField[0][k%ElectricDims[field][2]+ElectricDims[field][2]*(j+ElectricDims[field][1]*(i%ElectricDims[field][0]))]
+BaryonField[0][(k-1+ElectricDims[field][2])%ElectricDims[field][2]+ElectricDims[field][2]*(j+ElectricDims[field][1]*i)]
+BaryonField[0][k+ElectricDims[field][2]*(j+ElectricDims[field][1]*((i-1+ElectricDims[field][0])%ElectricDims[field][0]))]
+BaryonField[0][(k-1+ElectricDims[field][2])%ElectricDims[field][2]+ElectricDims[field][2]*(j+ElectricDims[field][1]*((i-1+ElectricDims[field][0])%ElectricDims[field][0]))]);
          
         if(field==2)
          ElectricField[field][index]=MHDCollapseTestMagneticFieldy*0.25*i*(*CellWidth[0])*
       (BaryonField[0][k+ElectricDims[field][2]*(j%ElectricDims[field][1]+ElectricDims[field][1]*(i%ElectricDims[field][0]))]
+BaryonField[0][k+ElectricDims[field][2]*(j+ElectricDims[field][1]*((i-1+ElectricDims[field][0])%ElectricDims[field][0]))]
+BaryonField[0][k+ElectricDims[field][2]*((j-1+ElectricDims[field][1])%ElectricDims[field][1]+ElectricDims[field][1]*i)]
+BaryonField[0][k+ElectricDims[field][2]*((j-1+ElectricDims[field][1])%ElectricDims[field][1]+ElectricDims[field][1]*((i-1+ElectricDims[field][0])%ElectricDims[field][0]))]);
}
     }
*/
  for(i=0;i<MagneticSize[0];i++) MagneticField[0][i] = 0.0;
  for(i=0;i<MagneticSize[1];i++) MagneticField[1][i] = 0.0;
  for(i=0;i<MagneticSize[2];i++) MagneticField[2][i] = 0.0;
...
#endif //ATHENA
    for(i=0;i < size; i++) 
      BaryonField[1][i] += 0.5*POW(CenteredB[0][i],2)+0.5*POW(CenteredB[1][i],2)+0.5*POW(CenteredB[2][i],2);
#ifdef ATHENA
  }
#endif //ATHENA  
  ...
  for(i=0;i < size; i++) {
          CenteredB[0][i] = MHDCollapseTestMagneticFieldx;
          CenteredB[1][i] = MHDCollapseTestMagneticFieldy;
          CenteredB[2][i] = MHDCollapseTestMagneticFieldz;
#ifdef ATHENA
	  if( EquationOfState == 0 ){
#endif //ATHENA
  ...
  for(i=0;i<MagneticSize[0];i++) MagneticField[0][i] = MHDCollapseTestMagneticFieldx;
  for(i=0;i<MagneticSize[1];i++) MagneticField[1][i] = MHDCollapseTestMagneticFieldy;
  for(i=0;i<MagneticSize[2];i++) MagneticField[2][i] = MHDCollapseTestMagneticFieldz;
