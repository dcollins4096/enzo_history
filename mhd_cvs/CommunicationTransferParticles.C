...
int CommunicationTransferParticles(grid *GridPointer[], int NumberOfGrids)
{
  ...
  if (NumberOfProcessors > 1) {
    ...
    JBPERF_START_MPI_GATHER("MPI_Allgatherv",GridsToSend,MPI_ParticleMoveList);
    CHECK_MPI_ERROR(MPI_Allgatherv(SendList, GridsToSend, MPI_ParticleMoveList,
				   ...
				   MPI_COMM_WORLD));
    JBPERF_STOP_MPI_GATHER("MPI_Allgatherv",GridsToSend,MPI_ParticleMoveList);
	...
	if (TransferSize > 0 && FromProcessor != ToProcessor) {
	  ...
	  if (FromProcessor == MyProcessorNumber) {
	    ...
	    JBPERF_START_MPI_SEND("MPI_Send",TransferSize*sizeof(float_int),MPI_BYTE);
	    CHECK_MPI_ERROR(MPI_Send(SharedList[j].Pointer[i], 
				     TransferSize*sizeof(float_int),
				     MPI_BYTE, ToProcessor, 
				     MPI_TRANSFERPARTICLE_TAG, 
				     MPI_COMM_WORLD));
	    JBPERF_STOP_MPI_SEND("MPI_Send",TransferSize*sizeof(float_int),MPI_BYTE);
	  ...
	  if (ToProcessor == MyProcessorNumber) {
	    ...
	    JBPERF_START_MPI_RECV("MPI_Recv",TransferSize*sizeof(float_int),MPI_BYTE);
	    ...
	    JBPERF_STOP_MPI_RECV("MPI_Recv",TransferSize*sizeof(float_int),MPI_BYTE);
	  }
