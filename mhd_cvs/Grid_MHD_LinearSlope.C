...
int grid::MHD_LinearSlope(float * Lhs, float * Rhs, int * index, int dim, int DifferenceOnly){
  int field; 
  ...
  for( field=0; field< NumberReconstructed; field ++ ){
    ...
    case 2:
      ...
      LeftDifference = MinMod( State[field][ index[1] ] - State[field][ index[3] ],
				    State[field][ index[0] ] - State[field][ index[1] ] );

      RightDifference = MinMod( State[field][ index[0] ] - State[field][ index[1] ],
    ...
    if( DifferenceOnly ){
      Lhs[field] = LeftDifference;
      Rhs[field] = RightDifference;
    }else{
      Lhs[field] = State[field][ index[1] ] + 0.5*LeftDifference;
      Rhs[field] = State[field][ index[0] ] - 0.5*RightDifference;
    }
  }
  return SUCCESS;
